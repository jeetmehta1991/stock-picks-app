...
**Mechanically enforced** by the B1744 auto-injection hook and scan_discipline_not_loaded (a substantive turn without the full skill in context is blocked). **B3097 CORRECTION (L871): at 380 KB the hook's output is persisted to a file with a 2 KB preview, so the hook no longer delivers this skill - scan_discipline_not_loaded is the live enforcement, and it fires at a close. Invoke the Skill after any compaction.** **S6-B3097b (B3102, owner-approved option a, 2026-09-24): the hook now emits a COMPACT INDEX - the invoke instruction, the brevity rule and the truth standard's core inside the 2 KB the harness always shows, then the first column of every tripwire row, capped at 16,000 characters. The index is a reminder, never the skill: only a Skill call delivers this file.**
...
JUDGMENT-ONLY: whether the checklists and learnings were actually READ is unobservable to any scan; the compensating mechanism is the B1744 auto-injection (this file arrives in context every turn) plus the anchor rules that keep lessons in the loaded files. **B3097 (L871): the auto-injection now arrives as a 2 KB preview - see the correction under Standing activation.** **S6-B3097b (B3102): it is now a compact index, not this file.**
...
- **THE SHIP-VS-ASK LINE (#310, S6-B2925, owner approved 2026-09-22):** does the change alter an owner-visible VERDICT, or the SET OF CELLS that receive one? If yes, ask; if it only changes which cells are looked at, or corrects a proven defect byte-identically elsewhere, ship. The recorded split case (is a RANKING rung a gate?) sits on the ASK side until ruled. Pinned verbatim: THE SHIP-VS-ASK LINE IS THE VERDICT TEST - DOES THE CHANGE ALTER AN OWNER-VISIBLE VERDICT OR THE SET OF CELLS THAT RECEIVE ONE; IF YES ASK, IF IT ONLY CHANGES WHAT IS LOOKED AT OR CORRECTS A PROVEN DEFECT BYTE-IDENTICALLY ELSEWHERE, SHIP.
...
| Answer an owner question of the form "why did this not run automatically?" | COUNT THE ASKS. A second question about the same mechanism is a class defect - enumerate every path from the event to the owner's screen and fix the OUTCOME (one supervisor invoked by the event's producer + a gate on the channel the owner reads), not the mechanism that just failed | L736 / #288 (six asks, six instance fixes: B2177, B2192, B2198/B2208, B2211, S6-B2436, S6-B2515; the sixth ask was the first ask) |
| Count anything held in an APPEND LOG - a ledger, a jsonl record, an event stream - OR look one up in it ("show me the open ones", "which rows are still X") | CALL ITS REDUCER. An append log answers only through last-row-wins / latest-per-key; a raw row count is a number about the FILE, not the world - always available, always plausible, and wrong the moment one key has two rows. One authoritative reader per log, and every consumer calls it | L794 / L737 / #271 - A LOOKUP IS NOT EXEMPT: "which are open" feels like a query rather than a statistic, so the reducer gets bypassed for exactly the read that reaches the owner as a decision set (MEASURED 2026-09-12: a hand-written regex said ~60 OPEN rows beside the reducer's 9, in the same response). (`queue_state.py` for the queue, `undelivered()` for landings; I printed "undelivered now: 6" from a raw comprehension where the reducer returns 0) |
| Decide whether a known data defect is worth correcting by reasoning about HOW MUCH data it touched - "only 6%", "noise", "it would not flip a ranking" | ASK WHICH QUANTITY THE DECISION READS AND WHERE ITS THRESHOLD SITS. A figure consulted for one bit - is the lower bound above zero - is destroyed by any perturbation that crosses zero and unharmed by a larger one that does not, so the SIZE of the contamination predicts nothing. And the correction is cheapest to skip exactly when the contaminated figure looks GOOD: the runs most worth re-grading are the ones you would least like to lose | L766 / #270 (MEASURED 2026-09-04: the span100 rerun graded is_ci_lo -0.078 on real ATR against +0.062 contaminated - a config sat in the positive set on a number priced off a 2pct proxy for 83.7% of its trades) |
| Price an option - "free", "nearly free", "just discipline", "only a re-grade" | OPEN THE PLAN ROW THAT DEFINES THE OPTION'S SHAPE, the last artifact that ran at that shape, and the mechanism that consumes its output - each EXECUTED / READ, or the price is UNVERIFIED. A price with no unit is the tell | L767 / L506(b) ext (MEASURED 2026-09-05: "nearly free" for a holdout shot whose plan row is 4y x 544 tickers - 16-40 h in 4-9 legs - and whose grader did not exist; retracted S6-B2612c, re-ruled at the true price S6-B2612d) |
| Run the pyramid, then edit ANY file before committing - a banner count, an L-entry, a skill row, "just docs" | THE PYRAMID VOUCHES FOR THE TREE IT MEASURED, NOT THE ONE YOU COMMIT. Docs are pinned too (test_b1486 reads the banner, test_b2526 reads LEARNINGS against CHECKLIST / SKILL): write the L-entry, the banner and the skill row FIRST, run the pyramid, then append the queue row - the one file whose row must follow the run | L768 / #292 ext (MEASURED 2026-09-05: B2612's pyramid ended 08:35:16, L767 landed after it, commit 371a6484f at 08:39:54 shipped two RED pins C6 never saw - C6 tested freshness on .py only, the carve-out its own docstring disclaimed; fixed B2613) |
| Add a counter, a warning, a report or a gate to a fallback - "the proxy is now observable" | THE CLASS IS EVERY SITE THAT TAKES THE FALLBACK, NOT THE CALLER THE FINDING CAME THROUGH. Grep the EXPRESSION (the literal default in `.get(k, default)`, the `* 0.02`), not the function name - siblings are copies, not calls. A sibling with no landing to warn into fails CLOSED: derive from the data it already holds, count, refuse to write on any unresolved row - and test the derivation against the recorded values before saying it reproduces them | L769 / L642 ext (MEASURED 2026-09-05: B1261 counted the engine's 2pct ATR proxy; the identical expression in two offline re-scorers stayed silent for 1,354 batch numbers, and one of them was the mechanism a live owner ruling would run over a cube with 214 of 437 rows lacking the field; the first draft of the fix derived on the wrong bar set and matched 0 of 373 recorded values until checked; fixed B2615) |
| Write a row into an APPEND-ONLY record - the queue, a jsonl ledger, an event log - while you are still deriving the number that goes in it | DERIVE FIRST, THEN WRITE ONCE. An append log cannot be edited, so a half-thought in it is permanent: correcting it costs a SECOND row, and until a reader gets that far the garbled figure stays quotable. The property that makes the record trustworthy is exactly what makes sloppiness in it expensive | L765 / C14 (MEASURED 2026-09-04: a band row shipped reading `minq8 +0.043... wait, minq8 +0.054` and needed B2607b to restate it; preflight C14 now refuses a staged queue row carrying a drafting marker, with inline-code spans stripped so a row RECORDING the defect is not refused by the rule it records) |
| Generalise a failure you reproduced on YOUR OWN path - a refused commit, a rejected call, a blocked write - to a SECOND actor (the engine, a hook, a supervisor, CI) | OPEN THE SECOND ACTOR'S CODE AND FIND THE LINE THAT MAKES IT DIFFER. Reproducing a failure three times proves it is DETERMINISTIC, never whose path it is deterministic ON - the sample grows while the population stays at one. And if the second actor's outcomes are already recorded, read those first: they are the population and your reproductions are not | L764 / #215 (MEASURED 2026-09-04: three refused reporting commits filed as a P1 class defect against the engine's landing commit; postconfig_landing.py:220 stages its own ledger row, and the day's three landing commits carried hashes in the same file I was quoting failures from) |
| Say a file is tracked, untracked, committed or ignored - or sweep a group of files by that property | ASK GIT, do not infer from the directory or from what its siblings look like. `git ls-files <glob>` and `git status --short` are one command each, and a tracked-status CONVENTION is a belief about a repo rather than a fact about it | L735 / #271 ext (31 per-cube JSONs were tracked and ~39 identical siblings were not, so a citation resolved for the author and not for a reader) |
| Report that a detached job, scheduled task, service or remote run has STARTED | ASK THE SYSTEM THAT OWNS IT, never the launcher's own output. A success token and an error can share one buffer, so a membership test cannot see the contradiction. Name the LEVEL you verified - registered / started / running / doing work - because they fail separately | L738 / #122 (Register-ScheduledTask was DENIED and the helper still printed registered_and_started; I published a launch for a task that did not exist) |
| Commit a byte-level edit to a tracked file | READ THE DIFF SIZE FIRST - `git diff --numstat` - and compare it to the edit you INTENDED. An order-of-magnitude gap means something happened you did not author. The check needs no theory of the cause, which is why it is the one worth having | L739 / S6-B2533 (a ~15-line addendum committed as 18,248 insertions and 18,226 deletions when a file's stored form flipped; content intact, the lesson ledger's reviewability destroyed) |
| Put a threshold, cap, floor or tie-break in a sentence that also carries a measured number | LABEL THE KIND - measured, derived, or CHOSEN - and give the chosen one its criterion. A value you picked, standing next to one you measured, borrows the measurement's authority for free, and #201 does not catch it because a chosen number has no source to name | L740 / #201 + #165 ("re-armed at 0.7 GB, below the measured band" - the band was measured, the 0.7 was picked, and the sentence hid which was which) |
| Generate a config, spec or manifest by copying an existing one | NAME ITS CONSUMER AND OPEN THE PARSER. #230a covers artifacts you READ; this is the mirror - an artifact you WRITE owes its consumer's required fields. If the template comes from a different family or tier, list what the two do NOT share before copying: that list is the set of fields that will be silently wrong | L741 / #230a (16 specs copied from an SMC template lacked the institutional family's four graded keys; the engine ran fine, all 16 landed ungradeable) |
| Say that nothing calls / reads / consumes something, or that it is unwired | SEARCH THE TREE, NOT THE FILE. "X calls this" needs one hit and self-verifies; "nothing calls this" is a claim about every file NOT in your search, so name the directories searched, the hit count, and what was excluded. A single-file grep supports "the definition is here" and nothing else | L742 / #280 + L499 (grepped table_c's own file, found only the def, and nearly reported a function unwired that has a dedicated renderer) |
| Promise the owner periodic updates on a long run | ASK WHICH CHANNEL OUTLIVES THIS SESSION. Monitors and crons are session-scoped and die silently - and a dead reporter looks exactly like a quiet period. Harden the REPORTING channel to the same standard as the job: a file the next session reads, plus a gate that blocks until it is delivered. Say which updates are best-effort and which are guaranteed | L743 / L637 (the chain ran detached and survived; the three Monitors watching it did not, and the owner asked why nothing was arriving) |
| Rely on a rule that says MECHANICALLY ENFORCED | A CONTROL HAS THREE PARTS - detector, requirement, remedy - and 'enforced' usually covers only one. RUN THE ENFORCEMENT AGAINST ITS OWN STATED EXAMPLE before trusting it, and check that the prescribed REMEDY can satisfy the requirement (a session-scoped reporter cannot monitor a detached job, however well worded) | L744 / #185 + #186 (the launch gate returned NOTHING for a bare run_phase1a.py, its own canonical example, while its mandated CronCreate remedy is session-only) |
| Describe your own work's coverage as partial, limited, or the easy part - OR attach a CAVEAT to your own proposal ('honest about X, silent about Y', 'this would not capture', 'only a partial view') | MEASURE IT ANYWAY. #230 EXT applies regardless of DIRECTION: a self-deprecating number feels like candour, and candour does not trigger the scepticism self-praise does, so the bias check never fires. Understating coverage invites the reader to fund work already done | L745 / #230 EXT / #278 (instance 1: called a corpus extension substantial; the file showed 43 of 44 gates already covered. INSTANCE 2, 2026-09-13: invented a caveat that a filtered cube's exits were 'never re-simulated' - two file-opens refuted it, the filter is EXACT, and the sweep found 1 of 4 stated limitations that day was invented, the only one living in prose rather than in a file) |
| Report that a gate or check is fixed in BOTH directions | COUNT THE SHAPES YOU TESTED. #182's denominator rule applies to your own test suite: a must-fire arm plus a must-quiet arm is a sample of size two from an open family of inputs, and the suite's two-category vocabulary is what makes it feel exhaustive. Name the shapes you did NOT try, or admit the residual | L746 / #182 (four arms passed, then the gate flagged my own heredoc and nested `python -c` - the quiet side was an open family, tested once) |
| Baseline a backlog with a shrink-only ratchet | NAME THE MEMBERS AND GIVE EACH A REASON, not just a count. A number stops regrowth but treats every member as interchangeable, so nobody can tell a legitimate exception from an abandoned one, and a later drop records neither which were fixed nor whether the right ones were. A bare integer is for a population too large to enumerate - say so explicitly | L747 / #280 (13 baselines in the suite, 10 shrink-only, only 3 requiring a reason - and the ratchet I had just written was in the ten, not the three) |
| Assert over the TEXT of a source file | ANCHOR ON SOMETHING THAT CANNOT APPEAR IN PROSE - the executable line with its syntax, an AST node, or a scoped slice. A source file holds the code AND the prose about it in one string, and a good comment names the exact token your assertion greps, so the better the documentation the more reliably it poisons the match | L748 / B1738 (`.index()` found the comment I had just written to explain the fix; a must-quiet arm matched the report's own preamble explaining the phrase) |
| Quote a figure from a MOVING artifact - heartbeat, progress counter, queue depth | CARRY THE READING'S AGE, not just its file. #201 makes a figure name WHERE it came from and says nothing about WHEN; on a live artifact those diverge constantly, and a source name reads as freshness because naming a live file implies you just looked. If the sentence says "right now" or "currently", the measurement belongs in THIS turn's tool output | L749 / #201 + L695 (reported sim-day 144 and 1.81h as "right now" with the file named; re-measuring gave sim-day 177 and 2.519h - 33 sim-days stale) |
| Get the SAME rejection from an automated gate twice | OPEN THE GATE'S SOURCE AND FIND THE PREDICATE. Its message is an honest English summary of the policy; the code implements a proxy for it, and only the code runs. An unchanged message between attempts is evidence about the CHECK, not about your phrasing - the input never reached the thing being tested | L750 / L638 (wrote JUDGMENT-ONLY four times; the member evaluates _artifact_touched and wanted a FILE TOUCH - eight closes to satisfy what one grep would have shown) |
| Sweep a parameter through a flag | RUN TWO VALUES THAT MUST DIFFER AND ASSERT THE OUTPUT DIFFERS, before grading anything. A flag has three links - PARSED, PASSED, READ - and argparse proves only the first. A flag that is accepted, stamped into the artifact, and never applied is WORSE than one that errors: it yields well-formed rows that self-document a setting they did not use | L751 / #230a (--min-committed-growth 14 returned fires 373, identical to 3, with the admit dict stamping 14 - four free levels would have shipped four identical rows) |
| Invent, run, or approve an analysis check for any config's landed cube - or hear the design say "per config" / "every landing" / "part of the band" | WIRE IT INTO THE BATTERY THE SAME TURN (a fail-closed family-runner leg) or file a ticket naming the battery gap. "Executed once" + "the tool exists" + "it is documented" is the three-part disguise of an unwired check - none of the three runs it on the NEXT landing. Companion: a re-scorer must REPRODUCE the landed baseline before any level it reports is believed, and every looser-than-production level of a PERSISTED parameter is engine-only by construction - scheduled with a PROVEN knob or struck with a reason (plan SS0.7) | L752 / #290 (S6-B2501's free levels: correct tool B2504, registered family B2520, pre-registered manifest B2527 - three batches, zero wiring, four configs landed ungraded; owner-caught) |
| Close a turn whose only purpose is satisfying the gate that blocked the last one | TREAT THE REMEDIATION PASS AS A WORKING TURN - every rule it cites binds it too. The three shapes measured in one pass: the gate run must POSTDATE the content it vouches for (re-run on the staged diff, not the diff you had when you started); a pipeline's exit status is its LAST command's, so a gate shaped `pytest \| tail` reports tail's 0 over pytest's failure (write to file, report `$?`); and a skill edit ships its fragment pin in the same call | L753 / Phase 3 + #67 + B2130 (B2570 committed against a pre-edit gate run and test_b1486 caught it; the task said exit 0 while the artifact held `1 failed`) |
| Write, or reuse, a grader / spot-check / verifier / table renderer that names ONE strategy (a STRAT constant, a hardcoded key set, a family-specific env knob) - or copy one for the next family | A SCRIPT WITH A STRAT CONSTANT IS THE FIRST INSTANCE OF A PORTABILITY CLASS, NOT A TOOL. Extract the contract at the SECOND family, never the third: one adapter (params_from_env / grade / free_levels / spot_check / engine_anchors) per SPECS entry, and REGISTRATION IS CHECKED AT LAUNCH - a battery that fails closed at landing has already spent the engine's 2-4 h. Until the adapter and launch check exist, run the runbook SS11.1 ON-RAMP probes R1-R9 by hand and paste them; every mechanism a runbook step cites is named with its file or marked PROPOSED-NOT-BUILT | L754 / #291 (B2573 audit: FAMILIES 2 of 219, 7 of 9 scripts one-strategy, 0 of 4 launch scripts check registration; `output_audit/b2573_optimisation_workflow_portability_audit.md`, tickets S6-B2573a-i) |
| Launch a pyramid, then edit anything under `scripts/`, `backtest/`, root `*.md` or `.claude/` while it runs - or run a patcher without its dry-run switch - or commit on a gate run older than your last edit | A PYRAMID IS A MEASUREMENT OF ONE TREE. Run it through `scripts/pyramid_gate.py`, which fingerprints that tree before pytest and again after: `tree=CHANGED` is VOID (exit 4) whatever pytest reported, and the gate run is the LAST action before the commit | L755 / #292 (three instances in one session: a false RED from a mid-run engine edit; a would-have-been false GREEN over three scripts an un-dry patcher overwrote at ~40%; a verdict that pre-dated its own doc edits) |
| Write a file your patcher did not create - a canonical doc, a queue, a roster - with `Path.write_text` | A PATCHER WRITES BYTES. On Windows `write_text` re-encodes every line ending in the file, so a 57-line append can stage as 18,868 changed lines and a lone CR becomes a line break. Worse, the three gates that ask "what did this change add" then read the whole file as new - and one of them demanded fabricated dispositions for 170 untouched entries. Write bytes (or `newline=""`), and compare `--numstat` against `--ignore-cr-at-eol` before committing | L756 / #293 (B2581/B2582: preflight C13 blocks it; MEASURED at-risk canonical docs = LEARNINGS.md 657 bare LF, CHECKLIST.md 53, CANONICAL_FACTS.md 843 - every other one is pure CRLF and immune) |
| Count members where any of them carries a caveat - not comparable, provisional, degraded, superseded, unreviewed | STATE THE COUNT BOTH WAYS. #182 disciplines the DENOMINATOR and is silent when a member is dropped from the NUMERATOR for a reason the count's definition never mentions - so a figure can be wrong while its scope is right, which is the half nobody checks. Write "3 of 12, of which one is not comparable, so 2 of 11 comparable", never the caveated figure alone. And when the lesson lands in LEARNINGS, its skill row ships in the SAME call - four consecutive entries drew this same block on the following close | L761 / #182 (MEASURED 2026-09-04: reported 2 of 12 ICG configs with a positive is_ci_lo; the sign over all twelve says 3 - span100 had been dropped for a grade-validity fact unrelated to its sign) |
| Take a gate's silence as evidence that a rule is covered - "the pin gate would have caught it", "nothing fired, so we are clean" | A CO-OCCURRENCE TRIGGER PROTECTS THE TRANSITION, NEVER THE STATE. A gate shaped "changed X without Y" reports on TURNS; coverage is a question about the POPULATION, and nothing that predates the convention will ever trip it. Measure the population once, record the figure with its method, and treat the gate as holding the line from that day forward | L760 / #231 (MEASURED 2026-09-04: 6 of 30 tripwire rows with an L-lineage cell carry a fragment test_b2123 pins - both numbers lower bounds; scan_skill_not_updated, scan_prose_only_rule and scan_queue_not_updated share the shape) |
| Apply a shell-safety rule to ONE kind of call - the patcher, the commit, the launcher - or write a SHORT read-only command that quotes a file's own text back at you | SWEEP THE TURN'S OTHER CALLS IN THE SAME BREATH. The discipline attaches to the artifact you are thinking about, not to the mechanism that bites: bash substitutes inside ANY double-quoted argument, so patcher, probe, one-liner and grep are one act to it. And note that every benign instance TRAINS the form - a substitution that holds a path a dozen times is how the shape becomes automatic | L759 / #245 (the turn that shipped the byte-writing patcher rule ran a dozen probes as `python -c` with an expanded variable; the repo sweep then found 2 LIVE instances, 1 escaped-in-a-heredoc and 1 captured artifact). **L772 (B2640) adds the LOCATION: verifying a MARKDOWN edit is where the unsafe form appears.** MEASURED over one doc-editing turn's 18 shell calls - 4 heredoc, 9 double-quoted `-c`, and the single backtick-bearing one was the command CHECKING that the edits landed, because a verification's payload is copied from the artifact and this repo's docs are full of inline-code backticks. Short, read-only and quoting something that already exists are the three properties that make a call feel safe and are exactly what makes it dangerous. Any payload COPIED FROM A FILE goes through a heredoc, however short |
| Reach for another campaign's cube, spec or measurement as evidence or as a template while working the current strategy | PROVE IT ON THE CURRENT STRATEGY'S OWN ARTIFACTS FIRST; another campaign earns at most one clause and never a ticket, and an admitted or finalised strategy is never reopened without the owner's override. A template is copied for its SHAPE, never its values | L868 / #295 (MEASURED 2026-09-24: cited smc and icg cubes for an engine defect three_white_soldiers' own Step-1 cubes proved, ticketed a re-audit of two admitted strategies, and the Step-2 spec template was still smc's b2399 spec) |
| Answer a question about the strategy in front of you with a fact about the PROGRAMME, the backlog or the roster - a spec census, a coverage percentage, an "N of the population" figure | WORK INSIDE THE CURRENT STRATEGY'S CONTEXT. A coverage denominator taken from a stage that ALREADY RAN passes #182's denominator rule while inverting the meaning, and the zoom-out arrives wearing rigour. Name the stage that produced the population and check whether an earlier stage already covered it; if a cross-strategy fact is load-bearing, state it in one clause and stop | L758 / #295 (owner 2026-09-03: R5 had already run the population - 196 distinct strategies in the merged cube - and 2-3 qualified, which is WHY optimisation is per-strategy; I argued against the running programme from "2 of 207") |
| Ship a gate, a ratchet or a refusal built from the one or two instances that bit you | ENUMERATE THE POPULATION ITS PRECONDITION SELECTS, then re-derive the predicate against that population - if any member would PASS, the predicate is fitted to the incident. The instances teach the SYMPTOM; only the population teaches the DISCRIMINATOR, and the tell is a condition resting on a property both instances happened to share. AND TWO WAYS THE CHECK ITSELF FAILS: THE SWEEP INHERITS ITS EXAMPLES - name a member you did NOT derive the pattern from and assert the pattern finds it, or report the count as a LOWER BOUND; and A MECHANISM IS NOT PINNED BECAUSE ITS WRAPPER IS - assert the VALUE the decision reads, not the message it prints | L757 / #294 (B2582: C13 keyed on ending DIVERSITY because both damaged files were mixed; the sweep found CANONICAL_FACTS.md uniform bare LF, where a whole-file flip leaves the kind count 1 -> 1 and the gate passes its own worst case. B2588: the sweep for that class returned 4 of a 6-member population, patterned on the single-line examples in front of it. B2589: `_bare_endings`, the value C13 blocks on, had 0 direct assertions - pinned in test_b2581_a_line_ending_flip_is_blocked_and_invisible_to_the_diff_gates, proved by mutation in both directions) |
...
| Treat "start/begin/kick off X" as approval for X's every stage | an instruction covers the stages up to the next irreversible or owner-owned gate, never past it - map each stage to the approval that covers it BEFORE executing (pead precedent: bands reviewed, Step 2 fired on its own word, admissions each ruled) | L779 (B2669: 'start the queued campaign' ran band design + the irreversible holdout read + admission unasked) |
| Run one member of a population the ask names, then close the ask | scope the EXECUTION to the ask's stated breadth, or declare the slice and keep the ticket open - a control refuting one pairing refutes that pairing only | L778 (B2662: 1 strategy x 1 axis closed an ask spanning 20 strategies; the full 13,104-trial grid found 299 qualifiers across 17 of 20) |
| Execute a campaign spec that was queued in an earlier batch | RE-READ the runbook's template sections AS AMENDED before running it - a rectification landed after queueing binds the execution (two legs per campaign: DEPTH own-knob levels AND BREADTH companion axes, runbook 11.2b2), because a rectification recorded only in the family section it was learned on repeats on the next family | L780 (B2671: top_decile ran depth-only - 3 own-knob axes, zero companion - after the institutional depth+breadth rectification; owner-caught) |
| Run a retroactive sweep after a correction names a class | RESTATE the class's POPULATION in your own words and check the sweep enumerates THAT population - a sweep over an adjacent population reads as compliance and leaves the named class open (campaigns were swept when the owner's class was the workbook's RULES; the rules-sweep then found 3 of 4 generals missing) | L781 (B2672, owner re-pointed the population) |
| Chain a commit (or any dependent step) behind a read of a LIVE background task's output | the FILE is not the RESULT - gate on the verdict token itself (pytest_exit=0 tree=SAME) in the same guard, or wait for the completion notification; tail of a partial file exits 0 and gates nothing, and the gate's --out json exists EMPTY while it runs | L782 (B2682a: a pushed commit message claimed GREEN mid-run, staging an empty pyramid json; self-caught, retracted, corrected by follow-up commit) |
| Summarize a campaign artifact through a view you authored for the occasion | render the STANDARD form FIRST (one column per axis, bands in the header) and summarize from THAT - a home-made view optimizes away exactly the disclosures the standard forces into sight, and a skip/refusal row in the artifact appears in the summary or the summary is wrong | L783 (B2696: a collapsed Table D dialect hid the artifact's own coverage-skip; the standard render exposed it in one pass and a 3-of-4 result had been reported as 4-of-4) |
| Render or update a campaign's Table D (any offline results view) | Table D is ONE unified table whose columns are the campaign's FULL Table A inventory - render via table_d_render.py; untested axes shown, never omitted (resim-only knobs at production value, marked UNTESTED-OFFLINE), and a late-landing axis RE-RENDERS the one table instead of spawning an addendum | L784 (B2699: third form correction on one table - the columns had encoded the swept slice, and a process constraint leaked into the deliverable's shape) |
| Close a campaign or publish its Step-2 verdict while any Table A axis sits UNTESTED | the venue never narrows the mandate: depth = ALL producer bands (resim included) is Priority 1 by owner ruling, breadth Priority 2; an unreachable axis is resim-scheduled or owner-waived in words at band review, and the disposition says 'offline leg negative; depth leg NOT RUN' - never 'strategy negative' | L785 (B2702: hub-1 shipped CLOSED-NEGATIVE over 8 of 11 axes while the producer bands were never run; the roster's own smc qualifier is a resim result) |
| Write JUDGMENT-ONLY / PROSE-ONLY on any new rule | MECHANIZE FIRST: name the mechanism designs attempted and why each fails; if any SLICE is mechanizable (a scan, a repo-text pin, a fail-closed CLI), build that slice in the SAME batch - the narrowest enforceable slice is the minimum | L786 (B2705: #299 was labelled JUDGMENT-ONLY while its declare-failure slice was mechanized in ~150 lines the same day, on the owner's 'Wire this') |
| Write a campaign spec's window / universe / scope from your own reasoning about what the step needs | COPY THE RULED ROW: open the runbook phase table and take the step's window and universe verbatim (Step 1 = 200 tickers x 1y ending AT the IS/HO boundary; Step 2 = 4y x 544). A deviation quotes the owner's words in the spec; a Step-1 window past the boundary is refused outright - it ranks on holdout data | L787 (B2711: a pilot ran STEP 2's 4-year window in Step 1's slot - 4x compute and a would-be holdout leak for the whole smc family; killed at sim-day 45) |
| Remediate a miss by adding a LEARNINGS entry + checklist item + tripwire row | ASK FIRST WHETHER A DEGREE OF FREEDOM CAN BE DELETED INSTEAD - a ruled value that cannot be typed needs no rule about typing it right. With 787 lessons in context there is a citable rule for almost any deviation, so each addition also arms the next one; if you add rather than delete, say so and say why deletion was impossible | L788 (B2713: council unanimous - the owner's one question had a 7-of-7 detection rate this session, the authored gates 0-of-7, because they validate the spec I wrote; the fix was making the window unwritable) |
| Wire a new gate into an existing pipeline | ENUMERATE THE CALLERS AND ARTIFACT KINDS FIRST - authored spec, generated artifact, test fixture, sibling gate - and say which layer judges each; then make every ESCAPE clause match EXACTLY, because substring identity on an escape is a silent exemption waiting for a short name (an escape fails OPEN, which nobody investigates) | L789 (B2716/B2717: the resolver gate failed five times on its own plumbing before one engine day ran - it judged its own injection, and a wave named 't' could grandfather itself out of the rule) |
| Satisfy an owner ruling about a FORMAT, or wire a renderer into an automatic path | GREP FOR EVERY RENDERER THAT EMITS THAT FORMAT AND SAY WHICH THE RULING BINDS - a ruling names a DOCUMENT, not a function; then confirm the thing you are wiring is the RULED version, because wiring converts a local defect into an enforced one. And pin a locked format on its COLUMNS, never its section headers | L790 (B2725: the unified-Table-D ruling was applied to one of two sibling renderers, and the unruled sibling was the one wired into every landing; its pin asserted the headers existed and went green on the rejected shape) |
| Wire a gate that reads a LEDGER, a registry, a roster or any other recorded fact | ASK WHICH TREE THAT FACT BELONGS TO - repo facts (admissions, roster, phase table, parameter registry) come from the CODE tree; run facts (spec, subset file, manifest, cube) come from the RUN'S root, which a test or a staged launch may relocate. A gate reading a repo fact from the run's root fails CLOSED ON THE ENVIRONMENT and refuses everything, which is indistinguishable from the gate working | L792 / #305 (B2739: the admitted-strategy gate read the admissions JSON from the spec's root, so any launch staged outside the repo was refused with 'admissions unreadable' whatever it graded - and the CODE_ROOT precedent sat three lines away in the same file) |
| Ship a long, winding answer-turn | lead with the answer; tables over prose; plain words - ~300 words before the end blocks (owner directive 2026-09-10: 'you have been providing long responses and are not structured well') | L777 / BREVITY AND STRUCTURE section |
| Build or read a per-item status view that answers 'what should we work on next' | CHECK ITS VOCABULARY HAS TERMINAL-NEGATIVE STATES derived from the committed rejection records, mutually exclusive with the work lanes - a vocabulary naming only planned states resurrects finished-and-rejected work on every regeneration, and reopening must be an owner-worded FLAG, never automatic | L804 / #201 (MEASURED 2026-09-16: 16 discarded strategies sat in work lanes, 7 of 48 'tightenable' were closed-FAIL siblings) |
| Regenerate a committed artifact whose numbers moved, and quote or circulate its figures | STAMP THE BUILD IDENTITY IN THE ARTIFACT ITSELF (generating batch + source commit + timestamp) - naming the file names every version at once, and a stale copy keeps speaking with the file's authority | L803 / #201 (MEASURED 2026-09-16: the owner quoted the first build's stream counts after two audited regenerations; nothing on any copy said which build it was) |
| Name a strategy, family, ticket or item as work to START - a ranking, a 'do these first', a recommended next batch | READ ITS COMPLETION RECORD FIRST and say what you read. A recommendation to do work ASSERTS THE WORK IS OUTSTANDING, which is a claim about the LEDGER and not about the item - and every property that makes an item look attractive is visible in the code while its completion status is not. Where no single view answers 'is this done', say so rather than screening on whatever happens to be queryable | L802 / #26 (MEASURED 2026-09-13: ranked six strategies as do-these-first and 2 were ALREADY ADMITTED; 9 of 14 Phase-1B admissions are the very family I recommended starting. Answering 'has X been optimised' needs a hand-join of 4 sources - the roster, the 1B roster, the admissions json and the ticket-keyed queue) |
| Land a change that makes a previously-false statement TRUE - registering a strategy, enabling a flag, adding a member to any set | GREP THE TEST TREE FOR THE SUBJECT'S NAME FIRST and rebuild any fixture that named it BECAUSE of the property you are changing. Where a fixture needs 'a member with property P', DERIVE one at runtime with a positive control rather than naming today's example - renaming buys one change of runway. And tell a stale fixture from a REGISTRY FREEZE: the freeze fired correctly and is updated in the batch that earns the new member, never widened | L801 / #196 (MEASURED 2026-09-13: registering hub-2 turned the pyramid RED on 2 tests, neither about the changed code; one grep would have listed both before a 12-minute gate cycle found them) |
| Execute a ticket whose scope is stated by ANALOGY - 'the same as X applied to Y', 'a straightforward port', 'the template', 'three more times' | OPEN THE NEW SUBJECT AND VERIFY THE ANALOGY'S LOAD-BEARING PROPERTIES FIRST, then correct the ticket in the same turn. An analogy is a PREDICTION about a system nobody opened, it carries no figure for #201 to demand a source for, and it inherits every accident of the one subject you HAVE opened | L800 / #222 (MEASURED 2026-09-13: 'the template applied three more times' - 2 of 3 assumptions false; a knob with no env actuator and a gate key persisted on 0 of 1340 fires, both settled by two greps and a coverage probe in under a minute) |
| Write something that SUMMARISES or PARTITIONS a population the codebase already holds - grid rows, cube cells, ledger tickets, test results | GREP FOR THE POPULATION'S EXISTING READERS FIRST AND COPY THEIR BUCKET LIST. The extra buckets are paid-for knowledge: they encode failures you have not met. A NEW function inherits nothing (L608), and nothing prompts you to look because no file containing the convention is open in front of you | L799 / #262 (MEASURED 2026-09-13: I gave a new report a two-way split; producer_variant_table.py:2169 already partitioned the same rows FOUR ways after 31-66 rows per config vanished from a funnel. Two live runs then lost rows. Sweep: 3 bucketing sites, 2 still not reconciling) |
| Compare a count in one artifact against a count in another - two files about the same cube, run, or strategy | NAME THE INSTRUMENT BEHIND EACH FIGURE FIRST. Artifacts stamp their SUBJECT and often not their GENERATOR, so two files about one cube share a vocabulary and look interchangeable while scoring different things. If the artifact does not disclose its generator, fix that before comparing | L798 / #309 (MEASURED 2026-09-13: a grid_auto reporting 0 graded rows was compared against a salvage grader that graded 528 cells on the SAME cube - both correct, different instruments; the wrong figure argued against running work the owner had already approved) |
| Revise a figure you have already given the owner once - a count, a rate, a coverage number - and find yourself about to revise it a SECOND time | STOP REVISING AND OPEN THE FIELD THAT CLASSIFIES ITS MEMBERS. A count that keeps needing reinterpretation is standing in for a classification nobody ran, and each revision FEELS like diligence because it is evidence-based, which is how the loop survives three rounds without anyone opening the field | L797 / #237 (MEASURED 2026-09-13: three readings of one 200-row grid across three turns; the verdict field settled it in one call - 169 were NO_EXIT_SELECTABLE non-observations, so the null bar was 1.90 SE not 2.61. Sweep: 91 of 91 grid artifacts carry a verdict field and a bare row count misleads in every one) |
| Compute a cost, a ratio or a coverage figure by hand from a record that encodes the same property twice - a coarse summary flag AND a precise per-level form | CALL THE FUNCTION THAT ENCODES THE PRECEDENCE, and have it return the BASIS with the number. The coarse half stays readable after a split and is the more convenient read, so the error is systematic - and it has a DIRECTION, which is what makes it dangerous: it always inflates cost, so it always argues for not running something, and a bias toward inaction fails silently | L796 / #308 (MEASURED 2026-09-13: a spec read 1:1 over 4,800 engine runs off the boolean and 8:1 over 600 off its per-level bands - an 8x error published in an owner-facing table, caught by an advisor, not by me) |
| Find a gate whose remedy you cannot perform - it cites text from an earlier turn, or keeps reporting something already fixed | READ THE PREDICATE ON THE LIVE ARTIFACT, not the message. A window that decides what happened THIS turn rests on *is this entry a real instruction*, and a predicate keyed on a TYPE TAG is a guess about transport - a channel it does not know about makes real instructions invisible and pins the window open, so every explanation of the block adds a new offender | L795 / #307 (MEASURED 2026-09-12: mid-turn messages arrive as type='attachment'; both window helpers skipped them, the window stood open at 1,132 entries and ~40 closes were blocked. One probe on the live transcript ended what a dozen rephrasings could not) |
| Cite a gate as protection when it keys on a FILE TYPE - a pyramid stamp on `*.py`, a linter on one extension - or read a compound task's exit status as proof it did both halves | MEASURE THE GATE'S REACH: what fraction of recent commits stage a file it keys on? A gate is structurally blind to a commit class that never contains its type, and the blindness is silent. Run the sweep BEFORE judging the instance - it can invert the verdict, because a scope that looks like an oversight may be load-bearing for an owner-ruled automation. And a task's NAME is not its behaviour: read the output for the VERDICT TOKEN, since a compound command's exit status is its LAST command's | L793 / #306 (MEASURED 2026-09-12: 15 of 40 commits staged zero `.py`, so C6 could not fire; the largest group is the B2520 landing supervisor. Same turn, 'Gate and commit' committed and never gated at a reported exit 0 - the gate had errored EXIT=2 on a missing required flag) |
...
| Convert line endings in a patch, or write prose that DESCRIBES a carriage return | NORMALISE EXACTLY ONCE, AT THE FINAL WRITE, OVER A PAYLOAD HELD IN PLAIN NEWLINES - never on a fragment, never on a concatenation of already-converted fragments, and never choose the ending by asking whether one is already PRESENT. Converting twice yields a doubled carriage return that git's text attribute collapses into a stored bare CR, which is invisible in the diff, in the rendered markdown and in the file - only a test reading the INDEX sees it. And keep escape sequences out of heredocs: describing a byte is exactly the case where the prose is backslash-bearing, so the topic re-triggers the defect | L858 / #314 (MEASURED 2026-09-23: one doubled sequence in a 597,365-byte file reached a commit and turned test_b2921 red; recording the lesson then reintroduced it through a heredoc, taking LEARNINGS.md from 0 two-byte endings to 5) |
| Act on a lesson that NAMES a script, a flag or a test - a blocker audit, a staleness check, a verifier | RUN THAT MECHANISM AND REPORT WHAT IT RETURNED. Re-deriving its answer by hand is a CHECK ON the tool, never a substitute: the manual version scopes itself to whatever you thought of, and that population is invisible. Effort is not coverage | L859 / #235 (MEASURED 2026-09-23: hand-verifying four tickets' blockers felt more rigorous than audit_ticket_staleness.py with its blockers flag, and covered 4 rows where the tool covers 22 and classifies each) |
| Receive an instruction that CHANGES STATE - stop a run, pause a campaign, rule a decision, land a dependency | RE-READ THE HELD TICKETS AND ASK WHAT EACH WAITS ON NOW. A blocker verified TRUE keeps the authority of having been checked, and that authority outlives the event that voids it - so the re-ask never fires, least of all when the voiding event is an owner instruction you are busy executing. State a blocker as its CONDITION, never as the EVENT you expect to produce it | L860 / #235 (MEASURED 2026-09-23: an owner pause dissolved the chain-contamination hold on three tickets and they were still reported blocked; the hold needed NO CONFIG IN FLIGHT, which I had written down as CHAIN DONE across all 36) |
| Write or read any view that says what a campaign TESTED - an inventory, a coverage table, a status column | DERIVE THE LABEL FROM THE ROWS, NEVER FROM A PROPERTY THAT CORRELATES WITH THEM. Ask *did any row vary this?*, not *could a row have varied this?* - the two coincide on the design you built it for and invert on the next one. And when the tested subset is EMPTY, say so: never fall back to printing the space that was AVAILABLE, which reads as a measurement (L580). #182 governs rendered views, not only sentences | L861 / #182 (MEASURED 2026-09-23: Table D keyed TESTED on whether a parameter had a free band, so three axes tested by engine re-simulation read UNTESTED-OFFLINE while the body showed their thresholds, and an axis nothing touched read TESTED at its four band levels) |
| Repeat a caveat about what a mechanism does - drops, loses, cannot, never | OPEN THE MECHANISM FIRST; a caveat is a claim about code and has an expiry the artifact never shows. When it is stale, fix the GENERATOR that writes it, not only the copy in front of you | L867 / #256 (MEASURED 2026-09-24: told the owner three times that a resume drops 67 open trades; backtest.py restores them, and run_wave.py wrote the stale caveat into 83 of 92 manifests) |
| Name a gate as a risk's mitigation, or quote a recovery path | OPEN THE GATE'S FUNCTION AND WRITE WHAT IT COMPARES, NOT WHAT IT IS CALLED; then list every gate the recovery path crosses and check each against what the session will do meanwhile (commit hourly) | L866 / #230 (MEASURED 2026-09-24: 'the launcher refuses on drift' was drift_check comparing HEAD's sha, so four queue-only commits refused leg 2 of a 10-leg Step-2 wave at sim-day 329) |
| Replace or restructure a governing document - the runbook, CHECKLIST, a skill, CLAUDE.md | COMPARE IT WITH THE ARCHIVED SOURCE BY MACHINE BEFORE IT REPLACES ANYTHING: `scripts/doc_rewrite_coverage.py`, every missing citation and every unmatched owner-reservation sentence restored or accepted with a reason. Having read the source minutes earlier is not the check | L869 / #321 (MEASURED 2026-09-24: a 634-line part drafted minutes after a full read switched a HALT condition the source had left to the owner and dropped pinned passages; the CLI fails that draft and passes the rebuild) |
| State WHEN, or FROM WHAT, you did something across a compaction - in an L-entry, a retraction, an RCA or a report | RUN `scripts/transcript_timeline.py` AND CITE THE ENTRY OR UTC TIME IT PRINTS. The compacted view replays a pre-compaction tool call as if it were the first action, and a summary can list a written file as still to write | L870 / #322 (MEASURED 2026-09-24: an L-entry blamed a compaction for a draft written five minutes before it; an Edit issued 98 s before a compaction was reported as the first action after it) |
| Continue substantive work after a context compaction, or write a SKILLS INVOKED line after one | RE-INVOKE Skill(execution-discipline) BEFORE THE NEXT SUBSTANTIVE STEP; FULLY LOADED means a Skill call made after the last compaction. The compacted copy is truncated, and the B1744 hook's re-injection is persisted to a file with only a 2 KB preview once the skill outgrows the harness's inline limit - the hook still runs and reports success, and what reached context is the preview | L871 / #229 (MEASURED 2026-09-24: hook output 367.5 KB, a 2 KB preview in context; I wrote FULLY LOADED on it and scan_discipline_not_loaded blocked the close) |
| Build or trust a detector that answers *is X running / present / in flight?* | READ THE ARTIFACT X ITSELF WRITES, NOT THE LOG OF ONE WAY OF STARTING IT. A detector keyed on a launcher's log sees that launcher only, and a blind disclosure prints the all-clear. And sequence heavy verification BEFORE a long run starts - once it runs there may be no window at all | L865 / #319 (MEASURED 2026-09-23: pyramid_gate recorded chain_inflight=none at both ends while a direct run_wave Step-2 wave ran; two pyramids beside it exhausted commit and the machine was restarted by hand at sim-day 19 of 1,003) |
| Report that an artifact does NOT carry a field, or that a field means something other than its name | OPEN THE WRITER. A persisted field's meaning is a PRODUCER fact, and explaining it from the SHAPE of its values is how a correct rule lands on the wrong instance. Watch for uniformity read as a defect: a value identical across a dimension is often the data's CONSTRUCTION, not a reader bug. The discriminator is usually one division - row count over group count against the field | L864 / #166 (MEASURED 2026-09-23: rendered 432 cells' trade counts as absent because fires was identical across all 24 exits; grade_candle_config.py counts cube rows under that name, the cube replays every entry through every exit, and rows/24 == fires exactly on 18 of 18 configs - the wrong conclusion was in a PIN, defended green) |
| Report that a run, chain, campaign or job has LANDED, finished, been paused or stopped | RETIRE ITS MONITOR IN THE SAME TURN. #185 gates the ARM and nothing gated the other end, so the disarm half lived on remembering. A monitor that outlives its subject does not fail - it SUCCEEDS on a prompt frozen at the moment it was armed, so every firing reads as diligence and repetition on a timer reads as corroboration. CronDelete it, or say in the response why it must stay armed | L862 / #320 (MEASURED 2026-09-23: cron 524950bc fired three more hourly cycles after its config landed COMPLETE, each re-asserting a terminal sim_day_index of 249 as current; it ended when the owner said discard it, not because anything noticed) |
| Pin a LOCKED format - a ruled table, report or artifact shape - or render one | PIN THE ORDERING, NOT ONLY THE COLUMNS. A column assertion is satisfied by a table whose rows are in ANY order, so the half a reader acts on goes unpinned while the half visible in a header is covered. Assert the ordering PROPERTY (a higher-Sharpe row carrying the lower bound ranks BELOW its rival), never the key name, so a re-expression of the right rule still passes. And when a format ruling exists, check EVERY sibling renderer inherited it | L863 / #303 (MEASURED 2026-09-23: table_d_render.py ranked on -is_sharpe, the key runbook 6.4b explicitly REJECTS, while its sibling producer_variant_table.py sorted on -ci at two sites - 15 of 25 top rows carried a negative lower bound and 2 of 25 positions agreed with the spec) |
...
| Write the REMEDIATION half of a gate message, an error string, or a refusal - the sentence telling the reader what to DO | A REMEDIATION INSTRUCTION IS A CLAIM ABOUT WHAT HAPPENS IF SOMEONE FOLLOWS IT - EXECUTE THE FIX PATH, NOT JUST THE DIAGNOSIS. The diagnosis half gets checked because it fires on real input and someone sees it. The fix half fires on NOBODY until a reader follows it, so it ships unverified by construction, and a confident remedy is obeyed without question because the gate that printed it was right about the problem. MEASURED 2026-09-20: a refusal I wrote to STOP a false message told operators to complete a tools block where it stood; the function that decides family readiness reads a different registry, so following the instruction changes nothing - the gate was right about the defect and wrong about the cure, one commit after being written | L820 / L688 (which says find the BRANCH that emits a string; this says run the PATH it recommends) |
| Ship a fix because a gate fired on ONE file | A DETECTOR'S PATTERN IS A CLAIM ABOUT EVERY SPELLING OF THE THING IT DETECTS - AND IT IS BLINDEST ON THE IDIOM ITS OWN NEIGHBOURS USE. MEASURED 2026-09-20: B1719b enumerated three RNG spellings and missed the module-level random.seed/random.sample idiom, so 2 of the 3 real sampling scripts were invisible and its hit rate was 1 of 3 while it read clean. It fired on the third only because that file used random.Random( - a style choice, not a semantic one; copying the siblings' idiom, which is how a third sibling normally gets written, would have kept it silent. New code is written by copying the nearest example, so a pattern that misses the neighbours has its gap where the next instance lands. Run ONE query - narrow-pattern hits versus wide-pattern hits over the same tree - before shipping the one-line fix | L821 / L688 / B2578 (a literal needle too narrow across LINES; this is too narrow across IDIOMS) |
| Model a new component on an existing SIBLING - a second grader, a second adapter, a second renderer | A SIBLING ENCODES WHAT WAS TRUE WHEN IT WAS WRITTEN - READ THE CONSUMER'S CONTRACT BEFORE INHERITING ITS ASSUMPTIONS. A sibling is ONE implementation of a contract, not the contract, and its assumptions were correct on the day it was written. The consumer that invokes both is where the CURRENT contract lives, and it moves without either file recording that it moved. MEASURED 2026-09-20: I modelled a candle grader on the institutional one and inherited its rule that a cube carries exactly one strategy - true when that sibling was written, and superseded by B2721, which established that a cube legitimately carries a graded strategy PLUS declared riders and added graded_and_riders to read it. A long/short pair run in one engine pass puts BOTH legs in one cube, so my draft would have refused the campaign's own shape on config 1 of 54. A council advisor asked the question; nothing in my process did. Distinct from L613, where the copy carries a DEFECT - here the sibling was correct and the world changed underneath it | L822 / L613 / L799 |
| Size a BACKLOG with a detector you just wrote - 'N of M declarations lack X', 'N of M rows are missing Y' | A DETECTOR TESTS THE PROPERTY YOU ENCODED, NOT THE ONE THE RULE REQUIRES - READ THE RULE'S OWN COMPLIANT FORM BEFORE SIZING A BACKLOG. MEASURED 2026-09-20: I sized the L698 backlog at 109 of 213 JUDGMENT-ONLY declarations naming no search, then hand-read 10 of the flagged set and found 10 of 10 FALSE POSITIVES - five state a reason, which is what L548 actually requires; one says 'this is NOT judgment-only'; two QUOTE the rule while describing a violation of it; one names a scan my pattern could not see because it lacked an underscore. The detector tested for a named SEARCH while the rule requires a REASON, so the count measured my encoding rather than the corpus. Before quoting a backlog figure, write down what a COMPLIANT instance looks like and confirm the matcher would recognise one | L823 / L644 / L713 (a detector's reading unit must span the rule's prescribed form) |
| Read a field out of a persisted artifact and find it ABSENT - 0 rows carry it, the key is missing, the dict is empty | A PROBE THAT REPORTS ABSENCE MUST PROVE IT CAN SEE PRESENCE - RUN IT AGAINST A KNOWN-PRESENT CASE BEFORE BELIEVING THE ZERO. MEASURED 2026-09-20: a probe reported 0 persisted signal keys across smc trades and I was one sentence from concluding the family's knobs are un-gradable offline. The probe used ast.literal_eval with NO json fallback, so it failed on EVERY row of a column whose values are JSON-style; re-measured with the fallback, all 948 rows carry a non-empty dict of 833 keys. A zero from a parser that cannot read the format is indistinguishable from a zero that means absence, and the absence conclusion is the one that ends the investigation. #166 says prove a PATTERN can match; this says prove the READER can parse | L824 / L724 (a uniform value from a new extractor is a parser hypothesis) / #166 |
| Migrate a record to a new encoding for SOME of its members - the env knobs, the new-style rows, the fields the ticket named | MIGRATING HALF A RECORD FLIPS ITS READER TO THE NEW ENCODING AND MAKES THE UNMIGRATED HALF INVISIBLE - MIGRATE THE WHOLE ENTRY. A reader that picks its encoding by asking whether ANY member carries the new field switches the WHOLE record the moment one does, and the members you did not migrate then contribute nothing. MEASURED 2026-09-20: migrating 15 env knobs to the free/resim split flipped leverage() to per-level for an entry whose 4 OFFLINE params were still on the coarse boolean, collapsing its free_combos from 32 to 1 - a 32x leverage claim destroyed by a change that looked purely additive, in the direction L796 names as dangerous because an inflated cost always argues for not running something. Caught ONLY because a pin had frozen the number. Before migrating part of a record, FIND THE READER and ask what it does with a half-migrated one | L825 / L726 / B2767 (both encodings coexist and the coarse one stays readable) |
| Report a gate's COVERAGE - 'N of N covered', 'zero blind', 'every entry is checked' | A GATE THAT DEFINES ITS OWN POPULATION WILL ALWAYS REPORT FULL COVERAGE - TAKE THE DENOMINATOR FROM A SOURCE THE GATE DOES NOT OWN. The coverage number and the gate's scope come from the SAME predicate, so a gate blind to a case simply does not count that case as its business, and reports 100 pct. MEASURED 2026-09-20: I reported the R1 adapter refusal as covering 8 of 8 engine-bound entries with 0 blind - computed with engine_requiring_params, which skips any parameter whose env field is absent. Taking the denominator from the BAND REGISTRY instead, 13 entries carrying 30 banded parameters marked engine_implemented=true have no env field and are invisible to it; 9 of the 13 are not battery families at all. This is L817 (a discriminator keyed on an optional field is blind where absent) recurring in the SAME SESSION that recorded it, because L817 was applied to the resim_band field and not to the env field beside it | L826 / L817 / #279 |
| Disposition a code fix while a long run, chain or campaign is LIVE - or tell the owner a code ticket is waiting on THEM | A LONG RUN PINS THE CODE IT LOADED AT START - A FIX TO THAT CODE IS BLOCKED BY THE RUN, NOT BY THE OWNER, AND SHIPS THE HOUR IT LANDS. A serial campaign's later configs import whatever is on disk when each STARTS, so an edit mid-chain leaves the early and late members on different code with nothing recording which - and that is a silent contamination of the comparison the campaign exists to make. MEASURED 2026-09-21: two tickets carried an owner-decision disposition when the real blocker was a running 36-config chain; asking the owner to rule would have been the wrong question, and the fix simply ships the hour the chain lands. Before routing a code ticket to the owner, ask what is RUNNING | L836 addendum / L619 (the mirror: wrongly labelling work owner-gated) / #279 |
| Read a final figure off a long run because its progress counter has reached the end - the last sim-day, the last epoch, 100 pct | A RUN AT ITS LAST SIM-DAY IS NOT FINISHED - PROVE TERMINALITY BY ZERO OPEN TRADES PLUS A FINISHED LINE IN THE CHAIN LOG, NEVER BY THE SIM-DAY INDEX. The counter reaches its final value while the job is still closing positions and writing its output, which is exactly the window where the number you want is still moving AND the run looks done. MEASURED 2026-09-22: a heartbeat at the last sim-day read 1.9291 h with open trades still outstanding; the final is 1.9708 h, and those 0.0417 h flipped a published conclusion from 0.2 pct below a mean to 2.0 pct above it - the third wrong statement about one run in one hour, the third from reading a moving artifact at a moment that looked terminal | L840b / L749 / #256 |
| Trust a content hash, digest or fingerprint that has come back UNCHANGED across many runs | A PROVENANCE GATE THAT COVERS ONE HALF OF THE CODE A RUN DEPENDS ON IS A GATE THAT REPORTS CLEAN WHILE THE OTHER HALF DRIFTS - ASK WHICH DIRECTORIES THE HASH ACTUALLY WALKS. An unchanged digest is evidence about COVERAGE at least as much as about the code, and a gate named for the thing it protects reads as complete. MEASURED 2026-09-22: an engine hash returned one identical digest across 12 launches and the freeze it enforces is genuinely real - and it walks backtest/ only, while the GRADING script that runs on every landing sits in scripts/, hashed by nothing. One grep of the hash function's own body settled it in seconds. Ask which directories it walks before trusting what its silence means | L844 / #224 / L586 |
| Find a gate firing on work you are confident is correct, or an unrelated test reddening the first time some state occurs | A GATE'S OWN SCAFFOLDING IS THE LEAST VERIFIED PART OF IT - A NEGATIVE CONTROL THAT READS LIVE REPO STATE MEASURES THE REPO, AND A GATE OVER AN ARTIFACT JUDGES ONE THAT NO LONGER EXISTS. Suspect the gate's SCAFFOLDING - its window, its negative control, its neutralisation - before the work. MEASURED 2026-09-22, two in one turn: a stall-check gate judged a monitor arm that had been deleted and replaced, firing forever on a job that no longer existed and costing three closes of rewording before I read the predicate; and an owner-decision gate read the live queue in its NEGATIVE CONTROL because it had no NEUTRAL entry, so it stayed green only until the first owner-gated ticket was ever closed, then reddened an unrelated corpus test. Neither was reachable by testing the gates - both needed a REAL state change, so each looked correct and passed its own suite until the world moved | L843 / L599 / L517 |
| Run a DRAFT script that resolves repo paths - from the scratchpad, a temp dir, anywhere that is not its intended home | A DRAFT CARRIES ITS DESTINATION'S PATH ASSUMPTIONS - A SCRIPT WRITTEN FOR THE REPO BUT RUN FROM A DRAFT LOCATION RESOLVES REPO FACTS FROM WHERE IT LIVES, SO RESOLVE BY MARKER AND REFUSE A TREE THE MARKER DOES NOT CONFIRM. MEASURED 2026-09-22 (L846): two analysis drafts written for scripts/ derived ROOT from their own file location and read a nonexistent tree; the failure surfaced only because a cube path happened to be missing. Both now resolve by marker and refuse otherwise - the refusal is the mechanism | L846 / #305 |
| Build a check, gate or audit that fails on MISSING or ABSENT data | AN ABSENCE-SHAPED OBSERVATION IS NOT A DEFECT UNTIL NO LIFECYCLE EXPLAINS IT - delisting, rename, late listing and window-predates-existence all produce sound absence, so fail only interior corruption and REPORT the edges. MEASURED 2026-09-22 (L847): the pre-leg bar audit's first live run failed 5 delisted-complete tickers of 200 sound ones; and LIVE-RUN a new gate before its refusal can block anything - standalone-first is why this cost one rerun, not a halted chain | L847 / #311 |
| Put a locked table - Table A, C, D, any named renderer's output - into a response | PRINT IT FROM ITS RENDERER OR DO NOT CALL IT THAT. Retyping creates a second unreviewed renderer, and the omissions read as editorial trimming rather than data loss. MEASURED 2026-09-23 (L848): four instances, three on Table C and one on Table D where 7 of 17 columns and 10 of 20 rows vanished while the same paragraph claimed it was printed and never retyped - every one owner-caught. An excerpt is fine when labelled an excerpt | L848 / #312 |
| Use a registry field as BOTH a display label and a lookup key - param names, column ids, config keys | FIND THE MEMBER WHOSE LABEL IS NOT ITS KEY AND TEST THAT ONE. The convention holds while every member's two readings coincide and fails SILENTLY on the first where they diverge - printing a not-recorded token about recorded data. MEASURED 2026-09-23 (L849): SPECS `param` was label and key, so the lookup rebuilt smc's keys exactly and missed all four candle axes, rendering `?` across 14 configs of the live campaign while Table D read the same values fine. Split the field; default the new one so no existing member moves | L849 / #313 |
| Append to a tracked text file from code, or pick a newline with `"\r\n" if "\r\n" in text else "\n"` | READ THE TARGET AND MATCH ITS DOMINANT ENDING - hardcode neither, and never infer from presence. The STORED form is a different question: with core.autocrlf on, git stores LF and checks out CRLF, so matching the stored form emits a line foreign to every neighbour (B3024b: that premise turned test_b2522 red). A writer cannot see its own mismatch, and presence-based inference lets ONE foreign line convert the whole file. MEASURED 2026-09-23 (L850): the landing hook's CRLF row in an LF-stored queue was invisible on its own commits and became a 120-line flip once 119 helper appends inherited it. Classify before fixing - a Windows .cmd writer REQUIRES CRLF | L850 / #314 |
| Ship a patch, held patch or behaviour change with a targeted test selector | NAME THE PINS YOUR CHANGE BREAKS, NOT ONLY THE ONES IT ADDS. A selector written from intent names what you BUILT; the pins encoding the OLD behaviour are exactly what a deliberate change should expect to break, and they are never in the selector because you were not thinking about them. MEASURED 2026-09-23 (L851): a parked patch named the fail-closed block it modifies three times in its own docstring and ran only its own new pin, so a pin contradicting an owner ruling would surface at the pyramid inside a large apply batch. Validate by applying in an isolated worktree and diffing the FULL suite against a control on clean HEAD | L851 / #315 |
| Write, or rely on, an idempotency guard - a patch that refuses to re-apply, a migration that skips work already done, any has-this-run-already check | ANCHOR IT ON A FORM PROSE CANNOT PRODUCE, AND RUN IT BEFORE THE FIRST WRITE. A guard keyed on a NAME is satisfied by any MENTION - MEASURED 2026-09-23 (L855): a docstring in an unrelated pin cited a held patch's test by name and made that owner-approved patch unapplyable, which would have surfaced at CHAIN DONE. And a guard placed after the edits it guards is decoration: the refusal fired at line 249 with the first write at line 78, leaving a half-patched tree while the output read as a clean no-op. Sweep the siblings - 2 of 2 held patches carried the shape, one blocking and one latent at zero mentions. Note also that a worktree command ending in a pipe reported EXIT 0 on this failure (L793) | L855 / #226 |
| Put a decision to the owner - a ruling, an approval, an A-or-B - whose write-up names an uncertainty of its own | BEFORE ESCALATING, WALK ITS OWN STATED UNCERTAINTIES AND MEASURE THE ONES THE ARTIFACTS ON DISK CAN SETTLE. A claim correctly labelled UNVERIFIED is COMPLIANT - the four evidence classes tag PROVENANCE, and UNVERIFIED is a valid one - so nothing fires and the honest label reads as rigour, which is what lets it sit. MEASURED 2026-09-23 (L856): an uncertainty I had labelled properly rode through FOUR consecutive closes inside an owner-facing decision while one command settled it, and settling it RETIRED one of the three objections I had raised against my own recommendation. The discriminator is who is waiting: unmeasured in your own reasoning is a note to self, unmeasured inside an escalation is a gap in the OWNER's evidence that only you can see is cheap to close. Check which way it points - mine favoured the option I was recommending (L559) | L856 / #318 |
| Re-derive WHERE a defect lives when the ticket already names it | A DEFECT'S LOCATION IS A CLAIM - THE HONEST PRODUCER THAT REPORTS A CONDITION AND THE CONSUMER THAT FAILS CLOSED ON IT ARE DIFFERENT FILES, AND THE TICKET THAT NAMED IT RIGHT IS THE ONE I OVERRODE. Re-deriving a location the ticket already records is not verification - it is a second guess competing with an answer written when someone had the code open, and it FEELS like the more rigorous move, which is why it is silent. MEASURED 2026-09-22: a ticket named its consumer correctly in its first sentence; I placed the defect in the honest PRODUCER, was forced to open that file, and retracted to a THIRD file that is declared report-only and cannot set an exit code. A behaviour pin for the true location had been green in the suite the whole time - a pin cannot make prose match it. The softened blocker built on the second wrong location reached the owner before the third file was checked | L845 / L841 / L639 |
| REPLACE an instrument rather than patch it - a monitor prompt, a runbook step, a spec template, a report format | REWRITING AN INSTRUMENT IS THE MOMENT TO SWEEP ITS KNOWN DEFECTS - A REWRITE THAT FIXES ONLY THE FAULT YOU WERE SENT FOR CARRIES EVERY OTHER ONE FORWARD WITH A FRESH TIMESTAMP. You are in fix mode on one named fault, so the replacement inherits the original's STRUCTURE including its blind spots, while looking like a fresh artifact - and any improvements you add make it look MORE reviewed. MEASURED 2026-09-22: sent to fix one wrong baseline in the hourly monitor, I rewrote the prompt with a better fix plus three unrelated corrections and reproduced the missing stall clause that L420 already records. Before replacing an instrument, grep the lesson record for that instrument's CLASS - monitor, cron, report - and check the replacement against each hit | L842 / L420 / #136 |
| Say what one of this repo's own gates, hooks or checks WILL do - expect it to fire, to void, to pass, to refuse | A CLAIM ABOUT WHAT YOUR OWN GATE MEASURES IS A CAPABILITY CLAIM - OPEN THE FILE THAT DEFINES ITS SCOPE BEFORE PREDICTING ITS VERDICT. #230 says a capability claim is a claim, and every example it gives is an EXTERNAL tool, format or budget - so the shape goes unrecognised when the subject is your own enforcement layer. MEASURED 2026-09-22: a landing hook committed mid-gate and I told the owner I expected tree=CHANGED and would re-run; the gate returned tree=SAME, and scripts/pyramid_gate.py states its scope in a comment at lines 40-42 - output_audit is excluded BY DESIGN for exactly that case. The tell is a sentence of the form I expect X to fire, about machinery that is one grep from being known. And watch the direction: a prediction promising extra caution reads as diligence, which is why nobody re-derives it | L841 / #230 / L559 |
| Extrapolate a finish, a cost or a completion date from a rate you read while the thing is still running | A RATE READ MID-RUN IS NOT STATIONARY - NAME THE SIM-DAY AN EXTRAPOLATION CAME FROM AND LABEL AN EARLY-DAY PROJECTION BIASED HIGH. L749 says carry a moving reading's AGE, and you can comply with that completely and still be wrong: the age is right and the MODEL is wrong, because a rate is not a constant you carry forward. MEASURED 2026-09-22 across three reads of one run's heartbeat: sim-day 117 extrapolated to 2.2331 h, sim-day 206 to 1.8752 h, the actual was 1.9291 h - the early figure high by 15.8 pct of the actual, and an owner-facing objection built on it was refuted by the actual landing 0.2 pct below the very mean it claimed to exceed. Note which way such an error points: the early reading made the run look SLOW, so it fed extra caution, and a cautious error is the one nobody re-derives | L840 / L749 / #256 |
| Put a BASELINE, threshold or target number into something that runs on a SCHEDULE - a cron prompt, a monitor, a runbook step, a template | A FIGURE INSIDE A SCHEDULED INSTRUMENT IS OUTSIDE EVERY REPO SWEEP AND RE-ASSERTS ITSELF ON A TIMER - GIVE THE BASELINE AN ARTIFACT TO BE QUOTED FROM, STAMPED WITH ITS WINDOW AND UNIVERSE. L836 covers two figures in one sentence, written once and caught once. A scheduled artifact re-states its figure on a timer, and repetition reads as corroboration; worse, it is not a file in the working tree, so the generalisation mandate's sweep CANNOT REACH IT. MEASURED 2026-09-22: an hourly monitor prompt carried R5 baselines at 4-year grain over 481 tickers for cubes running 1 year over 200 - a 4.09x over-performance would have read as 0.35x. AND NOTE WHAT THE SWEEP FOR IT DID: my first pass reported ONE tree-wide hit by reading the HEAD of a grep; the real population is 43 tracked files, 42 sound and 1 a genuine defect INSIDE the tree - so the reassuring half of a sweep is the half to re-run (L714). Derive the number into an artifact stamped with its window and universe and quote the artifact | L839 / L836 / #182 |
| Defer a ticket behind a NUMERIC or COUNTABLE trigger - re-open when the count exceeds N, on the third occurrence, once the log shows M | A NAMED TRIGGER THAT NOTHING EVALUATES IS A REMINDER, NOT A GATE - REGISTER AN EVALUATOR OR DISCLOSE THE TRIGGER AS UNEVALUATED. The rule that a deferral needs a named trigger makes the deferral honest and says nothing about anyone READING it, so the trigger is prose in a queue cell consulted only when a human happens to open the row. MEASURED 2026-09-22: a trigger reading exceeds 5 stood at 8 and had been over the line for three commits; the ticket's own last audit had measured 5 and written that it sat one event away. It surfaced because an unrelated turn-gate violation forced a grep of the log. If the quantity is countable, register an evaluator that DERIVES it from the artifact; if it is not, say so in the row so the audit can disclose it rather than print a clean zero | L838 / #224 / L639 |
| Rely on ONE commit to carry a set of artifacts that must land together - a Phase-5 member set, a fix plus its pin, a doc plus its generator | ANOTHER PROCESS MAY COMMIT TO THIS REPO WHILE YOU GATE - ASK WHAT ELSE COMMITS BEFORE RELYING ON ONE COMMIT TO CARRY A MEMBER SET. L815 covers YOUR batching splitting a set. This is the other source: an automated committer in the same repo stages overlapping paths and sweeps your edit into ITS commit, so the member lands in the record but not in the commit a one-deep gate inspects - and neither side warns the other. MEASURED 2026-09-22: the engine's landing hook stages the queue beside its own ledger row and took a queue row I had just appended into its commit while my pyramid was running; the file then read clean and the gate correctly reported the member missing. Before a set-carrying commit, ask what else writes to these paths - a landing hook, a cron, another session - and if anything does, verify with git log which commit actually holds each member rather than assuming yours does | L837 / L815 / B2132 |
| Report a sweep that found siblings of a defect you just fixed | RANK A SWEEP'S SIBLINGS BY HOW CONFIDENTLY THEY LIE, NOT BY HOW CLOSELY THEY RESEMBLE THE INSTANCE YOU FOUND. The instance you found is the one that happened to bite, not the worst one, and 'same class' says nothing about severity. Order them by the OUTPUT a reader would act on: a site that returns nothing at least looks like nothing; a site that returns a PLAUSIBLE WRONG VALUE is acted on without question. MEASURED 2026-09-21: my instance published a zero; its sibling computes a dominant-reason statistic by the same bad filter and would report a real-looking reason from 77 visible rows while the true dominant reason sits in 391,782 rows the filter cannot see. Say which sibling is worse and why, so the fix order follows harm rather than resemblance | L837 / L643 (a sweep must CLASSIFY, not count) / #166 |
| Filter a dataframe, ledger or record by a NAME and report how many matched - especially when the answer is zero | A BUCKET A FILTER CANNOT POPULATE MUST BE REFUSED, NOT REPORTED AS ZERO - CHECK THE COLUMN'S ATTRIBUTION BEFORE FILTERING ON IT. #166 says prove a GREP pattern can match before reporting absence; a dataframe filter is the same claim in different syntax and escapes the habit. MEASURED 2026-09-21: I published a zero as the decisive result of an investigation when the column being filtered held ONE literal value across all 391,782 rows, so the bucket was decidable by construction - and a fabricated zero standing beside four honest counts inherits their credibility. One nunique() call settles it. In an instrument, make it a REFUSAL that names the reason, not a zero | L837 / #166 / L824 |
| Put two RATES in one sentence - a conversion against a baseline conversion, a hit rate against a prior hit rate, a coverage pct against another | TWO RATES ARE NOT COMPARABLE BECAUSE BOTH ARE PERCENTAGES - NAME THE DENOMINATOR OF EACH BEFORE PUTTING THEM IN ONE SENTENCE. #182 disciplines a VERDICT's denominator and is SILENT on a rate. Two counts of different populations look wrong (1,596 against 560 invites 'over what window?'); two percentages look RIGHT, because they are already normalised, already unitless, already the same shape - and that shape is what hides the denominator. MEASURED 2026-09-21: I retracted a grain mismatch in my own words and committed the same class two rows later, setting 36.0 pct (post-screening) against 8.8 pct (pre-screening). Two council advisors caught it; no gate could | L836 / #182 / L664 |
| Treat a validator's clean result as permission to LAUNCH | ONE GATE'S SILENCE IS NOT LAUNCH READINESS - A SPEC-TIME VALIDATOR THAT DOES NOT ENFORCE WHAT THE LAUNCH-TIME GATE REQUIRES JUST MOVES THE REFUSAL DOWNSTREAM. MEASURED 2026-09-21: 36 specs returned 0 refusals and the chain halted on spec 1 at a different gate, for a field knowable in seconds. A thorough validator reads as authoritative over the whole question; scope is not implied by completeness. Name every gate between you and the thing you are starting, run them, and probe the NEXT one with the artifact the failure already produced - a gate can hold more than one absent field (B2937 refused three times) | L835 / L499 / #224 |
| Land a multi-line edit with a PATCHER or generated string | A PATCHER THAT BUILDS CODE FROM STRING LITERALS CAN COMMENT OUT WHAT IT MEANT TO ADD - AN ANCHOR COUNT CANNOT SEE THAT ITS MATCH SITS INSIDE A COMMENT. MEASURED 2026-09-21: a literal backslash-n in a non-raw literal is two characters, so the insert landed as one physical line and everything after the leading hash became comment text - six tripwire rows and the ratchet that counts them, commented out for 10 commits with the test GREEN, because the count stopped rising and the check stopped running together. Take the newline from the TARGET FILE, never from an escape, and re-read the written region as the PARSER sees it | L834 / L638 / #201 |
| Rely on a LADDER, on-ramp or checklist of rungs to say something is ready | A LADDER RUNG WITH NO ENFORCEMENT IS A CHECKLIST ITEM, NOT A GATE - AND THE UNENFORCED RUNG IS WHERE THE NEXT FAMILY WALKS THROUGH. Rungs get mechanized one at a time, and the enforced ones work loudly while the unenforced ones stay silent - so readiness reads green exactly where nothing is checking. MEASURED 2026-09-21: R1 refuses an incomplete adapter and fired on this pair; R2 was never mechanized, so the same pair reached launch-readiness with no Table D axis family and would have rendered six columns of dashes after 18 engine runs. Ask which SIBLING rungs of a mechanized one are still prose, and check a WORKING family against the rung's probe before concluding yours is uniquely broken - the probe itself may be loose | L833 / #224 / L790 |
| Quote a figure a SECOND time - in a ticket row, a commit message, a recommendation | A FIGURE THAT DRIVES A DECISION NEEDS A PERSISTED ARTIFACT, NOT JUST A COMPUTATION THAT ONCE RAN. #201 asks a figure to NAME its source and passes when you name a computation you ran; a computation that left no trace is not a source anyone can return to. MEASURED 2026-09-21, owner-caught with 'on what basis?': 27 of 54 was quoted across many turns with nothing on disk carrying it, and the sweep found 3 of 4 load-bearing figures in the same state - one of which had never been computed at all, only assumed symmetric with its mirror. Write it to an artifact the FIRST time; the test is whether the owner can re-derive it without you | L832 / #201 / L644 |
| Quote a NEW detector's output as evidence the detector works | A NEW DETECTOR'S FIRST OUTPUT IS NOT ITS VALIDATION - HAND-READ WHAT IT FLAGGED BEFORE QUOTING IT AS PROOF THE TOOL WORKS. It arrives wearing a measurement's clothes - the tool really ran, over real data, and returned a real number - and the error runs in the direction that makes your new tool look necessary. MEASURED 2026-09-21: a blocker audit's first run reported 2 stale predecessors, published in three places as validation; 2 of 2 were CITATIONS and the true count was 0. The discriminator that would have caught it sat in a sibling gate I had written one batch earlier - so DIFF A NEW DETECTOR AGAINST THE EXCLUSIONS OF THE ONE BEFORE IT | L831 / L644 / L608 / #201 |
| Summarise a backlog - "the rest are yours", "all remaining are owner-blocked", "nothing else is implementable" | DECLARING A BACKLOG OWNER-BLOCKED IS A CLAIM ABOUT EVERY TICKET IN IT, AND A BLOCKER IS THE LEAST-AUDITED CLAIM A TICKET CARRIES. That one sentence asserts N separate unexamined facts, and it is the sentence the owner acts on. MEASURED 2026-09-20: made three times in one session, wrong all three - a predecessor already EXECUTED, a byte measuring zero, and a two-month wait for an input 25 cubes already held. Run audit_ticket_staleness.py --blockers and name what each row waits on | L830 / L828 (status rots slower than cause) / #201 |
| Implement a ticket whose own row says NEEDS AN OWNER DECISION / OWNER RULING / BLOCKED ON APPROVAL | A TICKET THAT SAYS NEEDS AN OWNER DECISION IS A QUESTION ABOUT AUTHORITY, AND MERIT ARGUMENTS DO NOT ANSWER IT. Cost, a council, non-foreclosure and urgency are all MERIT arguments; the ticket asked WHO DECIDES. A self-convened council is not an approval body, and the budget you are protecting is the owner's to spend. MEASURED 2026-09-20: shipped one branch of a two-branch owner decision on exactly those three arguments, each true; the change was correct and reversible and the damage was that the owner can no longer tell which other decision-pending tickets he actually decided. ASK - the cost is three sentences - or state plainly that you are proceeding without the answer and why **MECHANIZED at B2929: `scan_owner_decision_taken` fires when a ticket whose EARLIER rows state a pending owner decision is marked terminal with no acknowledgement. L829 first filed this JUDGMENT-ONLY - no scan can read PROSE for load-bearing owner-decision language - and that was a compliance failure against #299, because the LEDGER is a state diff: prior rows are a fact, a terminal transition is a fact, and the pair needs no reading of intent. Keys on decision-pending forms, never citations. Proceeding is allowed; proceeding SILENTLY is not.** | L829 / L633 (the error is the SILENCE, not the disagreement) / L619 (the mirror: wrongly labelling work owner-gated) / #299 |
| Re-raise, re-quote or act on a BLOCKED / OPEN ticket | A TICKET BLOCKED ON AN OWNER DECISION CAN HAVE THAT DECISION EXECUTED BY ACCIDENT, AND NOTHING TELLS THE TICKET. STATUS is watched; CAUSE is not. MEASURED 2026-09-20: a row blocked on an owner ruling for an EOL renormalisation had had half that renormalisation landed ten days earlier inside an unrelated commit, undisclosed, and the exact byte its RCA blamed measured zero. Execute the row's OWN cited evidence - it names the file, offset or count, so it has already written the command - before quoting it or asking for the decision | L828 / L639 (a NUMBER decays; here the ACTION did) |
| Validate a TOOL a framework invokes - a grader, a checker, a renderer the battery/CI/hook calls for you | VALIDATE A TOOL THROUGH THE ARGV ITS CALLER BUILDS, NOT THE ARGV YOU TYPE - A DEFAULTED ARGUMENT IS INVISIBLE FROM THE COMMAND LINE. You type the arguments you are thinking about; the caller passes the ones it was configured with, and any argument with a DEFAULT is invisible in your run and load-bearing in its run. MEASURED 2026-09-20: a three-leg spot check was validated at 120 of 120 agreement by passing --strategy on the command line; the battery's own argv passes no such flag, and the checker defaulted to the LONG leg - so grading the SHORT leg of a long/short pair ran the long-leg check and recorded that step DONE against the wrong strategy. Rebuild the caller's argv in the test and assert on THAT | L827 / #276b (a control must take the same path as the claim) / L654 (a pin on the callee is not a pin on the wiring) |
| Report a sweep as CLOSING a class - "1 of 1", "exactly the incident", "zero collateral", "no siblings" | A SWEEP THAT REPORTS A CLASS CLOSED IS THE ONE LEAST LIKELY TO BE VERIFIED, BECAUSE CLOSURE ENDS THE WORK. A count that CONTINUES the work invites scrutiny because someone must act on it; a count that ENDS it invites none, and it is the same arithmetic either way. MEASURED 2026-09-20: 3 of 58 ticket rows written in one session asserted closure, 2 of the 3 were the same wrong claim repeated, and both were corrected only after a gate forced the classification. Before writing a closure figure, state what you would have to READ to be wrong, then read it | L819 / L679 (a probe measuring your own compliance gets MORE validation, not less - and a closure claim is the strongest self-measurement there is) |
| Choose the WINDOW for a turn-end gate, or find one blocking on text you have already corrected | A GATE OVER EVIDENCE MUST SPAN THE TURN; A GATE OVER A CLAIM MUST JUDGE THE LATEST BLOCK. The two need opposite windows and the same helper served both: an inspection done three closes ago is STILL TRUE, so evidence must accumulate - but a claim written in close 1 and corrected in close 5 has been REPLACED, and judging it forever means no wording in the current response can reach it and the turn can never close. Ask which KIND the gate reads before picking its window | L818 / B2555 (MEASURED 2026-09-20: the verdict-denominator gate blocked three consecutive closes on one sentence from an earlier one while the current close carried its denominators) |
| Report a new gate's BLAST RADIUS as evidence it is well-targeted - "refuses exactly 2 of 18", "zero collateral" | ASK OF THOSE IT DID NOT REFUSE, HOW MANY WERE EXPLICITLY EXEMPT VERSUS SILENTLY UNCLASSIFIABLE. A LOW REFUSAL COUNT IS WHAT A PRECISE RULE GIVES YOU AND WHAT A BLIND ONE GIVES YOU - blast radius and coverage are the SAME number and they mean opposite things, so the figure cannot tell them apart and reads as precision either way. The tell is a discriminator keyed on an OPTIONAL field: where it is absent the rule is silent, and silence counts as a pass. Split the unrefused population before quoting the count | L817 / L642 (MEASURED 2026-09-20: an R1 refusal reported as 2 of 18 with zero collateral; of the entries carrying env knobs but no engine-requiring band, 3 of 3 were BLIND on an absent resim_band and 0 of 3 explicitly exempt - including the most-run engine family in the repo) |
| Cite a PRIOR search, grid, holdout or sweep as evidence that a subject is exhausted, settled or dead | A COMPLETENESS CLAIM ABOUT A SEARCH IS SCOPED TO THE PARAMETER SPACE THAT EXISTED WHEN THE READ WAS TAKEN. A read that was COMPLETE on the day it ran covers only the axes that existed then, and adding a parameter later silently voids it - while the artifact keeps its authoritative shape, its line count and its FAIL verdict. Nothing re-opens the old conclusion, because nothing links a new knob to the reads it invalidated. Before quoting one, LIST THE AXES IT COVERS AND THE AXES THAT EXIST NOW, and state the verdict as N of M axes | L816 / #182 (MEASURED 2026-09-20: I cited a 130-line Step-2 holdout as proof a strategy pair was exhausted; the artifact covers ONE axis - the RSI levels - and the four anatomy axes did not exist until the same session wrote them, so the true scope was 1 of 5 and the owner had to overrule the recommendation) |
| Split a remediation, a fix, or any multi-part obligation across MORE THAN ONE COMMIT | ASK WHETHER ANY CHECK PROVES THOSE PARTS TOGETHER, AND HOW FAR BACK IT CAN SEE. A gate that proves its members by inspecting recent HISTORY rather than the working tree makes the ORDER of your commits part of what is checked - so a multi-commit arc becomes a sequence of PARTIAL STATES it will correctly refuse, and each refusal re-emits the mandatory blocks to the owner. The content being right buys nothing here; only the ordering does | L815 / L628 (MEASURED 2026-09-19: Phase-5 members are proved one commit deep by _artifact_touched; five consecutive closes blocked, each on a DIFFERENT member, each satisfying the one the last had missed, the gates correct every time) |
| Fix a shared classifier, scrub, predicate or helper and find the symptom STILL THERE | A FIX THAT DOES NOT CLEAR THE SYMPTOM IS EVIDENCE OF A SECOND CONSUMER, NOT EVIDENCE THE FIX WAS WRONG. The reflex is to doubt the patch and widen it, which is how an escape gets over-widened and starts failing open; the cheaper question is which OTHER code asks the same question. Grep the question's consumers BEFORE the second patch and route every leaf through the trunk, rather than fixing each leaf as it bites | L814 / L608 (MEASURED 2026-09-19: three patches to one launch question across two detectors in one turn - and the same pair had already been fixed per-consumer once for the heredoc strip at S6-B1925a, so the count was available before any of the three) |
| Correct a measurement, a counter, a parser or any figure other code consumes | READ EVERY CONSUMER AND ASK WHETHER IT WAS READING A REAL PROPERTY, OR A SIDE-EFFECT OF THE BUG. A defect produces a characteristic artifact and nearby code comes to treat that artifact as a signal - correct in the defective world, wrong the instant you fix it, and invisible because nothing ever failed. The tell is a consumer testing the measurement for a SPECIFIC VALUE (n == 1, == 0, is None) rather than using it as a magnitude. Delete the degree of freedom rather than documenting the proxy | L814 / #196 (MEASURED 2026-09-19: a band counter fix made n == 1 stop meaning "no actuator" seven lines below the edit; one file rendered FULL FACTORIAL 382,637,520) |
...
| Launch a background command whose exit status you will quote - a driver, a verifier, a gate run | END THE LAUNCH LINE WITH THE COMMAND'S OWN EXIT: `cmd > log 2>&1; rc=$?; echo "X=$rc"; exit $rc`. A trailing `echo` makes the completion notice report the ECHO's status, which succeeds whatever the command did; read the verdict token from the output before quoting any result | L872 / #306 (MEASURED 2026-09-24: 'finished with exit 0' was the echo's, and DRIVER_EXIT=3 - NOT EQUIVALENT - sat in the log) |
| Run the full pyramid while an engine run is live - "it is only a ranking run", "commit has headroom" | ASK WHAT ELSE READS THE PYRAMID'S OUTPUT BEFORE THE RUN ENDS, NOT ONLY WHAT IT COMPETES WITH: the stamp gates EVERY commit in the repo, the unattended landing supervisor's included, so a RED result from any cause - contention too - stalls the owner's automation. Run the suite before the launch or after the landing (runbook Step 2.4); pyramid_gate refuses it since B3108 (S6-B3062) and an override needs a recorded --beside-wave reason | L873 / #319 (MEASURED 2026-09-25: a pyramid beside the live c14 wave failed 2 of 1,537, one on memory; its RED stamp refused the c14 landing commit 5 of 5) |
| Run a probe that writes into live files - a mutation probe, a fault injector, a patcher test | BACK UP EVERY TARGET BEFORE THE FIRST WRITE, REPORT AN ANCHOR THAT DOES NOT OCCUR EXACTLY ONCE, AND AFTER ANY INTERRUPTION COMPARE THE TARGETS WITH THE BACKUP BEFORE TRUSTING ANYTHING THAT RAN THERE - a `finally` restore does not run when the process is killed | L874 / #323 (MEASURED 2026-09-25: an interrupted probe left preflight C1 failing open in the worktree; the next run's anchor count 0 surfaced it) |
