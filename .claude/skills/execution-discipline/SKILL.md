...
| Count anything held in an APPEND LOG - a ledger, a jsonl record, an event stream | CALL ITS REDUCER. An append log answers only through last-row-wins / latest-per-key; a raw row count is a number about the FILE, not the world - always available, always plausible, and wrong the moment one key has two rows. One authoritative reader per log, and every consumer calls it | L737 / #271 (`queue_state.py` for the queue, `undelivered()` for landings; I printed "undelivered now: 6" from a raw comprehension where the reducer returns 0) |
