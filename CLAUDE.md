...
- **Fork-first architecture** — for every new component, identify battle-tested libraries before proposing custom code. Default to forking unless integration cost > rebuild cost OR requirement is genuinely novel to this project. Custom code reserved for what's UNIQUE to our system (signal computation, agent prompts, risk context, earnings_tolerant logic, PIT semantics, validation methodology, the layered strategy roster — Layer 1 baseline 60 + Layer 2 Phase 0.D ICT/Earnings/Calendar + Layer 2D form-derived ICT + Layer 3 Pass 52 RESOLVED chart-pattern/categories + Layer 4 PENDING. **Live count as of 2026-05-26: `len(ALL_STRATEGIES) = 186` total registered / `len(DEPRECATED_STRATEGIES) = 0` (Batch 316a empty) / `len(STRATEGIES_DISABLED_MISSING_PRODUCER) = 1` (Batch 372 `dxy_headwind_multinational_short` — foreign_rev_pct producer absent) / 185 active for Phase 1A-β re-run**. The 108-133 figure in CANONICAL_FACTS.md F-002 is pre-Batch-316a stale.). Per L103: read library source before recommending. Per DEC-045 (RESOLVED Pass 27): use this approach across all phases. Already-adopted forks: smartmoneyconcepts (ICT), TradingAgents (multi-agent), QuantStats (analytics), Streamlit (dashboard), ib_async (broker), freezegun (tests), OpenBB+Polygon (fundamentals).
...
backtest/
  ...
  signals/
    ...
    screener.py          # 186 strategy classes registered live `len(ALL_STRATEGIES)` 2026-05-26 (was 148 pre-Batch 316a un-deprecation; Layer 1 baseline 60 + later layers per CANONICAL_FACTS.md F-002). 0 in DEPRECATED_STRATEGIES (Batch 316a empty). 1 in STRATEGIES_DISABLED_MISSING_PRODUCER (Batch 372 `dxy_headwind_multinational_short` — foreign_rev_pct producer absent). 185 active for Phase 1A-β cube; tests 185 × 25 = 4,625 cells.
  engine/
