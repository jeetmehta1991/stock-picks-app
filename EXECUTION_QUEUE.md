...
| 5 | `dashboard-batch435-verify` | Verify dashboard readability post-Batches 430 / 433 / 434 / 435 | BLOCKED | **Batch 435 shipped (1f08d161c).** Tab 1 KPI labels relabeled so source is unambiguous (Registered code / In this table this run / Fired in cube / Quiet in cube Tab 12) + inclusion-criteria callout under the KPI grid: literal criteria is "fired >= 1 trade in this backtest run". Tab 14 leads with a "First-time viewer? Start here" block explaining two-dataset architecture + recommended reading order + what "good" looks like + how to debug blank tabs. **Owner action:** hard refresh; click Tab 14 Reference and read the blue First-time-viewer box first. Surface remaining tab-level confusion as queue items so we can address each tab one at a time. |
