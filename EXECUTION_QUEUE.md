...
| 5 | `dashboard-batch434-verify` | Verify all tabs + new Tab 14 Reference render post-Batches 430 / 433 / 434 | BLOCKED | **Batches 430 / 433 / 434 shipped (c58be1529).** Tab 14 Reference now answers owner's "why 36 of 185" question with surfaced split (185 registered / 100 fired / 36 in CSV / 85 quiet) + full background docs (strategy library by layer, 25 exit methods grouped, 9 success criteria, DEC-426 5-Gate, universe architecture, regime classification, sizing tiers, tab-by-tab navigation guide, glossary, data flow). Tab 1 KPIs also show the same 185/100/36/85 split. **Owner action:** hard refresh; click 14. Reference tab first to read the schema; then walk back through tabs 1-13 with that context. Surface any individual tab confusion as separate queue items per CHECKLIST #94. |
