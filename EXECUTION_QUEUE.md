...
| 5 | `dashboard-batch437-verify` | Verify dashboard post-Batches 430/433/434/437 (cube-only architecture per owner directive) | BLOCKED | **Batch 437 shipped (fd583e62e).** Dashboard rebuilt with `--source output_batch395_final` (cube). Tab 1 now shows 101 strategies. Tab 14 banner explains cube-only + 4 Phase 1A-β gates off + which tabs will say "No data" because cube doesn't emit single-run analyst overlays. data.js trimmed 371 MB -> 17 MB (dropped wide trade_log columns + capped breakdowns at 200 rows). **Owner action:** hard refresh; surface remaining tab-level questions one at a time so each can be addressed cleanly. |
