...
| 5 | `dashboard-batch436-verify` | Verify dashboard readability post-Batches 430 / 433 / 434 / 435 / 436 | BLOCKED | **Batch 436 shipped (b6efd228d).** Tab 1 callout now answers "why 36 not 100" in-place: it cites both runs explicitly + the 4 flags the cube turned off (cap / dd-halt / regime / event suppression) so the viewer sees the mechanism. Tab 14 First-time-viewer block now has a concrete comparison table with the 4 flag rows + the resulting fire counts (36 vs 100) on either side, plus the sentence "Same threshold, different gates upstream of the trade log." **Owner action:** hard refresh; the Tab 1 callout + Tab 14 table should now answer the count-delta question directly. |
