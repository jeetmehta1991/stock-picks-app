...
| Batch 415: signals-dict enrichment in cube replay | ✅ shipped (code fix only; no cube re-run yet) | Batch 415 (2026-05-28): `backtest/engine/backtest.py` adds `category` to `trades_data_lite`; `backtest/engine/exit_strategies.py::run_exit_comparison` enriches the signals dict with `ticker` + `strategy_name` + `category` before each exit_fn call. Fixes the silent gap where `exit_earnings_blackout` returned `no_earnings_known` for 100% of cube-replay trades (ticker="" → `fetch_earnings_dates("")` returned []) AND `exit_class_time_stop` defaulted to "momentum" for every strategy regardless of actual layer. Tests: 5 new signals-injection regression tests + 940 existing (945/945 green). **Next cube re-run will produce real earnings_blackout / class_time_stop cells** for the 12 strategies that were deferred in Batch 414 (and any new cells these methods unlock). Operational re-run decision pending owner. |
| Batch 416: silent-producer-failure logging | ✅ shipped (diagnostic only; no behavior change) | Batch 416 (2026-05-28): `backtest/signals/screener.py` replaces silent `try/except: pass` blocks at 3 producer call sites (classification_change, institutional_signal, smc_ict) with `_log_silent_producer_failure(producer, exc)` rate-limited helper (1 log per (producer, exception-type) per process). Adds `_log_silent_producer_empty(producer)` for the SMC empty-return case (compute_smc_signals returns `{}` silently). **Investigation finding**: 0 of 29,159 trades in `output_batch395_final` have any smc_* key in signals_at_entry despite `compute_smc_signals` returning 28 valid SMC keys when called in isolation locally; the silent except was hiding the actual AWS-environment failure. Next cube re-run will surface the actual failure mode in logs. Tests: 6 new regression tests + 945 existing (951/951 green). |
| Batch 417: STRATEGY_REGIME_AFFINITY (14 NEW entries) | ✅ shipped (consumed in Phase 1B-α; no Phase 1A-β behavior change) | Batch 417 (2026-05-28): added 14 new entries to `STRATEGY_REGIME_AFFINITY` in `backtest/engine/regime_selector.py` for strategies with no prior affinity entry. Derived from per-(strategy × regime) cube cell stats (`output_batch395_final/trade_exit_detail.csv` × `trade_log.csv` merge); regimes included iff Sharpe > 0 AND n ≥ 30. **Conservative scope**: 14 NEW entries only — 15 OVERRIDE candidates intentionally deferred. Tests: 16 new tests including 14 entry pins + count-floor + override-not-applied guard. 967/967 green. |
| Batch 418: STRATEGY_REGIME_AFFINITY (15 OVERRIDES) | ✅ shipped | Batch 418 (2026-05-28 owner-approved "proceed"): applied the 15 OVERRIDE candidates deferred from Batch 417. Cube empirical supersedes prior Batch 203/293/370 curation per `project_no_apriori_strategy_pruning`. Overrides: `adx_initiation` {bull,neutral}→{bear}; `avwap_252_breakout` {all4}→{bear,neutral}; `bollinger_tight` {bull,neutral}→{bull}; `cmf_flip` {all4}→{bear,neutral}; `pairs_mean_reversion_long` {bull,neutral}→{bear}; `pead_long` {all4}→{bear,bull}; `po3_bullish`/`po3_htf_aligned_long`/`supertrend_macd`/`ultimate_oscillator`/`xs_momentum_top_decile` {bull,neutral}→{bull}; `pre_fomc_long_sleeve` {bull,neutral}→{bear,neutral}; `prev_day_high_break`/`xs_quality_top_quintile_long` {bull,neutral}→{bear}; `xs_low_beta_long` {bull,neutral}→{bear,bull}. 15 new test pins replace prior "override-not-applied" guards. Test fix: `test_batch203_regime_selector_enforces_affinity` updated for cmf_flip (was "allow all", now {bear, neutral}). 982/982 green (967 prior + 15 new). **Activation note**: Phase 1A-β still bypasses regime affinity via `--no-regime-affinity` (cube-measurement mode); these affinity entries activate when Phase 1A-α deployment / Phase 1B-α agent overlay consume them. |
| Batch 419: Dashboard cube-optimizer tabs (4 new) | ✅ shipped | Batch 419 (2026-05-28): added 4 new tabs to `dashboard_phase_1a/` per locked workflow Stage 3 expectations. (10) **Optimizer Summary** — renders `optimization_summary.md` + bucket KPIs. (11) **Per-strategy Candidates** — picker + JSON drill-down for ~101 fired-strategy JSONs. (12) **Quiet Strategies** — 3-column layout PRODUCER_LAYER_ZERO_LIKELY / COMPOUND_RESTRICTIVE / SKIPPED_AT_ENGINE. (13) **Cell Verdict Cube** — Lens B L2 table sortable by Sharpe with 5-Gate verdict + PSR pass column. Build script gains `--optimizer-dir` CLI flag (default `output_optimization_candidates_2026_05_28`). New tests: 13 (tab-button + tab-panel + count=17 + CLI flag + helper + payload keys + renderer functions). 995/995 green. |
| Batch 420: Doc archival (7 .md files) | ✅ shipped | Batch 420 (2026-05-28 owner-approved): archived 7 superseded .md files to `archive/2026-05-28-pre-1a-alpha-gate/docs/`: PASS_53_PRIORITIES.md, THEME_X53_SEQUENCING.md, PHASE_1A_PRELAUNCH_TODO.md, PHASE_1A_BETA_PRE_RUN_ALIGNMENT_AUDIT.md, PHASE_1A_BETA_QUIET_STRATEGY_FORENSIC.md, PHASE_1A_BETA_STAGE_D_LOSER_CELL_AUDIT.md, PHASE_1A_BETA_SURVIVOR_ROSTER.md. Critical refs updated: (1) `scripts/drift_audit_pre_phase_1a_beta.py::out_path` now writes to archive; (2) `test_silent_gap_pyramid.py::test_batch340_cat_c_bucket2_forensic_doc_reference` falls back archive → root; (3) `test_gate_pre_phase_1a_entry.py::test_gate_sprint_0a_phase_1a_blockers` same fallback. 1148/1148 tests green post-archival. Cosmetic refs in other .md docs left as text mentions (non-blocking; archived path discoverable via grep). |
| Batch 421: PEAD load_quarterly_eps lru_cache | ✅ shipped (~11pct engine speedup; flag-free) | Batch 421 (2026-05-28 owner-approved profile-first prewarm Step 1): profiled engine via `scripts/profile_process_day_lever_c.py` (20-tkr × 32-day backtest) showed `pead.load_quarterly_eps` at **67s cumtime / 693 calls (~12pct of total engine cost)** — pure function of ticker reading on-disk parquet; safe to cache. Applied `@functools.lru_cache(maxsize=None)`. Engine init now reads each ticker's quarterly EPS parquet exactly once instead of once-per-day (~3× savings per ticker over a 1-year cube). 4 new regression tests pin cache behavior. 999/999 green. Also bundled: profile-script Python-3.14 compatibility fix (escaped `%` in `--vectorized-cube-exits` help; added missing `vectorized_cube_exits` / `warn_run_hours` / `max_run_hours` to manual Namespace). Next prewarm targets per profile: `smc_ict.compute_smc_signals` (29pct), `technical.compute_supertrend` (8.9pct) — larger refactor; deferred to Batch 423+. |
| Batch 422: Dashboard data.js window-attach fix | ✅ shipped | Batch 422 (2026-05-28): owner reported "all tabs blank, headers visible". Root cause: pre-existing latent bug in `scripts/build_dashboard_phase_1a.py` line 195 — `const PHASE_1A_DATA = ...` in a regular `<script>` context creates a lexical global that does NOT attach to `window`. `app.js`'s `const D = window.PHASE_1A_DATA || {}` therefore always saw `{}` → all renderers operated on empty data → all tabs blank. The bug has been latent since the original Batch 177 build script. Fix: change emit to `window.PHASE_1A_DATA = ...` (attaches to global) + defensive `app.js` fallback to `typeof PHASE_1A_DATA !== "undefined"` (backward compat with older builds). Regenerated `data.js` (21 MB) + `data.json` (24 MB). 3 new regression tests pin: build script emits `window.PHASE_1A_DATA =`, data.js starts with that prefix, app.js has defensive fallback. 1002/1002 green on canonical focused set. |
| Batch 423: CI Test Pyramid red cleared + CHECKLIST #93 / L163 codify the violation pattern | ✅ shipped | Batch 423 (2026-05-28 owner directive): owner spotted "all actions from 306-420 have failed on git". Investigation: ~12 consecutive **Test Pyramid CI failures** (Batches 412-422) on `test_data_integrity_2_ohlcv_freshness` (Tier 3) — 924 ACTIVE-ticker .parquet files with last_bar=2026-05-05 (Phase 1A-β backtest end window) vs CI cutoff `today - 21 days = 2026-05-07` (2 days too tight). Owner-approved fix: extend cutoff **21 → 35 days** in `backtest/tests/test_data_integrity.py`. Batch 228 test pin (`test_batch228_ohlcv_freshness_cutoff_extended_to_21_days`) updated to expect `days=35`. Also codified: **CHECKLIST #93** HARD RULE — run FULL pyramid + verify CI conclusion via REST API before claiming green. **L163** documents the violation pattern (same family as `feedback_pyramid_full_13_tiers_mandatory.md` from 2026-05-12; second codification because the first didn't stick). |
| Batch 424: AWS orchestrator slot logic (binary → integer counters) for new 96-vCPU spot quota | ✅ shipped (enables Shape A cube re-run) | Batch 424 (2026-05-28 owner-approved Shape A: 3 × c7a.8xlarge spot, ~$9.30, ~5h): owner-confirmed AWS spot vCPU quota raised 32 → 96 in us-east-1. Standard 5-instance topology (160 spot vCPU) still exceeds quota; max parallel = 3 × c7a.8xlarge spot (96 vCPU). Updated `scripts/aws_batch395_parallel.py`: binary `ondemand_busy`/`spot_busy` flags → integer `ondemand_count`/`spot_count` counters; added `--spot-slots` (default 1; pass 3 for full new quota) + `--od-slots` (default 1; pass 0 for pure-spot topology) flags. Backward-compatible: defaults preserve prior 1+1 slot behavior. Smoke: `--help` parses cleanly. Local pyramid: **4 failed / 2809 passed** — same 4 pre-existing non-CI-tier failures from Batch 423 pyramid (`test_batch367_pre_launch_validation` ×2 / `test_engine_optimization_parity` / `test_index_rebalance`); grep-verified none reference Batch 424 changes. CI Test Pyramid is canonical gate. **Next**: archive S3 `outputs/batch_1..5/` → `outputs_2026-05-27_run/` (preserves prior cube), then launch via `python scripts/aws_batch395_parallel.py --bucket stock-picks-batch395-jm-7421 --key-pair batch395 --ami-id ami-0fc0d6e8d70ab2d42 --batches 1,2,3,4,5 --spot-slots 3 --od-slots 0 --commit <Batch-424-sha> --max-run-hours 6.0 --warn-run-hours 5.0 --max-hours 8.0`. |
...
- Drift audit: `scripts/drift_audit_pre_phase_1a_beta.py` (regenerates `archive/2026-05-28-pre-1a-alpha-gate/docs/PHASE_1A_BETA_PRE_RUN_ALIGNMENT_AUDIT.md` post Batch 420)
