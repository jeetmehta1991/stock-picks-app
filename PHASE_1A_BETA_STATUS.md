...
| Batch 417: STRATEGY_REGIME_AFFINITY (14 NEW entries) | ✅ shipped (consumed in Phase 1B-α; no Phase 1A-β behavior change) | Batch 417 (2026-05-28): added 14 new entries to `STRATEGY_REGIME_AFFINITY` in `backtest/engine/regime_selector.py` for strategies with no prior affinity entry. Derived from per-(strategy × regime) cube cell stats (`output_batch395_final/trade_exit_detail.csv` × `trade_log.csv` merge); regimes included iff Sharpe > 0 AND n ≥ 30. **Conservative scope**: 14 NEW entries only — 15 OVERRIDE candidates (where cube disagrees with existing Batch 203/293/370 curated entries) intentionally NOT applied; owner deferred those to per-strategy review. **Also deferred**: 25 strategies with all-regime-negative Sharpe (golden_cross_9_21, force_index_breakout, ichimoku_cloud_breakout, po3_bearish, stochrsi_overbought_short etc.) — owner deferred deprecation decision pending cube re-run with Batches 415/416 fixes. Tests: 16 new tests including 14 entry pins + count-floor + override-not-applied guard. 967/967 green (951 prior + 16 new). |
