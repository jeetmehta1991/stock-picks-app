...
**Sprint 0A.9** — **Polygon ticker events integration (DEC-500 Pass 53 owner directive 2026-05-05)** — `https://api.polygon.io/vX/reference/tickers/{ticker}/events` (Reference Data, included in Stocks Starter). Event types: ticker_change, ticker_split, name_change, listing_change, exchange_change, delisting, new_listing. Cache: `data_prefetch/polygon/events/{ticker}.parquet`. Feeds all 11 active TradingAgents per DEC-057 + DETAILED_PROJECT_PLAN §2.6 (Market / Fundamentals / News Analysts + Bull / Bear Researchers + Research Manager + Trader + Aggressive / Conservative / Neutral Risk Debaters + Portfolio Manager) + T2 SCREENER per DEC-380. Note: prior "6 TradingAgents" wording reflected pre-Pattern-2 conceptual roles; correct enumeration is 11+ active LLM nodes per L94 / Pass 26.
