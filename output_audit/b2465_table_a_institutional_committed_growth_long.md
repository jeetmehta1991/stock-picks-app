...
| P7 | `strat_institutional_committed_growth_long` | `min_committed_growth` | 3 | 1, 2, 3, 5, 11, 142 | FREE: 3, 5, 11, 142<br>RESIM: 1, 2 | **UNTESTED** | **0 configs** - not swept | `screener.py:6648` | PERSISTED, so this row splits PER LEVEL - which the pre-B2467 binary field could not express and which is why the old factorial read 31,500. Raising the bar (5, 11, 142) selects a STRICT SUBSET of rows already in the cube and grades FREE; lowering it (1, 2) admits rows the cube never contains and needs the engine. The fallback does NOT break this: raising the primary threshold leaves committed_growth_holders unchanged, so rows at 0 still take P8 identically and rows at 3-4 simply stop firing. Levels are the measured IS deciles over 1,275 IS rows. |
...
COMPUTED from the rows, never hand-written: **1 baseline + 16 varied = 17 configs**.
At the measured Step-1 median of 1.68 h/config (57 configs, 200 tickers x 1 year,
elapsed_s in the b2197 wave summaries) that is **29 hours**.
