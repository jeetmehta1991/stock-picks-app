...
> **🔴 B1250 DATA-CONTAMINATION CAVEAT (2026-07-08, Council 296).** The B1250 engine audit found that `signals_at_entry` was wiped to 0-4 keys for ALL Batch A trades by a checkpoint/resume serialization bug (ENG-1 in `output_audit/B1250_ENGINE_BUG_AUDIT_2026_07_08.md`). Consequence for THIS review's Section 2: every cube-replay exit cell ran with **ATR = 2%-of-price proxy** (not real ATR), and all signal-conditional exits (`atr_trail_vix_conditional`, `atr_trail_mae_conditional`, `smart_money_reversal`, `smc_mitigation_zone`, `reverse_signal`) silently degraded to their fallback trails — which is WHY their medians are identical to `atr_trail_1x`. Exit-family RELATIVE rankings among unconditional exits (time stops, R-multiples, breakeven_plus_trail, fixed trails) remain informative; verdicts on the five conditional exits and all ATR-scaled stop DISTANCES must be re-derived after the ENG-1 fix + exit-cube re-run. Strategy-level entry-edge medians (Section 3) are less affected (medians across 26 exits, most of which never read signals) but absolute PF levels carry the proxy-ATR distortion.
