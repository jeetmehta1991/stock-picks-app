...
def _engine_strategy_file(spec: dict) -> str:
    ...
    file stamps the split so the exemption is on-disk, never implicit."""
    riders_rel = spec.get("cube_riders")
    if not riders_rel:
        return spec["strategy_subset"]

    def _lines(rel):
        return [ln.strip() for ln in (ROOT / rel).read_text(encoding="utf-8").splitlines()
                if ln.strip() and not ln.lstrip().startswith("#")]

    graded = _lines(spec["strategy_subset"])
    riders = [r for r in _lines(riders_rel) if r not in graded]
    merged = ROOT / "output_audit" / f"_engine_set_{spec['wave']}.txt"
    merged.write_text(
        "# GENERATED by run_wave (B2710): graded subset + cube riders\n"
        f"# graded (battery): {', '.join(graded)}\n"
        f"# riders (cube-only, battery-exempt by design): {len(riders)}\n"
        + "\n".join(graded + riders) + "\n", encoding="utf-8")
