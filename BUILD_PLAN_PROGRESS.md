...
| **322: engine pool wiring** | **[QUEUED]** | Add `self._screen_pool = None`, `_init_screen_pool()` (mp.get_context("spawn")), pass `pool=self._screen_pool` to screen_universe, teardown on exit. Stage-D smoke run + byte-by-byte trade-log parity required before any Phase 1A-beta re-run. Estimated 1 focused working session; Hetzner CPX62 16-vCPU theoretical 4-8x speedup. |
