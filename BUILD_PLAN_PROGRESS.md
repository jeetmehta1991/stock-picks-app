...
| **Batch 351** Sprint 7 Phase A upstream-schema reconciliation + Tier 2 mocked-LLM tests | [SHIPPED 2026-05-25] | Read upstream `vendored/tradingagents/agents/utils/agent_states.py` + `graph/propagation.py` verbatim. Three field-name mismatches found in Batch 350's `AugmentedAgentState`: `trader_decision`->`trader_investment_plan`, `risk_debate_history`->`risk_debate_state`, `final_decision: dict`->`final_trade_decision: str`. Renamed in `state_augmentation.py`; declared all 14 upstream fields for type-completeness; added upstream-required init keys (`company_of_interest` + `asset_type` + `trade_date` + `past_context`) to `build_augmented_state()` output so the dict can drop into upstream `Propagator.create_initial_state` path. New canonical schema-diff doc: `PHASE_1B_STATE_SCHEMA_DIFF.md` (field-by-field upstream-vs-our table + rename history + next-batch follow-on plan). 12 new Tier 2 tests in `backtest/tests/test_toolkits_phase_a_tier2.py` covering T2 integration (upstream key population + alias agreement + project-extensions preserved) + T6 regression (TypedDict declares all upstream fields + zero Batch-350-legacy names + 9 project extensions). Full pyramid 1078 PASS / 0 FAIL. |
| **Batches 352-354** Forensic Cat-A/B/C re-cat + un-deprecated 23 verdict + regime-bias audit | [SHIPPED 2026-05-25] | Owner directive: forensic batches. `scripts/forensic_phase1a_beta_recat.py` reads `output_phase_1a_beta_merged_local/trade_log.csv` (7191 trades / 66 fired strategies). **Critical empirical findings:** (a) Cat-A 14 + Cat-B 20 + Cat-C 16 + Un-Deprecated 23 = **73/73 strategies QUIET** at Phase 1A-β run time (pre-Batch 312-314 fixes + Batch 218 deprecation still in force). (b) **0 of 66 fired strategies PASS** the CLAUDE.md passing criteria — 32 FAIL, 34 INSUFFICIENT_DATA. (c) **All 3 regimes negative-PnL**: bear 2910 trades / -7874.97pp / WR 25.12%; bull 3810 / -2641.14pp / WR 32.99%; neutral 471 / -871.04pp / WR 33.97%. Aggregate ~ -11,387pp across 7191 trades. **Worst-5 bear drivers**: xs_momentum_bottom_decile_short -1856pp, hull_rsi -1341pp. Phase 1A-β re-run after Batches 312-314 + 316a unblock + Wave 3 30/30 + look-ahead audit + Sprint 7 toolkits is mandatory before any winners.parquet extraction. Caveat: Sharpe is per-trade proxy (mean/std), not annualized portfolio Sharpe — the FAIL verdicts are directionally correct but quantitatively conservative. Report at `output_audit/phase1a_beta_recat.md` + JSON. 16 forensic tests pass (T1 unit + T2 smoke + T6 regression). |
