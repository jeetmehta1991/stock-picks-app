...
| **DECISION-440** | Alpha Vantage replaced by Polygon — Pass 52 owner-confirmed (turn 16): "I believe it was polygon. I remember covering in another chat on pricing about polygon." Polygon takes over Alpha Vantage's roles: news sentiment (replaces 25-ticker AV cap), fundamentals where applicable. Alpha Vantage code consumption (backtest/agents/pipeline.py + data/smart_money.py + scripts/prefetch_alphavantage_news.py) marked for deprecation post-Polygon migration. Polygon $30/month subscription — already considered Stage 1-2 in scope per DEC-410. Existing AV cache/parquet artifacts retained for transition period; new prefetches go through Polygon. Joint with DEC-410 (audit treats AV as deprecated and Polygon as canonical news/fundamentals source). Sub-decisions to follow: (a) Polygon news endpoint integration; (b) Polygon fundamentals coverage check; (c) AV code deprecation timeline. | PENDING | Batch X56 — API Endpoint Utilization Audit (Pass 52) | Pass 52 | - |
