...
| **DECISION-410** | API endpoint utilization audit — Pass 52 owner-approved CRITICAL TODO with **comprehensive 6-dimension use-case mapping schema** (Pass 52 turn 16 expanded per L136/CHECKLIST #57). Deep-dive inventory of ALL available endpoints/dimensions per Stage 0+Stage 2 in-scope APIs. **In-scope APIs (per CHECKLIST #56 scope filter):** yfinance, Polygon (replaces Alpha Vantage per owner Pass 52 confirmation; Polygon also adds news endpoint), OpenBB, Quiver, Finnhub, FRED, AAII, CNN F&G. **Out of scope:** Alpha Vantage (replaced by Polygon), Unusual Whales + Ortex (Stage 3+). **AUDIT-AS-SOURCE-OF-TRUTH:** API list per audit decisions takes precedence over PROJECT_PLAN section 10 when in conflict (owner Pass 52: "PROJECT_PLAN is outdated, heavily dependent on audit"). **Per-API audit schema (mandatory 6-dimension cross-reference per L136/#57):** (1) Subscription tier + cost + rate limits; (2) Available endpoints — every endpoint with PIT-safety flag, lookback range, free-in-tier flag, currently-used flag; (3) Currently consumed code references (file:function); (4) Gaps — endpoints in tier NOT consumed; **(5) Use-case cross-reference — for each endpoint: which strategies/agents/DEC-422 cube dimensions consume it (or could)? Map endpoints → 60+ strategies, ~11 agents, ~17 cube dimensions; (6) Caching & rate-limit feasibility for S&P 500 × 5-year scope (per DEC-411).** Each meaningful gap becomes candidate sub-decision (DEC-440+). **OpenBB consumption gap (PROJECT_PLAN says ✓, code shows zero):** dual treatment per Pass 52 owner approval — flag in API_AUDIT.md OpenBB section + sub-decision to either consume or remove from project scope (NOT a BUG; it's unresolved scope per audit-as-source-of-truth principle). **Deliverable:** new file `API_AUDIT.md` committed to repo. **Effort:** ~5-6 days (yfinance 0.5d, Polygon 1d greenfield, OpenBB 1d greenfield, Quiver 0.5-1d, Finnhub 0.5d, FRED 0.5d, AAII/CNN 0.5d). **Pre-condition gate before next backtest pass run.** Sub-decisions to be created per API after audit. NO additional cost — leverages existing tier only. | PENDING | Batch X56 — API Endpoint Utilization Audit (Pass 52) | Pass 52 | - |
