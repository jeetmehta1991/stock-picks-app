...
| **DECISION-070** | Portfolio-level exit logic — Pass 52 owner-approved DEFERRED_TO_STAGE_3 per L134/CHECKLIST #55 (system-design-level, not patch-level; cannot be batched with patches). Scope: portfolio-level drawdown trigger (>30% flatten), market-wide circuit breaker integration (DEC-385 L4), open position count cap (default 50). EXCLUDED from DEC-070 per Pass 52 owner clarifications: sector concentration (belongs in DEC-024 entry-side), strategy concentration (belongs in DEC-024). Sequencing: BUG-095 (CRITICAL OPEN, no portfolio state) → DEC-024 (entry-side concentration) → DEC-070 (exit-side portfolio logic). DEC-070 implementation deferred until Stage 3 prep theme (post Phase 0.B portfolio class). Status: DEFERRED_TO_STAGE_3. CAV-064 acknowledged (philosophy tension with PROJECT_PLAN section 12 buy-crisis-dips — Path A informational vs Path B hard limit owner direction needed at Stage 3 prep). | DEFERRED_TO_STAGE_3 | Batch X6 — Exits + Circuit Breakers | Pass 39 | DEC-422 |
...
| **DECISION-076** | Factor exposure breaker — Pass 52 owner-approved DEFERRED_TO_STAGE_3 per CHECKLIST #56 (focus-phase scope filter) + #55 (system-design-level, not patch). Portfolio-level breaker requires Phase 0.B Portfolio class (BUG-095 CRITICAL OPEN). Sequencing: BUG-095 → DEC-024 (entry-side concentration) → DEC-070 (exit-side portfolio drawdown trigger, also DEFERRED) → DEC-076 (factor-exposure granularity, more specific than DEC-070). Philosophy tension with PROJECT_PLAN section 12 buy-crisis-dips — owner direction needed at Stage 3 prep. Status: DEFERRED_TO_STAGE_3. | DEFERRED_TO_STAGE_3 | Batch X6 — Exits + Circuit Breakers | Pass 39 | - |
...
| **DECISION-258** | Options chain snapshot cache (OI + IV + put-call ratio) per ticker per day — Pass 52 owner-approved DEFER_TO_PHASE_1C. **Use-case map (per #57):** no current consumer in 60-strategy roster; `exit_volatility_regime` (DEC-067) uses VIX market + ticker realized vol (not implied); DEC-422 cube uses market-wide `vix_level_band` not per-ticker IV. Per #57 reuse-test: building options infrastructure without consumers is generic-template work. Re-evaluation gate: when/if options-flow strategies added in Phase 1C+ or Stage 3+ (Unusual Whales already Stage 3+ scope), revisit DEC-258. Status: DEFERRED_TO_PHASE_1C. | DEFERRED_TO_PHASE_1C | Batch X41 — Phase 0.A Data Prefetch Gaps | Pass 47 | - |
...
| **DECISION-395** | DEC-323 Phase 2 Polygon Reference / FactSet for full sector PIT — DEFERRED_TO_STAGE_3 per CHECKLIST #56 (sub-decision text already said "defer to Stage 3+"; status now formally aligned). Original scope preserved. Note: DEC-447 (Polygon reference PIT consumption pattern, just approved DEC-410 audit) may partially supersede if Polygon reference data covers sector PIT scope; verify during DEC-447 implementation. | DEFERRED_TO_STAGE_3 | Batch X53 — High-Impact Engine Bugs | Pass 52 | - |
...
| **DECISION-450** | Extend `prefetch_quiver.py` to 8+ unused endpoints — Pass 52 owner-approved (DEC-410 Batch 2, Hypothesis H-quiver-1). Add prefetch coverage for: `/historical/senatetrading/`, `/historical/housetrading/`, `/historical/lobbying/`, `/historical/wikipedia/`, `/historical/reddittraders/`, `/historical/twittersentiment/`, `/historical/offexchange/`, `/historical/appdownloads/`. Storage: `backtest/data/cache/quiver/{endpoint}/{TICKER}.parquet`. **Use-case map (#57):** Smart Money / Sentiment / Macro agents, sentiment_score cube dim feeder, regulatory tailwind dim potential, attention dim potential, institutional flow proxy. **Resolves:** BUG-190 (MEDIUM OPEN — Senate/Twitter/Off-Exchange/App Downloads not in prefetch). ~2-3 days. **Test signal:** non-empty parquet ≥80% S&P 500 per endpoint (or rate-limit-tier-realistic floor); cube dimensions populate from new endpoints. | PENDING | Batch X56 — API Endpoint Utilization Audit (Pass 52) | Pass 52 | - |
...
| **DECISION-452** | Quiver premium upgrade decision — Pass 52 owner-approved as conditional sub-decision (DEC-410 Batch 2, Hypothesis H-quiver-2). After DEC-450 (extended endpoints) lands and full S&P 500 prefetch is run, evaluate: did rate limits constrain prefetch? Did data quality suffer at free tier scale? If YES → upgrade to Quiver premium ($50-100/mo). If NO → free tier sufficient. **Use-case map (#57):** Quiver-dependent agents and cube dims (smart_money_score, sentiment_score). Effort: ~0.25d evaluation only (no implementation until decision). **Test signal:** post-DEC-450 prefetch metrics document rate-limit utilization, missing-data rate per endpoint; decision binary based on thresholds (e.g., >5% missing rate → upgrade). | PENDING | Batch X56 — API Endpoint Utilization Audit (Pass 52) | Pass 52 | - |
