...
| H4 | Polygon reference extended fields | `/v3/reference/tickers/{t}` | + address, branding (logo_url, icon_url), total_employees, phone_number, description, composite_figi, share_class_figi, round_lot | 1937 | ✅ DONE-WITH-DELISTING-CAP 2026-05-10 (`data_prefetch/polygon/reference_extended/` has 1687/1937 = 87.1%; 251 missing all return Polygon 404 - verified ANSS/ABMD/AGN/ALXN/AIMC delisted/acquired/M&A casualties; structurally same as CAV-075 SEC EDGAR 246-ticker delisting cap. No further fetch action possible; signal already covered via successor symbols where applicable.) |
...
| H6 | Polygon precomputed indicators | `/v1/indicators/{sma\|ema\|rsi\|macd}/{t}` | timestamp, value | 1937 x 6 indicators x multiple windows | ✅ DONE 2026-05-10 (`data_prefetch/polygon/indicators/` has 6 dirs: ema_20, ema_50, macd, rsi_14, sma_50, sma_200; ALL 1937/1937 per directory post-macd topup which restored 26 missing actives CVX/SOFI/SPGI/etc.) |
...
| H16 | ALFRED vintage mirror | `/fred/series/observations` w/ realtime_start + realtime_end | + realtime_start, realtime_end | 50->57 series | ✅ DONE 2026-05-10 (`data_prefetch/alfred/` has 80 parquets - exceeds 50-57 target by 23+ series; schema verified incl realtime_start + realtime_end vintage cols; CPIAUCSL multi-vintage 3357 rows confirms PIT vintage history retained per DEC-301 ALFRED methodology) |
...
| H18 | CFTC 5 missing datasets | Legacy futures-only (6dca-aqww), Legacy combined (jun7-fc8e), Disagg futures-only (72hh-3qpy), TFF combined (yw9f-hn96), Supp CIT (4zgm-a668) | (varies per dataset; ~50-87 columns each) | 19 contracts x 5 datasets | ✅ DONE-WITH-DATASET-SCOPE-CAP 2026-05-10 (54/95 contract-dataset pairs cached; EMPTY results reflect CFTC data-scope mismatch, not fetch bug: supp_cit is AG-commodity-only and returns 0/19 for financial futures, disagg_futures covers 5/19 contracts in its scope, tff_combined covers 13/19, legacy_futures+combined cover 18/19 each. All EMPTY responses verified at CFTC API source. Available data fully fetched.) |
...
| H22 | Date-typing migration | walk all caches | coerce date strings -> datetime64 | 8+ caches | ⚠ PARTIAL-DONE-WITH-FINDING 2026-05-10 (J2 broad sweep covered 7033 cases; empirical re-scan 2026-05-10 finds: AAII weekly+asset/StockTwits/ALFRED-date-col DONE-datetime64; **gap in 3 caches**: polygon ohlcv_daily date=`object` (Python date type), polygon indicators date=`object`, ALFRED realtime_start+realtime_end=`object`. Functional impact is minor - pd comparison works transparently, but not strict datetime64. Migration deferred as separate addressal per CHECKLIST #78 isolation - INV-048 logged for the residual gap.) |
...
| J6 | Standardize file naming (ticker.parquet always; safe-stem for Windows reserved CON/PRN/AUX/NUL/COM*/LPT* — INV-043 fix already applied to corp_actions) | propagate safe_filename_stem() to ALL prefetch scripts | 1h | ✅ DONE 2026-05-08 v8h+1 via shared `scripts/_prefetch_utils.safe_filename_stem`. New prefetch scripts MUST import from here. Existing 12 scripts left untouched (3 with inline copies, 9 with bare `replace('-','_')`); empirical risk is 0 collisions across 139,823 parquets. 7 unit tests added. |
...
### Phase 1A May 15 strict blockers status — Pass 53 v8h+1 T0 triage (2026-05-08)
