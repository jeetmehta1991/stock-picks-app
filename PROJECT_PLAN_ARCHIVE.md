# PROJECT_PLAN ARCHIVE — Pre-April-2026 Reference
**Purpose:** Historical preservation of the PROJECT_PLAN.md content before the April 2026 architectural rewrite.
**Why this exists:** The April 2026 rewrite consolidates all decisions through Pass 38 into a current-state focused PROJECT_PLAN.md. Per L94 (PROJECT_PLAN APPEND-ONLY discipline) and per owner instruction this turn ("Keep all that's relevant"), the pre-rewrite content is preserved here rather than deleted.
**Status:** Read-only historical reference. Do not edit. For current state see PROJECT_PLAN.md.
**Original size:** 2,415 lines spanning multiple revision cycles including the April 2026 RESTORED SECTIONS that recovered 789 lines from a prior rewrite incident (L94).
