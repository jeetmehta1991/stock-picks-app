...
>
> **B687 follow-on integration scaffold (this batch):** CLI runner [`scripts/run_conditional_information_diagnostic_on_strategies.py`](scripts/run_conditional_information_diagnostic_on_strategies.py) wires the diagnostic to a parquet gate-signal panel + forward-return column. Pre-configured `STRATEGY_GATES` for T3 (hull_rsi), T8 (ichimoku_cloud_breakout), W8 (cpr_narrow_bullish post-B654) per reviewer's "cheapest single action" recommendation. Usage: `--gate-panel <path>.parquet --return-col forward_return_5day --strategies hull_rsi ichimoku_cloud_breakout cpr_narrow_bullish --output <out>.json`, or `--cluster trend` / `--cluster pivot`. Currently FAILS-CLOSED with PENDING-B660 + PENDING-cube-replay message — runnable the moment B668 cube replay emits the gate-signal + forward-return panel.
>
