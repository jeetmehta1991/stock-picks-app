...
> **B739 DECISION 5 BANNER (2026-06-13) — full SM cluster Cat 1-6 sequence approved.** Owner approved Decision 5 walkthrough; 22 of 24 SM-cluster tickets are now `APPROVED-IN-SEQUENCE` in `EXECUTION_QUEUE.md`. Critical path: **B718b/c/d → B736 lint → B690 → B690b AWS run → B693 zero-fire diagnostic → Pattern F (13F + confluence wraps SM-31..SM-41 + congressional) + SMC Pattern J in parallel → B714 routing (delete/consolidate/retain) → docstring sweep on survivors → SM-1/SM-2 tuning (Pattern F + OOS-watchdog B734 satisfied) → borrow-cost source replacement + PRE-B671 short re-run → SM effective-N → C2 correction.** Cat 7 items shipped as B739.
>
> ---
>
> **B716 CONFIRMATION BANNER (2026-06-12) — B713 prediction empirically vindicated.** B689 re-run completed 2026-06-12 10:14 UTC ([fire_count_measured_b660_post_b689_extended.json](output_audit/fire_count_measured_b660_post_b689_extended.json)). **Smart-money cluster STILL measured 0 fires across all 38 SM-cluster-related strategies** — exactly as the B713 review predicted. B689 wired in ICT / SMC / chart-pattern / calendar producers but NOT SM-specific producers (13F, insider, congressional, Quiver). **B713 ticket inventory remains accurate; B690 producer wireup remains blocking for the entire SM cluster.** No B660 measurement-based dispositions will appear for SM strategies until B690 ships. See `S4-B716-SM-CLUSTER-B690-STILL-REQUIRED-B713-PREDICTION-CONFIRMED` in `EXECUTION_QUEUE.md`.
>
> ---
>
> **B713 STATUS BANNER (2026-06-12) — 4TH-PASS ADVERSARIAL REVIEW: 4 of reviewer's highest-impact claims source-verified at the line-number level.** Output: [STAGE_4_SMART_MONEY_CLUSTER_B713_ADVERSARIAL_REVIEW.md](STAGE_4_SMART_MONEY_CLUSTER_B713_ADVERSARIAL_REVIEW.md). **HIGHEST-URGENCY FINDING (REVIEWER 100% CORRECT, PHASE 0)**: the `inspect.currentframe` borrow guard verified LIVE at [screener.py:152-157, 218-224](backtest/signals/screener.py#L152) with the **exact fail-open behavior** the reviewer predicted (`if s is not None` -> if frame introspection fails, the guard silently does nothing and shorts fire unprotected; invisible at the call site). **The prior-round critique against this shipped while the objection sat queued.** Must be reverted to explicit `borrow_ok` gate + registration-time lint BEFORE B690 wires producers (at which point the guard becomes live financial risk). **OTHER VERIFIED FINDINGS**: (a) DTC threshold `>8.0` at [screener.py:104](backtest/signals/screener.py#L104) misses the canonical GME pre-squeeze range 5-7 (reviewer correct — threshold direction is wrong relative to stated motivation); (b) B660 measured **40 of 40 pure-SM strategies = 0 fires** (harness gap; won't resolve until B690 300-500 LOC refactor); (c) every pre-B671 short-backtest ACROSS ALL CLUSTERS was borrow-unprotected -- **cross-cluster blocking** invalidation that the doc scoped too narrowly as SM-5-local. **Win-rate tuning premature for this cluster** per [feedback_no_prior_edge_consolidate_before_tune](C:/Users/jeetm/.claude/projects/c--Users-jeetm-Github-stock-picks-app/memory/feedback_no_prior_edge_consolidate_before_tune.md); SM-1/SM-2 insider cluster-buying are the only grounded survivors (Lakonishok-Lee anchor + real levers: 10b5-1 filter, role weighting, cluster size). **14 B713 tickets queued across 6 phases.** Phase 0 (inspect.currentframe revert) is single-highest-urgency item.
>
> ---
>
> **B693 BANNER ADDENDUM (2026-06-11) — selective-reading correction.** External reviewer caught B691's selective-reading methodology problem. **The PENDING-B690 disposition for the 44/44 FAIL still stands** (TIER 2 producer wire-in is the documented blocker), but the "100% false negative" claim now requires verification per-strategy via [`scripts/diagnose_zero_fires.py`](scripts/diagnose_zero_fires.py) once B690 re-run completes — not assumed. Plus: the cluster's 44 strategies share the same structural issue as the breakout cluster's BR-7/BR-8 (Pattern N redundancy). The reviewer's framework applies: even after B690 wires the smart-money producers and the cluster fires honestly, the **outcome-correlation matrix (not primitive count)** is the right measure of effective hypothesis count for multiple-testing correction. The 44 smart-money strategies on shared insider/13F/SEC primitives are not 44 independent bets; the effective N is much lower. Queued: `S4-B693-SMART-MONEY-OUTCOME-CORRELATION-NOT-PRIMITIVE-COUNT`.
>
> ---
>
...
## B673 Cross-Cutting Feasibility Findings (External Reviewer 2026-06-10 — 2nd-Wave Critique)

> **Status banner (2026-06-10):** external reviewer 2nd-wave critique on the post-B672 smart-money cluster doc shifted the critique axis from "is the code honest about the signal" (the doc now is) to **"can the engine actually capture the alpha the signal points at, given how these filings reach the market and how the entry mechanism works."** Seven cross-cutting feasibility findings (CC1-CC7) + several per-strategy reframings. **Producer-code verification performed on the 4 most-consequential claims; 3 confirmed against current code, 1 confirmed against producer's own annotation.** These findings are CRITICAL because they identify failure modes the prior reviewer rounds (which focused on signal honesty + temporality) did not surface.

### Cross-cutting feasibility findings (severity-ordered)

| # | Finding | Verification | Severity | Status |
|---|---|---|---|---|
| **CC1** | EVENT-alpha uncapturable via next-day-open after a gap. M&A targets gap 20-40% on announcement; 13D filings pop on filing; insider Form 4 + 13D announcements gap before the engine's daily-bar next-open entry can fire. The academic CARs cited (Bradley-Desai-Kim ~30% premium, Brav ~7% activist abnormal return, Lakonishok-Lee insider) are measured from PRE-event price or announcement bar — NOT from "next-day-open after detection." A daily engine systematically buys AFTER the gap, capturing residual drift (a fraction of the headline number, sometimes wrong-signed via post-gap fade). | ✅ Inherent to engine design (daily bar + next-open entry); no producer fix possible without intraday data | **HIGH** | NEW ticket queued |
| **CC2** | 13F contaminated by passive/index flows. Largest 13F filers (Vanguard, BlackRock, State Street) hold everything mechanically via index funds; "institutional_increased" is partly mechanical price-chasing (price up → index weight up → passive buy → next 13F shows "institutional increased"). Strategy gate is correlated with the trend gate THROUGH the same past return — possibly circular. | ✅ Verified via `grep "index_fund\|passive\|index_filer" backtest/data/smart_money.py` → 0 hits. **No active-manager filtering in producer.** | **HIGH** | NEW ticket queued |
| **CC3** | Confidential treatment structurally hides the best institutional accumulation. SEC rules let managers request confidential treatment to delay disclosure (Berkshire does this routinely on new large positions). The observed 13F is biased toward what institutions DON'T mind revealing (established, already-public, already-appreciated holdings) and AWAY from the fresh high-conviction accumulation that would actually carry alpha. | ✅ Inherent SEC rule (Securities Act §13(f)(3) — no producer fix possible) | **HIGH** | NEW ticket — STRUCTURAL CEILING documented |
| **CC4** | Quiver vendor PIT/restatement integrity unaudited. Three hazards: (i) 13F amendments (13F-A) — if feed serves amended holdings at original date → lookahead; (ii) Insider Form 4 corrections / late filings — stamping to transaction date vs filing date matters; (iii) Backfill — vendors backfill history when fixing parsers, giving backtest data that wasn't actually available. | ✅ Verified via grep — no amendment or backfill handling visible in `smart_money.py` | **HIGH** | NEW ticket queued — gates the cluster like B660 gates fire counts |
| **CC5** | 10b5-1 contamination dilutes insider signal. Post-2023 Form 4 has a 10b5-1 checkbox; scheduled plan purchases are not informed (pre-committed months earlier). If producer counts all P-code transactions without filtering the 10b5-1 flag, cluster-buy signal is diluted with scheduled trades. | ✅ Verified via `grep "10b5\|TransactionCode" backtest/data/smart_money.py` → producer filters codes ('P', 'L') correctly BUT **no 10b5-1 flag filtering**. CC5 confirmed gap. | **HIGH** | NEW ticket queued |
| **CC6** | Cited alphas are pre-crowding decay. Cohen-Frazzini-Malloy 2008, Lakonishok-Lee, Ziobrowski congressional studies were measured BEFORE retail 13F/Pelosi-tracker apps + funds arbitraged the edges. Citing 2008 RFS alpha to justify a 2026 strategy implies a magnitude the market has likely competed down. Magnitude analog of the Pattern B timing-attribution overclaim. | ✅ Citation discipline meta-issue; no producer fix | **MEDIUM** | Pattern B disposition extension — magnitudes are now overclaim AND timing was overclaim |
| **CC7** | Effective hypothesis count ≈ 4, not 41. Stripping near-constant smart-money gates (Pattern F's premise) the cluster collapses to: 1 13F-state factor + 1 insider-EVENT factor + 1 13D/M&A event factor + 1 alt-data bucket = 4 underlying signals reskinned 41 ways. For C2 multiple-testing correction this matters: the family-wise / FDR math should treat correlated reskins as near-duplicate hypotheses (effective N closer to 4 than 41), or the correction is simultaneously too lenient AND per-strategy Sharpes are near-identical. | ✅ Inherent to the cluster's structure (per the cluster scope inventory itself) | **HIGH** | NEW ticket queued — extends Pattern H (cross-cluster dedup) to WITHIN-cluster redundancy |

### Per-strategy feasibility reframings

| Strategy | Prior disposition | Reviewer feasibility critique | Action |
|---|---|---|---|
| **SM-4** `strat_ma_target_long` | Walked clean (EVENT ✅ + PASS fire-count) per the prior doc + B672 expansion | **FEASIBILITY FAILURE, not docstring fix.** M&A target: press release → stock gaps 20-40% to just below deal price → 8-K Item 1.01 files within 4 days → strategy detects "8-K Item 1.01 within 30d" → engine enters next-day-open. **You are buying a target that already gapped weeks ago, now trading at 2-5% merger-arb spread, with capped upside (the spread) and full downside if deal breaks (-25%+).** This is naked short-volatility merger-arb with terrible risk/reward, NOT the +2-3pp CAR the literature cites. Cited Bradley-Desai-Kim ~30% premium IS the announcement-day gap you already missed. The new acquirer/target disambiguation finding (B672 walk) is real but secondary — even pure target-side signal is uncapturable via next-open entry. **PIT trap on disambiguation:** target/acquirer determination may not be cleanly determinable from filing alone in real time; resolving via "cross-reference with counterparty's filing" risks using later-arrived data to label the bar retroactively. | NEW ticket queued — SM-4 should be flagged as FEASIBILITY FAILURE class; pre-announcement signals (unusual options, rumor, accumulation) are a different + harder strategy |
| **SM-5** `strat_short_borrow_trap_avoid` | B671 RESOLVED — centralized inspect.currentframe gate + DTC 5→8 tighten | **Two compounding issues:** (i) **bi-monthly FINRA cadence + ~14-day stale data.** ✅ Verified in producer at [screener.py:3825](backtest/signals/screener.py#L3825) — comment reads `days_to_cover (FINRA bi-monthly, ~14d stale)`. In a real squeeze DTC explodes over DAYS; the guard reads a number from BEFORE the squeeze started. **Structurally too slow as a squeeze circuit-breaker; works only as a baseline borrow filter.** (ii) **Pre-B671 short-strategy backtest contamination:** the doc's B670 finding that SM-5 was an orphan emitter (engine dropped avoid output) means EVERY short-strategy backtest run before B671 had ZERO borrow protection. Any historical SHORT Sharpe in the system was computed without the guard now considered essential. Those backtests need RE-RUNNING, not just forward-protected. | NEW ticket queued — SM-5 framing should be "baseline borrow filter only, not squeeze circuit-breaker"; historical short-Sharpe re-computation gate |
| **SM-18, SM-19** Pattern-F EXEMPT (multi-quarter persistence precompute) | Prior doc EXEMPTED these from Pattern F audit on grounds of "genuine STATE not Pattern B overclaim" | **EXEMPTION INVERTS THE LOGIC.** A position institutions have increased for 5 straight quarters is BY CONSTRUCTION the most stale + least timing-informative version of 13F — it's a large, stable, already-appreciated holding. Genuine-STATE doesn't rescue it; it indicts it. SM-18/SM-19 should be the FIRST into Pattern F audit, not exempt. The exempt-because-honest-STATE reasoning conflates honesty with usefulness — a 5-quarter persistent holding is just "this is a big stable stock," precisely the no-marginal-contribution case Pattern F is designed to find. | NEW ticket queued — Pattern F audit scope EXPANDED to include SM-18 + SM-19 (highest-priority specimens, not exempt) |
| **SM-10, SM-15** (RSI-oversold 13F variants) | Walked clean | RSI-mean-reversion strategies wearing a 13F badge. RSI<35/<40 + above-200-EMA + 13F-eligibility = buy-the-dip-in-an-uptrend. **The 13F gate's marginal contribution is almost certainly near-zero at bar of fire; the edge (if any) is the RSI mean-reversion**, which is itself thin + cost-sensitive in liquid names. Per CC2 + CC6, these will likely FAIL Pattern F. | Cross-ref Pattern F priority |
| **SM-8, SM-14, SM-16, SM-17, SM-27, SM-28** (MACD/vol-spike 13F variants) | Walked clean | **Momentum-on-momentum:** 13F gate is partly momentum echo (CC2); MACD/vol-spike gate is also momentum. Strategies double-count the same factor through correlated gates. Pattern F will show 13F gate adds ~nothing the MACD/vol gate doesn't already capture. | Cross-ref Pattern F priority |
| **Sub-cluster E (10 confluence wraps SM-31..SM-41)** | Pattern E bullet-text reframe (held pending B660) | **UNION-gate on always-somewhat-present signals ≈ always-true on liquid universe** (some institution holds it, some insider traded recently, some congressional filing exists). Wrap ≈ base strategy + near-no-op overlay. UNLIKE an AND-gate (which throttles fires visibly), UNION wraps barely change fire-count or selection. **Real question is whether these 10 wraps should EXIST as registry slots at all, or are they 10 duplicates inflating C2 family size (CC7).** Bullet-text reframe makes them honestly-described; doesn't address whether they should exist. | Pattern E disposition EXPANDED to include "do they justify registry slots" question; cube ablation against base strategies (52w_high_breakout, rsi_oversold, etc.) needed |
| **SM-1, SM-2** (insider cluster long — the cluster's "crown jewels") | B663 closed | Treated correctly as foundational but **edge is small-cap concentrated**, where engine costs/slippage/borrow are worst + survivorship is most severe. Small-caps that went to zero after insiders bought (insiders are wrong plenty, especially in distress) are MISSING from a survivor universe → backtest overstates. Plus CC1 (next-open misses pop, milder than M&A) + CC5 (10b5-1 dilution) + CC4 (PIT integrity) all compound. **Real edge thin after small-cap costs; needs 10b5-1 filter + delisting-inclusive universe to be trusted.** | Cross-ref existing C5 survivorship + new 10b5-1 ticket |
| **SM-3** (activist 13D long) | Walked clean | Brav-style ~7% activist abnormal return is **front-run by construction** — activist has been buying for the 10-day (now 5-business-day post-Feb-2024) window BEFORE filing; pop happens ON filing. Daily next-open entry catches residual drift, not event. **The 2024 acceleration of 13D deadline (10 calendar → 5 business days) is a REGIME CHANGE in the data source** that any pre-2024 backtest won't reflect. | NEW ticket queued — 13D-window-regime-change-2024 |
| **Edge cases that fire systematically (not rarely)** | — | (a) Index reconstitution (Russell/S&P additions force passive 13F-"increase"); (b) 13F amendments restating to original dates → lookahead (CC4); (c) Insider option-exercise-and-hold vs open-market — codes M/A vs P; producer filters to ('P','L') ✅ verified; (d) Deal-break on M&A targets — full gap-down (SM-4 left tail); (e) Dual-class/ADR ticker-mapping errors; (f) Spinoffs creating phantom distribution-then-accumulation; (g) Confidential-treatment EXPIRY fires "recent_init" strategies on stale months-old accumulation. | Multiple NEW tickets queued |

### Net effect on cluster disposition

- **Pattern F audit (existing ticket `S5-13F-SLEEVE-MARGINAL-CONTRIBUTION-TEST`) EXPANDED in scope:** must now test (i) active-manager-only 13F vs all-13F (not just gate ON vs OFF) per CC2; (ii) include SM-18 + SM-19 (formerly exempt) per inverted-exemption-logic finding; (iii) treat reskins as near-duplicates per CC7; (iv) gated on Quiver PIT integrity audit per CC4
- **Pattern B disposition EXTENDED:** magnitude overclaim (CC6) joins the timing overclaim that Pattern B already addressed. Docstring sweep now needs to strip BOTH the bar-of-fire timing implication AND the pre-crowding magnitude implication
- **EVENT strategies (SM-1/2/3/4) need a "capturable-after-gap" haircut on cited expected edges per CC1** — no producer fix possible without intraday data; documentation honesty fix
- **SM-4 RECLASSIFIED** from "walked clean" to "feasibility failure under daily-bar-next-open entry mechanism" — if M&A alpha is desired, requires pre-announcement signals (different + harder strategy)
- **SM-5 RECLASSIFIED** from "borrow-trap circuit-breaker" to "baseline borrow filter only" — bi-monthly FINRA cadence makes squeeze-detection structurally impossible; documentation honesty fix
- **Pre-B671 SHORT-strategy backtests CONTAMINATED** — historical Sharpes computed without borrow protection (orphan emitter) must be re-run after B671 wiring takes effect to be trusted
- **6 NEW EXECUTION_QUEUE tickets surfaced** (see B673 update in EXECUTION_QUEUE.md)

### Architectural concerns from prior round CARRIED FORWARD

The reviewer's prior-round critique of the **B671 SM-5 `inspect.currentframe()` centralized gate** as "load-bearing risk control on a language-non-guaranteed stack-introspection mechanism, in the most-traversed code path, invisible at the call site" remains UNRESOLVED. Recommendation (revert to explicit named gate + registration-time lint) is queued as a SEPARATE owner-decision ticket pending the doc's prior-round response. Reviewer-noted compounding issue from this round: even WITH the centralized gate, the bi-monthly FINRA cadence (CC5/SM-5) means the guard cannot serve its stated purpose (squeeze circuit-breaker) — so the architectural risk of inspect.currentframe is being incurred for a guard that is structurally too slow for the most dangerous case anyway.
...
| **Smart Money (THIS DOC)** | **[STAGE_4_SMART_MONEY_CLUSTER_WALKS.md](STAGE_4_SMART_MONEY_CLUSTER_WALKS.md)** | **41** | **✅ 2 rounds (B669 + B673 → B674 incorporated)** | **(already iterated; 12 NEW EXECUTION_QUEUE tickets pending owner decisions)** |
