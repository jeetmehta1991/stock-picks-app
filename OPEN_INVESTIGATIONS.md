...
- **Status:** **RESOLVED-IMPLEMENTED B1009 2026-06-22** (Council 103 Option-6 SHIP-S5+S4 owner-approved 'Approve all proceed council this'). Fix: `exit_strategies.py:507` now passes `as_of=entry_date` to `fetch_earnings_dates`. Verified via 4 unit tests in `backtest/tests/test_b1009_inv_057_058_earnings_blackout_pit.py` (test_b1009_inv_057_fetch_earnings_dates_respects_as_of_when_set + test_b1009_inv_057_exit_earnings_blackout_passes_as_of_entry_date). Pyramid GREEN 852+2. Cube re-measurement remains owner-pre-approval-gated per CHECKLIST #13 expensive-job protocol.
...
- **Status:** **RESOLVED-IMPLEMENTED B1009 2026-06-22** (Council 103 Option-6 SHIP-S5+S4 owner-approved 'Approve all proceed council this'). Fix: `fetcher.py:255` now uses `end_date + 30 days` proxy (Option-d per B995/B998/B999 finalized). Honest proxy-approximation acknowledgment in producer docstring. Filing_date fallback retained if end_date absent. Verified via 2 unit tests in `test_b1009_inv_057_058_earnings_blackout_pit.py` (test_b1009_inv_058_end_date_proxy_used + test_b1009_inv_058_fetcher_source_uses_end_date_proxy). Pyramid GREEN 852+2. Cube re-measurement remains owner-pre-approval-gated per CHECKLIST #13 expensive-job protocol.
