...
- **Status:** OPEN — 5 walk-2 strategies + any other earnings_blackout consumers pending dedicated fix batch. **B1001 (2026-06-22) UPDATE:** B998 + B999 investigations completed pre-flight TODOs; Option-d (`end_date + 30 days` proxy) finalized as B996 fix scope (per B995 prep doc). Fix-batch readiness package complete; awaiting explicit owner approval of S5-EARNINGS-BLACKOUT-LOOKAHEAD-FIX-BATCH ticket.
...
- **Status:** OPEN — bundled with INV-057 fix batch. **B1001 (2026-06-22) UPDATE:** B998 verified Polygon parquet has NO `announce_date` or `earnings_release_date` column (only `filing_date` / `end_date` / `period_of_report_date` (NULL) / `start_date`); B999 verified Finnhub `/stock/earnings` endpoint has NO `announce_date` field either (only `period` = fiscal-end-date + actual/estimate/surprise). Option-c (Finnhub fallback) NOT VIABLE per local cache. Option-d (`end_date + 30 days` proxy) finalized as B996 fix; cleanest available; matches empirical filing-gap median ~32 days across 8 sampled tickers (AAPL 31d / MSFT 27d / GOOGL 29d / AMZN 29d / TSLA 35d / JPM 39d / WMT 39d / JNJ 36d). Honest acknowledgment of proxy-approximation required in producer docstring. B995 fix-batch readiness package updated per B998/B999 findings.
