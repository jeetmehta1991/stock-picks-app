...
A separate analysis asked whether a strategy does better with an exit that VARIES by the regime it entered in, versus one exit everywhere. Originally 17 strategies showed a gain; after net-of-cost + winsorization (B1377) **13 survived** with an out-of-sample DeltaSharpe >= 0.3 (IS-pick 2022-2025 / OOS-measure 2025-2026).

**But none of them reached the promoted 22.** Of the 13 conditional survivors, 13 rows landed DROP and 1 landed PASS-noFDR under the true-holdout grading. So **every one of the 22 promoted cells uses a SINGLE exit across all regimes** - the `Cond` column in Table A is `N` for all of them.

**Why both results are true at once:** the conditional test is RELATIVE - *does varying the exit beat this strategy's own single best exit?* The holdout gate is ABSOLUTE - *is the resulting Sharpe >= 0.5?* A strategy can improve on itself by varying its exit and still sit below the absolute bar. That is exactly what happened: 13 improved relative to themselves; none cleared 0.5 out-of-sample.

| Regime-conditional survivor | OOS DeltaSharpe | Holdout verdict |
|---|---|---|
| `turtle_soup_long` | +1.02 | long=DROP |
| `supertrend_macd_short` | +0.96 | short=DROP |
| `smc_fvg_retest_short` | +0.88 | short=DROP |
| `52w_low_breakdown_pullback_short` | +0.66 | short=DROP |
| `hammer_at_support_long` | +0.62 | long=DROP |
| `week_opening_gap_fill_up` | +0.62 | long=DROP |
| `pead_short_negative_yoy_growth` | +0.49 | short=DROP |
| `squeeze_breakout_with_smart_money_long` | +0.48 | long=DROP |
| `52w_high_breakout_with_smart_money_vol_below_long` | +0.38 | long=DROP |
| `double_bottom_long` | +0.37 | long=DROP |
| `smc_equal_lows_sweep_long` | +0.35 | long=DROP |
| `roc_burst` | +0.31 | long=PASS-noFDR; short=DROP |
| `risk_off_bond_equity_short` | +0.30 | short=DROP |

**Consequence for deployment:** no per-regime exit switching is required for the promoted set. Each strategy carries one exit. Per-regime EVIDENCE still varies by cell - see the 'Regimes with holdout evidence' column in Table A, which shows where each cell actually has n >= 30 to stand on.
...
# FINAL PHASE 1B-ALPHA STRATEGY LIST - all 37 cells, one table

Every cell that goes forward, with every field this document holds. **22 carry forward evidence; 15 are directive-added short mirrors that do not.** Sort order: evidenced first (by holdout Sharpe), then measured mirrors, then inherited-exit mirrors.

**Column key** - `Class`: EVIDENCED = cleared the holdout gate; MIRROR (measured) = short mirror that exists in the cube and FAILED its own holdout; MIRROR (inherited) = short mirror never backtested, exit taken from its long parent. `Canon` = canonical `PASSING_CRITERIA` gates cleared out of 6 well-specified ones (Sharpe, profit factor, Sortino, Calmar, PSR, min_trades); `-` = not computed (mirrors). `R:R` = win rate >= 0.5 AND payoff >= 1.5. `Cond` = regime-conditional exit (all N - see section E). `EXPL` = registered EXPLORATORY. Full `fires` expressions in the Appendix.

| # | Strategy | Dir | Class | Category | Exit | Exit source | Verdict | Holdout Sharpe (n) | 95% CI lo | WR | Payoff | R:R | >=0.7 | Canon | Cond | EXPL | Regimes with holdout evidence | Entry gate |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | `avwap_252_breakout` | long | EVIDENCED | vwap | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.532 (314) | 0.246 | 0.363 | 3.6 | no | no | 5/6 | N | - | **bull 0.474**(n=254); bear n=26<30; **neutral 0.77**(n=34) | avwap, rsi, volume_gaps |
| 2 | `break_retest_confluence` | long | EVIDENCED | confluence | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.52 (143) | 0.134 | 0.315 | 5.09 | no | no | 4/6 | N | - | **bull 0.523**(n=131); bear n=3<30; neutral n=9<30 | close_pattern, macd, moving_avgs_trend, volume_gaps |
| 3 | `bullish_engulfing_support` | long | EVIDENCED | candle | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.725 (83) | 0.167 | 0.301 | 7.22 | no | YES | 4/6 | N | - | **bull 0.716**(n=67); bear n=6<30; neutral n=10<30 | candles, money_flow, pivots |
| 4 | `cpr_narrow_bullish` | long | EVIDENCED | pivot | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.568 (114) | 0.157 | 0.377 | 4.51 | no | no | 4/6 | N | - | **bull 0.485**(n=88); bear n=9<30; neutral n=17<30 | avwap, moving_avgs_trend, pivots |
| 5 | `force_index_breakout` | long | EVIDENCED | breakout | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.519 (259) | 0.181 | 0.309 | 5.42 | no | no | 5/6 | N | - | **bull 0.265**(n=191); **bear 1.468**(n=32); **neutral 0.497**(n=36) | close_pattern, money_flow, moving_avgs_trend |
| 6 | `institutional_committed_growth_long` | long | EVIDENCED | institutional_persistence | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.638 (666) | 0.437 | 0.383 | 4.54 | no | no | 5/6 | N | - | **bull 0.514**(n=540); **bear 1.535**(n=60); **neutral 0.573**(n=66) | institutional_13f, moving_avgs_trend |
| 7 | `institutional_high_conviction_long` | long | EVIDENCED | smart_money_13f | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.561 (738) | 0.377 | 0.328 | 5.44 | no | no | 5/6 | N | - | **bull 0.516**(n=618); **bear 1.389**(n=34); **neutral 0.352**(n=86) | institutional_13f, moving_avgs_trend |
| 8 | `institutional_persistence_breakout_long` | long | EVIDENCED | institutional_persistence | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.683 (136) | 0.317 | 0.404 | 6.0 | no | no | 6/6 | N | - | **bull 0.58**(n=110); bear n=8<30; neutral n=18<30 | institutional_13f, moving_avgs_trend |
| 9 | `institutional_persistence_oversold_long` | long | EVIDENCED | institutional_persistence | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.517 (199) | 0.14 | 0.357 | 3.49 | no | no | 5/6 | N | - | **bull 0.358**(n=160); bear n=26<30; neutral n=13<30 | institutional_13f, moving_avgs_trend, rsi |
| 10 | `macd_fast_crossover` | long | EVIDENCED | momentum | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.537 (378) | 0.264 | 0.333 | 4.8 | no | no | 5/6 | N | - | **bull 0.415**(n=284); **bear 0.983**(n=59); **neutral 0.728**(n=35) | macd |
| 11 | `mfi_oversold_with_smart_money_long` | long | EVIDENCED | smart_money_sleeve | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.839 (71) | 0.113 | 0.366 | 4.45 | no | YES | 4/6 | N | - | **bull 0.676**(n=53); bear n=11<30; neutral n=7<30 | money_flow, moving_avgs_trend |
| 12 | `news_sentiment_long` | long | EVIDENCED | news_sentiment | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.635 (173) | 0.214 | 0.353 | 4.97 | no | no | 4/6 | N | - | **bull 0.42**(n=140); bear n=19<30; neutral n=14<30 | moving_avgs_trend, news_sentiment |
| 13 | `pead_long_high_yoy_growth_only` | long | EVIDENCED | event_driven | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.585 (422) | 0.361 | 0.308 | 7.05 | no | no | 5/6 | N | - | **bull 0.557**(n=354); bear n=26<30; **neutral 0.4**(n=42) | pead_earnings |
| 14 | `poc_magnet_long` | long | EVIDENCED | volume_profile | `time_stop_10d` | holdout-picked | **PASS** | 0.808 (151) | 0.134 | 0.603 | 1.14 | no | YES | 5/6 | N | - | **bull 0.396**(n=125); bear n=11<30; neutral n=15<30 | moving_avgs_trend, volume_profile |
| 15 | `r1_break_retest` | long | EVIDENCED | pivot | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.501 (338) | 0.222 | 0.299 | 4.95 | no | no | 5/6 | N | - | **bull 0.33**(n=272); bear n=26<30; **neutral 1.088**(n=40) | avwap, close_pattern, macd, pivots, volume_gaps |
| 16 | `rsi_oversold` | long | EVIDENCED | mean_reversion | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.596 (293) | 0.327 | 0.321 | 5.92 | no | no | 4/6 | N | - | **bull 0.612**(n=278); bear n=4<30; neutral n=11<30 | moving_avgs_trend, rsi |
| 17 | `rsi_volume_200ema` | long | EVIDENCED | confluence | `earnings_blackout` | holdout-picked | **PASS** | 0.545 (63) | 0.062 | 0.635 | 1.4 | no | no | 4/6 | N | - | **bull 0.423**(n=56); bear n=1<30; neutral n=6<30 | moving_avgs_trend, rsi, volume_gaps |
| 18 | `smc_breaker_block_long` | long | EVIDENCED | smc | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.693 (356) | 0.408 | 0.393 | 4.77 | no | no | 6/6 | N | - | **bull 0.479**(n=283); **bear 1.883**(n=35); **neutral 0.827**(n=38) | moving_avgs_trend, smc_ict |
| 19 | `smc_inverse_fvg` | long | EVIDENCED | smc | `regime_flip` | holdout-picked | **PASS** | 0.813 (92) | 0.193 | 0.587 | 2.01 | YES | YES | 5/6 | N | - | **bull 0.242**(n=69); bear n=18<30; neutral n=5<30 | money_flow, moving_avgs_trend, smc_ict, volume_gaps |
| 20 | `totm_long` | long | EVIDENCED | calendar | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.963 (86) | 0.291 | 0.314 | 10.27 | no | YES | 4/6 | N | - | **bull 0.45**(n=72); bear n=14<30 | calendar, moving_avgs_trend |
| 21 | `xs_combined_momentum_low_ivol` | long | EVIDENCED | factor | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.962 (35) | 0.188 | 0.371 | 9.73 | no | YES | 5/6 | N | - | bull n=27<30; bear n=7<30; neutral n=1<30 | cross_sectional_factor, moving_avgs_trend |
| 22 | `xs_momentum_with_smart_money_long` | long | EVIDENCED | smart_money_sleeve | `breakeven_plus_trail` | holdout-picked | **PASS** | 0.952 (162) | 0.503 | 0.457 | 5.04 | no | YES | 6/6 | N | - | **bull 0.6**(n=126); bear n=24<30; neutral n=12<30 | cross_sectional_factor, moving_avgs_trend |
| 23 | `avwap_252_breakout` | short | MIRROR (measured) | vwap | `hybrid_50pct_target` | own holdout-picked (mirror of avwap_252_breakout) | **DROP** | -0.565 (154) | -0.932 | 0.422 | 0.79 | no | no | - | N | - | **bull -0.364**(n=136); bear n=10<30; neutral n=8<30 | avwap, rsi, volume_gaps |
| 24 | `break_retest_confluence` | short | MIRROR (measured) | confluence | `breakeven_plus_trail` | own holdout-picked (mirror of break_retest_confluence) | **DROP** | -0.24 (296) | -0.55 | 0.203 | 2.96 | no | no | - | N | - | **bull -0.076**(n=210); **bear -1.871**(n=40); **neutral -0.394**(n=46) | close_pattern, macd, moving_avgs_trend, volume_gaps |
| 25 | `bullish_engulfing_support` | short | MIRROR (measured) | candle | `breakeven_plus_trail` | own holdout-picked (mirror of bullish_engulfing_support) | **DROP** | -0.038 (152) | -0.476 | 0.224 | 3.29 | no | no | - | N | - | **bull -0.335**(n=123); bear n=9<30; neutral n=20<30 | candles, money_flow, pivots |
| 26 | `cpr_narrow_bullish` | short | MIRROR (measured) | pivot | `breakeven_plus_trail` | own holdout-picked (mirror of cpr_narrow_bullish) | **DROP** | -0.505 (153) | -0.989 | 0.216 | 2.19 | no | no | - | N | - | **bull -0.239**(n=123); bear n=11<30; neutral n=19<30 | avwap, moving_avgs_trend, pivots |
| 27 | `force_index_breakout` | short | MIRROR (measured) | breakout | `hybrid_50pct_target` | own holdout-picked (mirror of force_index_breakout) | **DROP** | -0.506 (417) | -0.711 | 0.448 | 0.72 | no | no | - | N | - | **bull -0.373**(n=347); bear n=29<30; **neutral -0.852**(n=41) | close_pattern, money_flow, moving_avgs_trend |
| 28 | `macd_fast_crossover` | short | MIRROR (measured) | momentum | `breakeven_plus_trail` | own holdout-picked (mirror of macd_fast_crossover) | **DROP** | -0.201 (589) | -0.443 | 0.236 | 2.63 | no | no | - | N | - | **bull -0.084**(n=538); bear n=22<30; neutral n=29<30 | macd |
| 29 | `pead_short_negative_yoy_growth` | short | MIRROR (measured) | event_driven | `class_time_stop` | own holdout-picked (mirror of pead_long_high_yoy_growth_only) | **DROP** | -0.847 (496) | -1.068 | 0.369 | 0.63 | no | no | - | Y | - | **bull -0.825**(n=432); bear n=14<30; **neutral -0.814**(n=50) | pead_earnings |
| 30 | `r1_break_retest` | short | MIRROR (measured) | pivot | `breakeven_plus_trail` | own holdout-picked (mirror of r1_break_retest) | **DROP** | -0.266 (398) | -0.55 | 0.221 | 2.63 | no | no | - | N | - | **bull -0.177**(n=341); bear n=20<30; **neutral -1.044**(n=37) | avwap, close_pattern, macd, pivots, volume_gaps |
| 31 | `rsi_oversold` | short | MIRROR (measured) | mean_reversion | `breakeven_plus_trail` | own holdout-picked (mirror of rsi_oversold) | **DROP** | 0.079 (169) | -0.358 | 0.29 | 2.69 | no | no | - | N | - | **bull 0.358**(n=116); bear n=20<30; **neutral -1.672**(n=33) | moving_avgs_trend, rsi |
| 32 | `rsi_volume_200ema` | short | MIRROR (measured) | confluence | `earnings_blackout` | own holdout-picked (mirror of rsi_volume_200ema) | **DROP** | -0.149 (74) | -0.559 | 0.473 | 0.9 | no | no | - | N | - | **bull -0.145**(n=59); bear n=1<30; neutral n=14<30 | moving_avgs_trend, rsi, volume_gaps |
| 33 | `smc_breaker_block_short` | short | MIRROR (measured) | smc | `atr_trail_2x` | own holdout-picked (mirror of smc_breaker_block_long) | **DROP** | -0.7 (427) | -1.051 | 0.316 | 1.33 | no | no | - | N | - | **bull -0.507**(n=358); bear n=27<30; **neutral -1.098**(n=42) | moving_avgs_trend, smc_ict |
| 34 | `smc_inverse_fvg` | short | MIRROR (measured) | smc | `breakeven_plus_trail` | own holdout-picked (mirror of smc_inverse_fvg) | **DROP** | -0.234 (139) | -0.709 | 0.317 | 1.72 | no | no | - | N | - | **bull 0.132**(n=106); bear n=10<30; neutral n=23<30 | money_flow, moving_avgs_trend, smc_ict, volume_gaps |
| 35 | `news_sentiment_short` | short | MIRROR (inherited) | news_sentiment | `breakeven_plus_trail` | INHERITED from news_sentiment_long | **NO DATA** | never backtested | - | - | - | - | - | - | N | Y | none - never backtested | moving_avgs_trend, news_sentiment |
| 36 | `poc_magnet_short` | short | MIRROR (inherited) | volume_profile | `time_stop_10d` | INHERITED from poc_magnet_long | **NO DATA** | never backtested | - | - | - | - | - | - | N | Y | none - never backtested | moving_avgs_trend, volume_profile |
| 37 | `xs_combined_momentum_high_ivol_short` | short | MIRROR (inherited) | factor | `breakeven_plus_trail` | INHERITED from xs_combined_momentum_low_ivol | **NO DATA** | never backtested | - | - | - | - | - | - | N | Y | none - never backtested | cross_sectional_factor, moving_avgs_trend |

**Totals:** 22 evidenced long + 12 measured mirrors + 3 inherited-exit mirrors = **37 cells**. Of the 22 evidenced, 3 clear all six canonical gates; every mirror in this table is EXPLORATORY and carries either negative or no holdout evidence.


## Appendix - entry-gate formulas for the 22 evidenced cells (exact per-leg `fires` expression)

- **`avwap_252_breakout`** [long, vwap]: `- avwap_252low_loss_recent_3d<br>- avwap_252low_reclaim_recent_3d<br>- 14-day Relative Strength Index of close (Wilder smoothing). Numeric value [0, 100]; check inline strategy code for the specific comparison threshold (e.g. < 30 = oversold, > 70 = overbought)<br>- today's volume >= 20-day average volume (ratio >= 1.0); 20-day window includes today`
- **`break_retest_confluence`** [long, confluence]: `- today's close STRICTLY LESS than 20-day EMA (B609 F2 - symmetric to price_above_ema_20; fixes silent-gap on strat_break_retest_confluence SHORT side).<br>- today's close STRICTLY LESS than 50-day EMA (B609 F2 - symmetric to price_above_ema_50).<br>- today's close STRICTLY GREATER THAN today's open (bullish bar)<br>- today's close STRICTLY LESS THAN today's open (bearish bar)<br>- (today's close - today's low) / (today's high - today's low) <= 0.40 -- close is in the BOTTOM 40% of today's bar range (strong-close bear signal; B589 added)<br>- (today's high - today's close) / (today's high - today's low) <= 0.40 -- close is in the TOP 40% of today's bar range (strong-close bull signal; B589 added)<br>- MACD 12/26/9 histogram STRICTLY LESS than 0 (B609 F2 - symmetric to macd_12_26_9_bullish; fixes silent-gap where strat_break_retest_confluence SHORT used `not macd_bullish` which auto-passed when key was missing).<br>- macd_12_26_9_bullish<br>- price_above_ema_20<br>- price_above_ema_50<br>- (BUG-111 multi-bar pattern, anchored on DC20 = prior-20-day max-CLOSE level). ALL conditions: (a) breakout: some bar 2-8 bars ago (lag in 2..8) closed STRICTLY ABOVE the max CLOSE of the 20 bars preceding that bar; (b) retest: between the breakout bar and today, at least one bar's LOW was <= (breakout_level + 1.5*ATR(14)) -- i.e. price came back to within 1.5 ATR of the broken level from above; (c) hold: today's close >= broken_level (still trading above the flipped resistance->support). All 3 required; first lag that satisfies all 3 fires the signal (TYPICAL: 3-5 bars post-breakout).<br>- Mirror of resistance_break_retest (BUG-111). DC20-anchored on prior-20-day min-CLOSE. ALL: (a) breakdown 2-8 bars ago closed STRICTLY BELOW the prior 20d min CLOSE; (b) at least one subsequent bar's HIGH >= (breakdown_level - 1.5*ATR(14)) (retest came back within 1.5 ATR from below); (c) today's close <= broken_level (still below the flipped support->resistance). 1.5*ATR uses 14-bar EWM Wilder ATR.<br>- today's volume / 20-day average volume STRICTLY LESS THAN 1.0 (window includes today). Bulkowski 2005: retest pattern forms on LOWER volume than the initial break (supply absorption thesis).`
- **`bullish_engulfing_support`** [long, candle]: `- today's close is within +/-0.50% of one of: fib 38.2% / fib 50.0% / fib 61.8% (retracement from prior 50-bar swing-high to swing-low)<br>- OR today's open >= yesterday's close AND today's close <= yesterday's open AND today is bearish AND yesterday was bullish -- bearish reversal 2-bar pattern<br>- OR bearish_pin_bar<br>- OR today's open <= yesterday's close AND today's close >= yesterday's open AND today is bullish AND yesterday was bearish -- bullish reversal 2-bar pattern<br>- OR bullish_pin_bar<br>- OR 3-bar bearish reversal: bar -2 strongly bullish, bar -1 small-body, today strongly bearish closing in lower half of bar -2's body<br>- OR 3-bar bullish reversal: bar -2 strongly bearish, bar -1 small-body (any direction), today strongly bullish closing in upper half of bar -2's body<br>- OR today's close is within +/-0.30% of pivot point R1 (Standard: R1 = 2P - prior_L)<br>- OR today's close is within +/-0.30% of pivot point R2 (Standard: R2 = P + (prior_H - prior_L))<br>- OR today's close is within +/-0.30% of pivot point S1 (Standard pivot: S1 = 2P - H, where P = (prior_H + prior_L + prior_C)/3)<br>- OR today's close is within +/-0.30% of pivot point S2 (Standard: S2 = P - (prior_H - prior_L))<br>- OR obv_bearish<br>- OR On-Balance Volume in uptrend: today's OBV > OBV from 5 bars ago (cumulative volume on up-days minus volume on down-days)`
- **`cpr_narrow_bullish`** [long, pivot]: `- today's close > AVWAP anchored at the lowest LOW of the prior 50 trading days. AVWAP cumulates (typical_price * volume) since the anchor bar; close above means the upleg from that swing low is still institutionally supported.<br>- above_cpr<br>- today's close < AVWAP anchored at the lowest LOW of the prior 50 trading days (B612 F2).<br>- below_cpr<br>- below_ema_200<br>- cpr_narrow_tight<br>- today's close STRICTLY GREATER THAN the 200-day exponential moving average of close (long-term uptrend gate)`
- **`force_index_breakout`** [long, breakout]: `- today's close STRICTLY LESS than 20-day EMA (B609 F2 - symmetric to price_above_ema_20; fixes silent-gap on strat_break_retest_confluence SHORT side).<br>- today's close STRICTLY GREATER THAN today's open (bullish bar)<br>- today's close STRICTLY LESS THAN today's open (bearish bar)<br>- force_index_cross_dn<br>- force_index_cross_up<br>- price_above_ema_20`
- **`institutional_committed_growth_long`** [long, institutional_persistence]: `- committed_growth_holders<br>- institutional_increased<br>- today's close STRICTLY GREATER THAN the 200-day exponential moving average of close (long-term uptrend gate)`
- **`institutional_high_conviction_long`** [long, smart_money_13f]: `- institutional_new_positions<br>- price_above_ema_50`
- **`institutional_persistence_breakout_long`** [long, institutional_persistence]: `- institutional_increased<br>- today's close STRICTLY GREATER THAN the 200-day exponential moving average of close (long-term uptrend gate)<br>- (BUG-111 multi-bar pattern, anchored on DC20 = prior-20-day max-CLOSE level). ALL conditions: (a) breakout: some bar 2-8 bars ago (lag in 2..8) closed STRICTLY ABOVE the max CLOSE of the 20 bars preceding that bar; (b) retest: between the breakout bar and today, at least one bar's LOW was <= (breakout_level + 1.5*ATR(14)) -- i.e. price came back to within 1.5 ATR of the broken level from above; (c) hold: today's close >= broken_level (still trading above the flipped resistance->support). All 3 required; first lag that satisfies all 3 fires the signal (TYPICAL: 3-5 bars post-breakout).`
- **`institutional_persistence_oversold_long`** [long, institutional_persistence]: `- institutional_increased<br>- today's close STRICTLY GREATER THAN the 200-day exponential moving average of close (long-term uptrend gate)<br>- 14-day Relative Strength Index of close (Wilder smoothing). Numeric value [0, 100]; check inline strategy code for the specific comparison threshold (e.g. < 30 = oversold, > 70 = overbought)`
- **`macd_fast_crossover`** [long, momentum]: `- macd_8_21_5_crossover_dn<br>- macd_8_21_5_crossover_up`
- **`mfi_oversold_with_smart_money_long`** [long, smart_money_sleeve]: `- mfi_broad_oversold<br>- today's close STRICTLY GREATER THAN the 200-day exponential moving average of close (long-term uptrend gate)`
- **`news_sentiment_long`** [long, news_sentiment]: `- number of articles published about this ticker in the current 7-day window (coverage gate)<br>- news_sentiment_mean<br>- today's close STRICTLY GREATER THAN the 200-day exponential moving average of close (long-term uptrend gate)`
- **`pead_long_high_yoy_growth_only`** [long, event_driven]: `- earnings_eps_yoy_growth<br>- within_pead_window<br>- yoy_surprise_high<br>- yoy_surprise_threshold_long`
- **`poc_magnet_long`** [long, volume_profile]: `- today's close STRICTLY GREATER THAN the 200-day exponential moving average of close (long-term uptrend gate)<br>- vp_close_above_poc<br>- vp_close_near_poc_pct`
- **`r1_break_retest`** [long, pivot]: `- today's close > AVWAP anchored at the lowest LOW of the prior 20 trading days (B598 added for symmetric anchor pair with above_avwap_20high). Used by volume_spike_breakout LONG (B598).<br>- above_r1<br>- today's close < AVWAP anchored at the highest HIGH of the prior 20 trading days (B612 F2 - symmetric to above_avwap_20high; fixes silent-gap on volume_spike_breakout SHORT, volume_spike_breakout_retest SHORT, r1_break_retest SHORT).<br>- below_s1<br>- today's close STRICTLY GREATER THAN today's open (bullish bar)<br>- today's close STRICTLY LESS THAN today's open (bearish bar)<br>- (today's close - today's low) / (today's high - today's low) <= 0.40 -- close is in the BOTTOM 40% of today's bar range (strong-close bear signal; B589 added)<br>- (today's high - today's close) / (today's high - today's low) <= 0.40 -- close is in the TOP 40% of today's bar range (strong-close bull signal; B589 added)<br>- MACD 12/26/9 histogram STRICTLY LESS than 0 (B609 F2 - symmetric to macd_12_26_9_bullish; fixes silent-gap where strat_break_retest_confluence SHORT used `not macd_bullish` which auto-passed when key was missing).<br>- macd_12_26_9_bullish<br>- (B606 NEW; R1-anchored, NOT DC20-anchored). ALL: (a) some bar B 2-8 ago closed > R1_at_B, where R1_at_B is computed from bar (B-1)'s H/L/C using standard pivot formula (R1 = 2*P - L; P = (H+L+C)/3); (b) subsequent bar's LOW touched within 1.5*ATR(14) of R1_at_B (retest); (c) today's close >= R1_at_B (still above broken R1). Replaces the buggy DC20-anchored resistance_break_retest in strat_r1_break_retest. LOCAL signal consumed by strat_r1_break_retest only (B606).<br>- Mirror of r1_break_retest_long (B606). ALL: (a) some bar B 2-8 ago closed < S1_at_B (S1 = 2*P - H from bar B-1's H/L/C); (b) subsequent bar's HIGH touched within 1.5*ATR of S1_at_B (retest from below); (c) today's close <= S1_at_B (still below broken S1). Consumed by strat_r1_break_retest SHORT side.<br>- today's volume / 20-day average volume STRICTLY LESS THAN 1.0 (window includes today). Bulkowski 2005: retest pattern forms on LOWER volume than the initial break (supply absorption thesis).`
- **`rsi_oversold`** [long, mean_reversion]: `- below_ema_200<br>- OR below_sma_50<br>- OR today's close STRICTLY GREATER THAN the 200-day exponential moving average of close (long-term uptrend gate)<br>- OR today's close STRICTLY GREATER THAN the 50-day simple moving average of close<br>- OR 14-day Relative Strength Index of close (Wilder smoothing). Numeric value [0, 100]; check inline strategy code for the specific comparison threshold (e.g. < 30 = oversold, > 70 = overbought)<br>- OR 2-day RSI of close (Connors RSI methodology; faster and more sensitive than 14-day)`
- **`rsi_volume_200ema`** [long, confluence]: `- below_ema_200<br>- today's close STRICTLY GREATER THAN the 200-day exponential moving average of close (long-term uptrend gate)<br>- 14-day Relative Strength Index of close (Wilder smoothing). Numeric value [0, 100]; check inline strategy code for the specific comparison threshold (e.g. < 30 = oversold, > 70 = overbought)<br>- today's volume >= 20-day average volume (ratio >= 1.0); 20-day window includes today`
- **`smc_breaker_block_long`** [long, smc]: `- today's close STRICTLY GREATER THAN the 200-day exponential moving average of close (long-term uptrend gate)<br>- Active bullish breaker block (failed bearish order block that price reversed back through; now acts as support)`
- **`smc_inverse_fvg`** [long, smc]: `- below_ema_200<br>- force_index_cross_up<br>- today's close STRICTLY GREATER THAN the 200-day exponential moving average of close (long-term uptrend gate)<br>- smc_inverse_fvg_bearish<br>- smc_inverse_fvg_bullish<br>- today's volume >= 2.0x the 20-day average volume`
- **`totm_long`** [long, calendar]: `- is_totm_window_first_day<br>- today's close STRICTLY GREATER THAN the 200-day exponential moving average of close (long-term uptrend gate)`
- **`xs_combined_momentum_low_ivol`** [long, factor]: `- today's close STRICTLY GREATER THAN the 200-day exponential moving average of close (long-term uptrend gate)<br>- xs_ivol_decile<br>- xs_momentum_top_quintile`
- **`xs_momentum_with_smart_money_long`** [long, smart_money_sleeve]: `- today's close STRICTLY GREATER THAN the 200-day exponential moving average of close (long-term uptrend gate)<br>- xs_momentum_top_decile`
