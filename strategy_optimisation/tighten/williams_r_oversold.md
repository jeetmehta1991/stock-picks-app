...
P2  cmf_negative  <- backtest/signals/screener.py +1
       DEFN: Chaikin Money Flow, 20-bar rolling (technical.py:1695-1699)
       knobs P2.1-P2.1 (band rows in Table A)
P3  cmf_positive  <- backtest/signals/screener.py +1
       DEFN: Chaikin Money Flow, 20-bar rolling (technical.py:1695-1699)
...
P6  williams_r_oversold  <- backtest/signals/screener.py +3
       DEFN: Williams %R(14) < -80 (technical.py:702)
       knobs P6.1-P6.2 (band rows in Table A)

============================== STRATEGY LAYER ==============================

P7  williams_r > -20   [EXISTING-THRESHOLD]
P8  _short_borrow_trap_active(s)   [helper gate]
P9  rsi_2 escape-hatch thresholds (long < 5, short > 95)   [local-variable gate]

Gate body, VERBATIM from backtest/signals/screener.py strat_williams_r_oversold (docstring and return dropped):

```python
rsi_2 = s.get('rsi_2', 50)
above_200 = s.get('price_above_ema_200', False)
below_200 = s.get('below_ema_200', False)
fl = (s.get('williams_r_oversold') or rsi_2 < 5) and above_200 and s.get('cmf_positive')
fs = (s.get('williams_r', 0) > -20 or rsi_2 > 95) and below_200 and s.get('cmf_negative') and (not _short_borrow_trap_active(s))
```
...
| P2 | PRODUCER | cmf_negative - emitted by backtest/signals/screener.py +1; the boolean's UNDERLYING condition is bandable through its producer's internals | Chaikin Money Flow, 20-bar rolling (technical.py:1695-1699) | leg required True | - (knobs below) | only where the condition's input magnitudes are persisted on the fires - else none | variants over unpersisted bars/inputs - RESIM; a shared producer's resim runs the FULL OPEN consumer set and its one cube is graded per consumer (11.2s - results reused by construction); knobs DEFINED below (P2.x) | BANDS-DEFINED |
| P2.1 | BAND | level (cmf < 0) - technical.py cmf block | mirror of cmf_positive | 0.0 | 0, -0.02, -0.05 | TIGHTER levels on persisted cmf | LOOSER; DEFINED-NO-ACTUATOR | T3 review before any grid |
| P3 | PRODUCER | cmf_positive - emitted by backtest/signals/screener.py +1; the boolean's UNDERLYING condition is bandable through its producer's internals | Chaikin Money Flow, 20-bar rolling (technical.py:1695-1699) | leg required True | - (knobs below) | only where the condition's input magnitudes are persisted on the fires - else none | variants over unpersisted bars/inputs - RESIM; a shared producer's resim runs the FULL OPEN consumer set and its one cube is graded per consumer (11.2s - results reused by construction); knobs DEFINED below (P3.x) | BANDS-DEFINED |
| P3.1 | BAND | level (cmf > 0) - backtest/signals/technical.py:1699 | BRACKET zero | 0.0 | 0, 0.02, 0.05 | TIGHTER levels on persisted cmf | LOOSER (cmf > -0.02); DEFINED-NO-ACTUATOR | T3 review before any grid |
...
| P4.1 | BAND | ema span (the 200 in above/below_ema_200) - backtest/signals/technical.py compute_ema_sma | BRACKET production; 150/250 are the adjacent canon spans | 200 | 150, 200, 250 | none - other spans' values are unpersisted | the whole band; DEFINED-NO-ACTUATOR | T3 review before any grid |
...
| P6 | PRODUCER | williams_r_oversold - emitted by backtest/signals/screener.py +3; the boolean's UNDERLYING condition is bandable through its producer's internals | Williams %R(14) < -80 (technical.py:702) | leg required True | - (knobs below) | only where the condition's input magnitudes are persisted on the fires - else none | variants over unpersisted bars/inputs - RESIM; a shared producer's resim runs the FULL OPEN consumer set and its one cube is graded per consumer (11.2s - results reused by construction); knobs DEFINED below (P6.x) | BANDS-DEFINED |
| P6.1 | BAND | period - backtest/signals/technical.py:692 | BRACKET production | 14 | 10, 14, 20 | none | the whole band; DEFINED-NO-ACTUATOR | T3 review before any grid |
| P6.2 | BAND | oversold threshold - technical.py:702 | BRACKET canon -80; williams_r IS persisted | -80 | -90, -85, -80 | TIGHTER (< -85, < -90) - subset on persisted williams_r | LOOSER (> -80); DEFINED-NO-ACTUATOR | T3 review before any grid |
| P7 | STRATEGY | williams_r `> -20` [EXISTING-THRESHOLD] | gate threshold on the persisted magnitude | `> -20` | production + 2 tighter measured levels | measured tighter QUANTS levels - see the free-band section below | looser side - band at R1 | MEASURED-PRE-R1 |
...
| P9 | STRATEGY | rsi_2 escape-hatch thresholds (long < 5, short > 95) - backtest/signals/screener.py strat_williams_r_oversold (local rsi_2 compare - invisible to the extractor) | BRACKET production (Connors RSI-2 canon); rsi_2 persisted | 5 / 95 | [3, 5] / [95, 97] | TIGHTER (< 3, > 97) - subset on persisted rsi_2 | LOOSER | T3 review |
...
|---|---|---|---|---|---|
| williams_r | backtest/signals/screener.py +1 | `> -20` | 100.0% | TIGHTER = RAISE the floor: -19.89 -> 1014 (40%); -13.08 -> 508 (20%) | LOOSER = LOWER the threshold: RESIM - band from the SPECS entry (to be built) |
...
| P2.1 | level (cmf < 0) | 3 | subset-safe | no - derives offline |
| P3.1 | level (cmf > 0) | 3 | subset-safe | no - derives offline |
| P4.1 | ema span (the 200 in above/below_ema_200 | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P5.1 | rsi (fast escape-hatch) span | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P6.1 | period | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P6.2 | oversold threshold | 3 | subset-safe | no - derives offline |
| P7 | williams_r > -20 | 3 | subset-safe | no - derives offline |
| P8 | days_to_cover cap 5.0 | 1 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P9 | rsi_2 escape-hatch thresholds (long < 5, | 1 | subset-safe | no - derives offline |

```
FULL FACTORIAL     3 x 3 x 3 x 3 x 3 x 3 x 3 x 3 x 1 x 1 = 6561
offline gradings   81 level-combinations x 24 exits = 1944
...
B-row candidates NOT in this factorial: 651 census axes join it only when REGISTERED at the T3 band review.
