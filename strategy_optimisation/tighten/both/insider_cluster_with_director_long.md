...
P1  insider_cluster_active  <- backtest/signals/insider_buying.py +1
       DEFN: insider buy cluster active in the 30d window (smart_money.py cluster block)
       knobs P1.1-P1.1 (band rows in Table A)
P2  insider_unique_buyers_30d  <- backtest/signals/insider_buying.py +1
       DEFN: count of unique insider buyers in 30d (smart_money.py)
...
|---|---|---|---|---|---|---|---|---|
| P1 | PRODUCER | insider_cluster_active - emitted by backtest/signals/insider_buying.py +1; the boolean's UNDERLYING condition is bandable through its producer's internals | insider buy cluster active in the 30d window (smart_money.py cluster block) | leg required True | - (knobs below) | only where the condition's input magnitudes are persisted on the fires - else none | variants over unpersisted bars/inputs - RESIM; a shared producer's resim runs the FULL OPEN consumer set and its one cube is graded per consumer (11.2s - results reused by construction); knobs DEFINED below (P1.x) | BANDS-DEFINED |
| P1.1 | BAND | cluster rule (unique buyers >= N in window) - backtest/data/smart_money.py cluster block | BRACKET the shipped cluster rule | N in 30d | N [2, 3, 5]; window [14, 30, 60] days | N-side TIGHTER via persisted insider_unique_buyers_30d | window changes; DEFINED-NO-ACTUATOR | T3 review before any grid |
| P2 | PRODUCER | insider_unique_buyers_30d - emitted by backtest/signals/insider_buying.py +1; the boolean's UNDERLYING condition is bandable through its producer's internals | count of unique insider buyers in 30d (smart_money.py) | leg required True | - (knobs below) | only where the condition's input magnitudes are persisted on the fires - else none | variants over unpersisted bars/inputs - RESIM; a shared producer's resim runs the FULL OPEN consumer set and its one cube is graded per consumer (11.2s - results reused by construction); knobs DEFINED below (P2.x) | BANDS-DEFINED |
| P2.1 | BAND | count window (days) - backtest/data/smart_money.py:433-439 region | BRACKET production | 30 | [14, 30, 60] | count floors via the persisted count | window changes; DEFINED-NO-ACTUATOR | T3 review before any grid |
...
| P4 | STRATEGY | insider_director_buyers_30d `>= 1` [EXISTING-THRESHOLD] | gate threshold on the persisted magnitude | `>= 1` | production + 1 tighter measured levels | measured tighter QUANTS levels - see the free-band section below | looser side - band at R1 | MEASURED-PRE-R1 |
| P5 | STRATEGY | insider_officer_buyers_30d `>= 1` [EXISTING-THRESHOLD] | gate threshold on the persisted magnitude | `>= 1` | production + 1 tighter measured levels | measured tighter QUANTS levels - see the free-band section below | looser side - band at R1 | MEASURED-PRE-R1 |
...
|---|---|---|---|---|
| P1.1.1 | cluster rule (unique buyers >= N in wind | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P1.1.2 | cluster rule (unique buyers >= N in wind | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P2.1 | count window (days) | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P3.1 | ema span (the 200 in above/below_ema_200 | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P4 | insider_director_buyers_30d >= 1 | 2 | subset-safe | no - derives offline |
| P5 | insider_officer_buyers_30d >= 1 | 2 | subset-safe | no - derives offline |

```
FULL FACTORIAL     3 x 3 x 3 x 3 x 2 x 2 = 324
...
B-row candidates NOT in this factorial: 517 census axes join it only when REGISTERED at the T3 band review.
