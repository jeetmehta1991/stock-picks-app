...
|---|---|---|---|---|
| P1.1.1 | bb (period, k) | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P1.1.2 | bb (period, k) | 1 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
...
| P3.1 | proximity tolerance to S1 (abs dist/leve | 4 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P4.1 | proximity tolerance to S2 (abs dist/leve | 4 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P5 | rsi_14 < 35 | 5 | subset-safe | no - derives offline |
| P5.1 | rsi span | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |

```
FULL FACTORIAL     3 x 1 x 3 x 3 x 3 x 4 x 4 x 5 x 3 = 19440
offline gradings   5 level-combinations x 24 exits = 120
ENGINE RUNS        1 (actuated fire-adding axes only)
PENDING ACTUATION  3888 level-combinations are DEFINED but have no env knob - they are a FEATURE REQUEST, not a runnable band (plan 11.0b state 1; B2866)
