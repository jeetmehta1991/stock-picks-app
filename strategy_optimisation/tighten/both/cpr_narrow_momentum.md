...
| P1.1 | BAND | buffer pct (price vs cpr_top) - backtest/signals/technical.py:146 region | 0.0 | none - today's price is not a signal key | the whole band; DEFINED-NO-ACTUATOR | BRACKET zero; strict inequality today; T3 review before any grid |
...
| P2.1 | BAND | buffer pct (price below cpr_bottom by) - backtest/signals/technical.py:146 | 0.0 | none - entry price is not a signal key | the whole band; DEFINED-NO-ACTUATOR | BRACKET zero; T3 review before any grid |
...
| P4.1 | BAND | width threshold (cpr_width < rng * X) - backtest/signals/technical.py:103 | 0.05 | none - cpr_width IS persisted but the denominator (prior-day range) is not | the whole band; DEFINED-NO-ACTUATOR | BRACKET production (B654 local 0.05; the family's 0.15 stays with its own consumers); T3 review before any grid |
...
| P5.1 | BAND | span triple (fast, slow, signal) - backtest/signals/technical.py:634-648 | (12,26,9) | the (8,21,5) SWAP - re-evaluate on persisted macd_8_21_5 keys | any third triple; DEFINED-NO-ACTUATOR | MEASURED availability - both triples are emitted and macd_8_21_5_* IS persisted; T3 review before any grid |
...
| P6.1 | BAND | span triple (mirror of the bearish entry) - backtest/signals/technical.py:634-648 | (12,26,9) | the (8,21,5) swap on persisted keys | any third triple; DEFINED-NO-ACTUATOR | MEASURED availability (both persisted); T3 review before any grid |
...
| P7.1 | BAND | ema span (the 200 in above/below_ema_200) - backtest/signals/technical.py compute_ema_sma | 200 | none - other spans' values are unpersisted | the whole band; DEFINED-NO-ACTUATOR | BRACKET production; 150/250 are the adjacent canon spans; T3 review before any grid |
| P8 | STRATEGY | rsi_14 `< 50` [EXISTING-THRESHOLD] | `< 50` | measured tighter QUANTS levels - see the free-band section below | looser side - band at R1 | MEASURED-PRE-R1 |
...
| P9 | STRATEGY | rsi_14 `> 50` [EXISTING-THRESHOLD] | `> 50` | measured tighter QUANTS levels - see the free-band section below | looser side - band at R1 | MEASURED-PRE-R1 |
