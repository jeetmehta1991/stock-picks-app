...
|---|---|---|---|---|
| P1.1 | (structural) c > o - optional min body p | 1 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P2.1 | range-position cutoff | 1 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P3 | news_count_5d >= 3 | 2 | subset-safe | no - derives offline |
| P4 | news_sentiment_5d <= -0.3 | 2 | subset-safe | no - derives offline |
| P5 | news_sentiment_shift > 0.2 | 2 | subset-safe | no - derives offline |
| P6 | pct_change_5d < -0.1 | 2 | subset-safe | no - derives offline |

```
FULL FACTORIAL     1 x 1 x 2 x 2 x 2 x 2 = 16
...
B-row candidates NOT in this factorial: 0 census axes join it only when REGISTERED at the T3 band review.
