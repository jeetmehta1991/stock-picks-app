...
| P5.1 | BAND | proximity tolerance to S2 (abs dist/level) - backtest/signals/technical.py:76 (near), :81 (near_wide) | BRACKET production 0.003; 0.015 is the shipped near_*_wide | 0.003 | [0.002, 0.003, 0.005, 0.015 (the _wide variant)] | none - the level is persisted but today's price is not a signal key | the whole band; DEFINED-NO-ACTUATOR | T3 review before any grid |
| P6 | STRATEGY | rsi_14 `< 45` [EXISTING-THRESHOLD] | Wilder RSI over the named period - the persisted numeric the strategy thresholds (technical.py:540) | `< 45` | production + 4 tighter measured levels | measured tighter QUANTS levels - see the free-band section below | looser side - band at R1 | MEASURED-PRE-R1 |
...
| P7 | STRATEGY | rsi_14 `> 60` [EXISTING-THRESHOLD] | Wilder RSI over the named period - the persisted numeric the strategy thresholds (technical.py:540) | `> 60` | production + 0 tighter measured levels | measured tighter QUANTS levels - see the free-band section below | looser side - band at R1 | MEASURED-PRE-R1 |
| P7.1 | BAND | rsi span - backtest/signals/technical.py rsi block | BRACKET production with adjacent canon spans | 14 | [9, 14, 21] | none - values at other spans unpersisted | the whole band; DEFINED-NO-ACTUATOR | T3 review before any grid |
...
|---|---|---|---|---|
| P1.1.1 | implicit anatomy knobs (min body/wick/st | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P1.1.2 | implicit anatomy knobs (min body/wick/st | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P1.1.3 | implicit anatomy knobs (min body/wick/st | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P2.1.1 | implicit anatomy knobs (min body/wick/st | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P2.1.2 | implicit anatomy knobs (min body/wick/st | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P2.1.3 | implicit anatomy knobs (min body/wick/st | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P3.1.1 | implicit anatomy knobs (min body/wick/st | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P3.1.2 | implicit anatomy knobs (min body/wick/st | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P3.1.3 | implicit anatomy knobs (min body/wick/st | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P4.1 | proximity tolerance to R2 (abs dist/leve | 4 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P5.1 | proximity tolerance to S2 (abs dist/leve | 4 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P6 | rsi_14 < 45 | 5 | subset-safe | no - derives offline |
| P6.1 | rsi span | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P7 | rsi_14 > 60 | 1 | subset-safe | no - derives offline |
| P7.1 | rsi span | 3 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |
| P8 | days_to_cover cap 5.0 | 1 | **FIRE-ADDING** | no - production only (DEFINED-NO-ACTUATOR) |

```
FULL FACTORIAL     3 x 3 x 3 x 3 x 3 x 3 x 3 x 3 x 3 x 4 x 4 x 5 x 3 x 1 x 3 x 1 = 14171760
offline gradings   5 level-combinations x 24 exits = 120
ENGINE RUNS        1 (actuated fire-adding axes only)
PENDING ACTUATION  2834352 level-combinations are DEFINED but have no env knob - they are a FEATURE REQUEST, not a runnable band (plan 11.0b state 1; B2866)
...
B-row candidates NOT in this factorial: 527 census axes join it only when REGISTERED at the T3 band review.
