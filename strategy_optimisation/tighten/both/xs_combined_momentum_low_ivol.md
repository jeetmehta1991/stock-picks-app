...
| P2.1 | BAND | quantile cut (quintile) - backtest/signals cross-sectional factor block | top 20pct | re-cuts via persisted xs decile/raw keys - OFFLINE | recompute at other formation windows; DEFINED-NO-ACTUATOR | BRACKET the cut; the underlying 12-1 momentum and decile keys are persisted; T3 review before any grid |
| P3 | STRATEGY | xs_ivol_decile `<= 4` [EXISTING-THRESHOLD] | `<= 4` | measured tighter QUANTS levels - see the free-band section below | looser side - band at R1 | MEASURED-PRE-R1 |
