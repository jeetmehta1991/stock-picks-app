...
|---|---|---|---|---|---|---|
| P1 | PRODUCER | stochrsi_cross_dn - emitted by backtest/signals/screener.py +1; its tunables live inside that producer | leg required True | - (a boolean leg has no offline tighter level) | producer knobs - INVENTORY-PENDING-R1 (SPECS) | INVENTORY-PENDING-R1 |
| P2 | PRODUCER | stochrsi_overbought - emitted by backtest/signals/screener.py +1; its tunables live inside that producer | leg required True | - (a boolean leg has no offline tighter level) | producer knobs - INVENTORY-PENDING-R1 (SPECS) | INVENTORY-PENDING-R1 |
...
| P4 | STRATEGY-HELPER | _short_borrow_trap_active(s) - a helper gate; its internals are outside the source pattern (lower bound) | required | - | inspect at R1 | INVENTORY-PENDING-R1 |
...
|---|---|---|---|---|---|
| rsi_14 | backtest/signals/screener.py | `> 45` | 100.0% | 51.72 -> 3430 (80%); 56.94 -> 2573 (60%); 62.132 -> 1715 (40%); 68.178 -> 858 (20%) | looser (lower the threshold): RESIM - band from the SPECS entry (to be built) |
