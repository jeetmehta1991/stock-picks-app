...
**Live value (`len(ALL_STRATEGIES)` 2026-06-22 post-B996; source `python -c "from backtest.signals.screener import ALL_STRATEGIES; print(len(ALL_STRATEGIES))"` returns 219):** **219 IMPLEMENTED strategy classes registered** (B611 baseline 221; B613 net 0; B615 +1 = 222; B620 -1 = 221; B636 +1 = 222; B639 -1 = 221; B645 +1 = 222; B682 -4 = 218; B685 +3 = 221; B686 +1 = 222; B709 +2 EMPIRICAL-RESTORE per B702 adversarial review verdict = 224 -- phi correlation = 0.297 on 29 T1a tickers 2020-2026; 70% of EV-3 fires are a distinct population EV-1 misses; B682 deletion of strat_pead_long_high_yoy_growth_only + strat_pead_short_negative_yoy_growth empirically wrong; B722 -3 = 221; **B874 -2 = 219 per S4-B754-A-19 Pattern W council 5-lens option A2 deletion of strat_camarilla_rsi_obv dual + standalone**). `DEPRECATED_STRATEGIES` empty. `STRATEGIES_DISABLED_MISSING_PRODUCER` **3** (Batch 372 dxy_headwind_multinational_short — foreign_rev_pct producer absent; **B975 2026-06-21** naked_poc_retest_long — naked_poc_count + naked_poc_nearest_distance_pct producers never implemented in volume_profile.py per Council 77 P1 Bucket A A5 C2 fix; **B984 2026-06-21** m_and_a_target_long — `8k_item_1_01_filed_within_30d` producer never reliable per CLAUDE.md EV-7 buyback_8k_recent_long DELETED CC-B 8-K population-mixing M&A Item 1.01 SM-4 feasibility-failure carry per Council 88 walk-1 Sub-A). `EXPLORATORY_STRATEGIES` **12** (post-B992 walk-4: pivot_s3_capitulation B644 + pivot_r3_blowoff_short B652 + cup_and_handle_long B685 + institutional_persistent_holders_long B979 + 52w_high_breakout B992 + 52w_high_breakout_with_smart_money_long B992 + _vol_below_long B992 + 52w_low_breakdown B992 + bollinger_tight_with_smart_money_long B992 + classification_change_breakout_long B992 + _from_tech_short B992 + _momentum_long B992). **216 active for next cube iteration** (216 × 26 = 5,616 cells × 7 regimes = 39,312 total cube cells per PATH_TO_PHASE_1B_ALPHA.md; note EXPLORATORY-tagged strategies remain in registry for cube replay coverage but excluded from multiple-testing family-N per `cube_eligible_for_multiple_testing` flag).
...
**Acceptable phrasing variants (B1003 2026-06-22 updated per CLAUDE.md banner explicit stale flag on prior 108-133 figure):** "219 IMPLEMENTED + 12 EXPLORATORY + 3 DISABLED = 216 active (Phase 1A-β cube)" — or scope-narrow: "Layer 1 baseline (60 classes per PROJECT_PLAN §6)". *Not acceptable:* a bare "60 strategies" or "72 strategies" without scope qualifier; the legacy "108-133+ classes" phrasing PRE-DATES Pass 53 Layer 1.I symmetry adds + Layer 2D ICT inline-spec + Class 7 NEW walks (B603/B636/B645/B685/B686 etc.) and should not be used (banner-flagged stale by CLAUDE.md 2026-06-18).

**Code-current state (B1003 2026-06-22 updated):** `len(ALL_STRATEGIES) == 219` in `backtest/signals/screener.py` (verified via `python -c "from backtest.signals.screener import ALL_STRATEGIES; print(len(ALL_STRATEGIES))"`). Layer 1 + Layer 2 + Layer 2D ICT + Layer 3A + Layer 3B + Class 7 NEW walks IMPLEMENTED. `STRATEGIES_DISABLED_MISSING_PRODUCER` 3 (dxy_headwind / naked_poc_retest_long / m_and_a_target_long). `EXPLORATORY_STRATEGIES` 12 (Multiple-testing-correction excluded). 216 active for cube. Layer 4 PENDING per DEC-141/142/143/145/176. **Per owner directive, code-current is canonical post-Pass 53 Sprint 0A through B996.** Prior "len == 60" claim was Pass 52 era and is significantly stale; corrected in B1003 stale-claim audit per Council 101.
