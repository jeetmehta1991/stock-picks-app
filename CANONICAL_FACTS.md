...
**Live value (`len(ALL_STRATEGIES)` 2026-08-23 post-B2103; source `python -c "from backtest.signals.screener import ALL_STRATEGIES; print(len(ALL_STRATEGIES))"` returns 219):** **219 IMPLEMENTED strategy classes registered** (B2101-B2103 tranche A COMPLETE: +6 EXPLORATORY M-strategies - pocket_pivot_long, consec_downdays_quality_long, gap_and_go_long, failed_breakout_2b_short, rs_line_sector_leader_long, earnings_avwap_reclaim_long) (B2098 owner-approved F23: classification_change 10 -> 1 consolidation, 222 -> 213 - the producer emits every destination key in one return dict, so the 9 deleted variants are deterministic subsets reconstructable offline from the survivor; prior lineage preserved below) (B611 baseline 221; B613 net 0; B615 +1 = 222; B620 -1 = 221; B636 +1 = 222; B639 -1 = 221; B645 +1 = 222; B682 -4 = 218; B685 +3 = 221; B686 +1 = 222; B709 +2 EMPIRICAL-RESTORE per B702 adversarial review verdict = 224 -- phi correlation = 0.297 on 29 T1a tickers 2020-2026; 70% of EV-3 fires are a distinct population EV-1 misses; B682 deletion of strat_pead_long_high_yoy_growth_only + strat_pead_short_negative_yoy_growth empirically wrong; B722 -3 = 221; **B874 -2 = 219 per S4-B754-A-19 Pattern W council 5-lens option A2 deletion of strat_camarilla_rsi_obv dual + standalone**; B1010 +1 = 220 per Council 103 Option-6 Class 7 NEW strat_insider_cluster_concentrated_sell_short). `DEPRECATED_STRATEGIES` empty. `STRATEGIES_DISABLED_MISSING_PRODUCER` **0** (B1189 DELETED dxy_headwind_multinational_short entirely; historical note — Batch 372 had disabled it for an absent foreign_rev_pct producer; **B1035 2026-06-27 Council 129 Option-6 owner-approved REVERSED B975 + B984 after F2/F3 sub-agent runtime probes**: F2 confirmed naked_poc producer EXISTS at screener.py:8257-8272 inline orchestrator [B975 was BLIND-SPOT-3 false-positive — extractor b974_classify_signal_orphan_17.py:202 skips screener.py orchestrator by design]; F3 confirmed m_and_a producer EXISTS-RELIABLE at sec_edgar_extractor.py:239-344 + B748d pin test 8 still passes [B984 "CC-B 8-K population-mixing carry" was citation-slip from EV-7 deletion; screener.py:3454-3456 explicitly distinguishes EV-7's `recent_8k_filed` proxy from properly-parsed Item 1.01 strategies]). `EXPLORATORY_STRATEGIES` **21** (B2101-B2103 +6 tranche-A) (B1382 +3 mirror shorts; B2085 +2 pairs legs; B2098 -3 classification entries deleted with their strategies) (post-B1035 walk: pivot_s3_capitulation B644 + pivot_r3_blowoff_short B652 + cup_and_handle_long B685 + institutional_persistent_holders_long B979 + 52w_high_breakout B992 + 52w_high_breakout_with_smart_money_long B992 + _vol_below_long B992 + 52w_low_breakdown B992 + bollinger_tight_with_smart_money_long B992 + classification_change_breakout_long B992 + _from_tech_short B992 + _momentum_long B992 + m_and_a_target_long B1035 [pending SM-4 feasibility cube verdict via S4-B673 ticket]). **219 registered / 218 grading-active post-B2103 (the data-scarce survivor stays registered-disabled)** (219 × 24 = 5,256 cells post-B2110 - LEVER9 (a) deprecated smart_money_reversal + atr_trail_mae_conditional, both never-selected outcome-duplicates of the surviving atr_trail_1x × 7 regimes = 39,858 total cube cells per PATH_TO_PHASE_1B_ALPHA.md; note EXPLORATORY-tagged strategies remain in registry for cube replay coverage but excluded from multiple-testing family-N per `cube_eligible_for_multiple_testing` flag).

**Value (planned target):** **199 RESOLVED-DECIDED strategy classes across 6 layers post owner "Approve all" + Layer 1.I symmetry + Q1+Q2+Q3 (Layer 5 flag schema + Layer 6 27 new) 2026-05-06 (was 134 pre-symmetry; 108-118 pre-Pass-53-Option-2). Goes to 203 when Layer 4 PENDING-DEC promoted; ~213 with Layer 2D form-derived ICT estimate. ~200-400+ multi-TF variants projected. Total unique testable strategies projected: 200+ confirmed (could exceed 400 with full multi-TF expansion).** Gap: **B897 freshness update: live count 219 has now EXCEEDED original 199 target (by +20) due to B709 EMPIRICAL-RESTORE + B685+B686 Class 7 NEW additions + Layer 2D ICT inline-spec sweeps**. Original "186 live" reference was Batch 372 Pass 53 era; superseded by B611-B874 strategy walks.
...
**Acceptable phrasing variants (B1035 2026-06-27 updated per Council 129 Option-6 owner-approved naked_poc + m_and_a re-enable):** "222 IMPLEMENTED + 16 EXPLORATORY + 0 DISABLED = 222 active (Phase 1A-β cube)" (B1382) — or scope-narrow: "Layer 1 baseline (60 classes per PROJECT_PLAN §6)". *Not acceptable:* a bare "60 strategies" or "72 strategies" without scope qualifier; the legacy "108-133+ classes" phrasing PRE-DATES Pass 53 Layer 1.I symmetry adds + Layer 2D ICT inline-spec + Class 7 NEW walks (B603/B636/B645/B685/B686 etc.) and should not be used (banner-flagged stale by CLAUDE.md 2026-06-18).

**Code-current state (B1382 2026-07-25 updated):** `len(ALL_STRATEGIES) == 222` in `backtest/signals/screener.py` (verified via `python -c "from backtest.signals.screener import ALL_STRATEGIES; print(len(ALL_STRATEGIES))"`). Layer 1 + Layer 2 + Layer 2D ICT + Layer 3A + Layer 3B + Class 7 NEW walks IMPLEMENTED. `STRATEGIES_DISABLED_MISSING_PRODUCER` 0 (B1189 DELETED dxy_headwind; B975 + B984 previously reversed by B1035 after runtime probes confirmed producers exist). `EXPLORATORY_STRATEGIES` 16 (Multiple-testing-correction excluded; B1382 +3 mirror shorts unvalidated-by-construction per L229). 222 active for cube. Layer 4 PENDING per DEC-141/142/143/145/176. **Per owner directive, code-current is canonical post-Pass 53 Sprint 0A through B1035.** Prior "len == 60" claim was Pass 52 era and is significantly stale; corrected in B1003 stale-claim audit per Council 101.
