...
| BUG-290 | HIGH  -  `cap_band` consumed by `strat_january_effect_small_cap_long` but NEVER produced at signal-compute time. Only derived in exit_context.py post-trade-close. Entry gate always fails | Batch 312 reference | DOCUMENTED Pass 53 Batch 312 2026-05-24 (Phase 1A-beta 60-quiet-strategy forensic surfaced. Need producer for cap_band in screener.py/compute_all_signals from info_dict.market_cap via bands (micro <$300M / small $300M-$2B / mid $2B-$10B / large $10B-$200B / mega >$200B). Same fix would also enable per-classifier dashboard slicing of strategy x cap_band cells per owner's per-classifier cube vision. Reference only - not blocking Batch 312-QUALITY work.) |
