...
**INSTANCE - A FIELD'S MEANING IS A PRODUCER FACT, AND UNIFORMITY CAN BE THE DATA'S SHAPE (B3088 / L864).** This item says prove a pattern CAN match before reporting absence. Its sibling: prove you have read the WRITER before reporting that a field carries something other than its name suggests. MEASURED 2026-09-23, owner-caught: I reported Table D's per-cell trade counts as absent across 432 cells and rendered them '-', reasoning that `fires` was identical across all 24 exits so it could not be a per-cell count. grade_candle_config.py:268 and :279 both count CUBE ROWS - the field is misnamed - and the uniformity is correct BY CONSTRUCTION, because the cube replays every entry through all 24 exits. **The cheap discriminator: divide the row count by the exit count and see whether it equals the field.** 18 of 18 configs matched exactly. Worse, the wrong conclusion had been written into a PIN, so the suite defended it green (L706). ENFORCED BY test_b3088_table_d_counts_are_trades_and_carry_a_depth_tier.
...
**INSTANCE 2 - THE DENOMINATOR CAN BE A RENDERED VIEW, NOT A SENTENCE (B3082 / L861).** Every example above is a prose sentence, so this item was never applied to a TABLE - yet Table D's inventory preamble IS the TESTED/UNTESTED enumeration this rule demands. MEASURED 2026-09-23 on 18 three_white_soldiers artifacts: it decided TESTED by asking whether a parameter COULD be graded offline rather than whether any row VARIED it, so three axes tested by engine re-simulation read UNTESTED and an axis nothing touched read TESTED at its four band levels. **Apply #182 to any rendered view that enumerates coverage**, and never let a label fall back to the space that was AVAILABLE when the subset USED is empty. ENFORCED BY test_b3082_inventory_labels_come_from_evidence_not_band_shape.
...
**Instance L840b (B2978): A RUN AT ITS LAST SIM-DAY IS NOT FINISHED - PROVE TERMINALITY BY ZERO OPEN TRADES PLUS A FINISHED LINE IN THE CHAIN LOG, NEVER BY THE SIM-DAY INDEX. COMPLIANCE FAILURE AGAINST ITEM #256, third instance in one hour and this one INSIDE the lesson written about the first two. MEASURED 2026-09-22: L840 published 1.9291 h as candle_tws_c07's actual; that reading was taken with sim_day_index already at 249 and open_trades still non-zero, and the final value in the same heartbeat is 1.9708 h. Corrected consequences: the sim-day-117 extrapolation is high by 0.2623 h, which is 13.3 pct of the 1.9708 h final rather than the published 15.8 pct; and 1.9708 h against the 1.9322 h seven-config mean is 2.0 pct ABOVE it, not 0.2 pct below - so the original body-axis claim was wrong in MAGNITUDE and the retraction was wrong in SIGN. THE DISCRIMINATOR: a run at its last sim-day is still closing positions and writing its cube, so terminality is zero open trades AND a finished line in output_audit/serial_chain.log. Mechanism: scripts/candle_runtime_table.py refuses any non-terminal run by name with its reason, pinned by test_b2978_runtime_table_refuses_a_run_that_is_not_terminal.**

**Instance L845 (B2986): A DEFECT'S LOCATION IS A CLAIM - THE HONEST PRODUCER THAT REPORTS A CONDITION AND THE CONSUMER THAT FAILS CLOSED ON IT ARE DIFFERENT FILES, AND THE TICKET THAT NAMED IT RIGHT IS THE ONE I OVERRODE. COMPLIANCE FAILURE AGAINST ITEM #699 - a file blamed must be opened, and this is the instance where opening it was not enough. MEASURED 2026-09-22: S6-B2919 names its own defect correctly in its first sentence - the grader is honest and the consumer run_postconfig.grid_step2_graded fails closed. I overrode that twice. First the S6-B2984 row placed the closed path at grade_candle_config.py:243, which assigns BELOW_POWER_FLOOR and returns at 246, three lines before the only FAIL assignment at 253. Forced by the gate to open the file, I retracted to a THIRD wrong place, telling the owner it was roster_core.py:455 and :490 - the BH-FDR partition report, declared REPORT-ONLY at :470-472 and unable to set an exit code. The real path is run_postconfig.py:546 consumed at :649. THE PIN ALREADY EXISTED at test_unit.py:34306 and did not help, because a pin asserts behaviour and nothing makes a prose location claim match it. THE CONSEQUENCE INVERTED AND REACHED THE OWNER: on the second wrong location I said the chain-blocking argument was weakened when it is strengthened, run_postconfig.py being the battery supervisor that runs on every landing. THE RULE: when a ticket already names a location, re-deriving it is a second guess competing with a recorded answer - read the ticket's location claim first and open THAT file. Pinned by test_b2986_the_honest_producer_and_the_closing_consumer_are_different_files.**

**Instance L844 (B2985): A PROVENANCE GATE THAT COVERS ONE HALF OF THE CODE A RUN DEPENDS ON IS A GATE THAT REPORTS CLEAN WHILE THE OTHER HALF DRIFTS - ASK WHICH DIRECTORIES THE HASH ACTUALLY WALKS. COMPLIANCE FAILURE AGAINST ITEM #224 - a gate nobody calls is not enforcement - in the form where the gate IS called and covers half its subject. MEASURED 2026-09-22: engine_path_hash at scripts/run_serial_chain.py:175 walks backtest/ except tests and REFUSES a launch on an unaccepted change, and every launch of the b2944b chain carries b9bfcf115410ae77 with 128 files, so the engine freeze is real and enforced. But the GRADING path is in scripts/ - grade_candle_config.py is the candle family's registered landing adapter at producer_variant_table.py:2160 and :2331 - and nothing hashes, records or refuses a change to it. THE TELL: a digest unchanged across many runs is evidence about its COVERAGE as much as about the code; one grep of its own function settles which directories it walks. Mechanism: battery_path_hash records the grading path beside the engine's per wave, RECORDED NOT GATED because a refusal over scripts/ would block launches for unrelated helper edits (L586); pinned by test_b2985_battery_path_is_fingerprinted_beside_the_engine.**

**Instance L843 (B2981): A GATE'S OWN SCAFFOLDING IS THE LEAST VERIFIED PART OF IT - A NEGATIVE CONTROL THAT READS LIVE REPO STATE MEASURES THE REPO, AND A GATE OVER AN ARTIFACT JUDGES ONE THAT NO LONGER EXISTS. COMPLIANCE FAILURE AGAINST ITEM #226 - prove a check can fail, and prove its CONTROL can pass. MEASURED 2026-09-22, two defects in the audit machinery surfaced by ordinary compliant work. (1) scan_monitor_without_stall_check judged a CronCreate arm that had since been deleted and replaced by a compliant one, firing forever on a job that no longer existed; fixed by collecting deletes alongside creates and exempting an arm superseded by a later delete plus a later compliant create, keyed on the observable sequence and deliberately not symmetric. (2) scan_owner_decision_taken reads EXECUTION_QUEUE.md from disk and had NO NEUTRAL entry, so the corpus negative control asked what the working tree looks like rather than whether ordinary prose trips the gate - it stayed green only until the first owner-gated ticket was closed, which happened this turn and turned test_b1760 red. NEITHER WAS REACHABLE BY TESTING THE GATES: both needed a real state change. When a gate fires on work you are sure of, or an unrelated test reddens the first time some state occurs, suspect the gate's window, control and neutralisation before the work. Pinned by test_b2981_stall_gate_does_not_judge_a_retired_monitor (six arms) and by the restored NEUTRAL entry that test_b1760 exercises every run.**

**Instance L842 (B2980): REWRITING AN INSTRUMENT IS THE MOMENT TO SWEEP ITS KNOWN DEFECTS - A REWRITE THAT FIXES ONLY THE FAULT YOU WERE SENT FOR CARRIES EVERY OTHER ONE FORWARD WITH A FRESH TIMESTAMP. COMPLIANCE FAILURE AGAINST ITEM #420 - a monitor that reports progress cannot report a hang. MEASURED 2026-09-22: told to fix ONE fault in the hourly monitor, I rewrote the whole prompt with the council's better fix plus three unrelated corrections - and reproduced the missing stall clause that L420 / S6-B1555a already records, whose own text says three ticks called a hung run healthy. A patch declares what it changes; a REWRITE silently re-asserts everything it does not change, with a fresh timestamp that makes the whole artifact look newly considered, and the three deliberate improvements made it look MORE reviewed. Cost was nil - the defective job lived about four minutes and no tick fired - but it was caught by a gate rather than by me. THE RULE: before replacing an instrument, grep the lesson record for that instrument's CLASS and check the replacement against each hit, not against the fault you were sent for. Mechanism: scan_monitor_no_stall_check already exists and is what caught it; no new detector, per #136.**

**Instance L841 (B2977): A CLAIM ABOUT WHAT YOUR OWN GATE MEASURES IS A CAPABILITY CLAIM - OPEN THE FILE THAT DEFINES ITS SCOPE BEFORE PREDICTING ITS VERDICT. COMPLIANCE FAILURE AGAINST ITEM #230 - a capability claim is a claim - in the shape its own examples do not suggest, because they are about external tools and formats while this is about MY OWN enforcement layer. MEASURED 2026-09-22: the engine's landing hook committed mid-gate and regenerated files under output_audit, and I told the owner in writing that I expected tree=CHANGED and would re-run. It returned tree=SAME. scripts/pyramid_gate.py lines 40-42 state the answer in a comment - SCOPE_DIRS is scripts, backtest and .claude/skills plus root markdown, and output_audit is excluded BY DESIGN because a landing during the run is not an edit to the thing being measured; EXECUTION_QUEUE.md is in scope with an APPEND_TOLERANT carve-out for landing rows. One command would have settled it. Note the direction: the prediction was CAUTIOUS, promising a re-run, so it read as diligence (L559, second time this session). Detection JUDGMENT-ONLY with its search named - scan_unverified_cause keys on cause language and a prediction carries none; durability via the test_b2123 fragment.**

**Instance L840 (B2976): A RATE READ MID-RUN IS NOT STATIONARY - NAME THE SIM-DAY AN EXTRAPOLATION CAME FROM AND LABEL AN EARLY-DAY PROJECTION BIASED HIGH. COMPLIANCE FAILURE AGAINST ITEM #256 - a figure you repeat is re-derived, not carried - in its sharpest form, because the figure was a RATE and re-deriving it INVERTED the conclusion it supported. MEASURED 2026-09-22 across three reads of candle_tws_c07's own run_heartbeat.json: at sim-day 117 the rate 0.008932 h per sim-day extrapolated to 2.2331 h, at sim-day 206 the rate 0.007501 extrapolated to 1.8752 h, and the run ended at 1.9291 h - so the early extrapolation was high by 15.8 pct of the actual, and the owner-facing objection built on it (that the config runs slower than the 1.9322 h landed mean) was refuted by an actual sitting 0.2 pct BELOW that mean. Distinct from L749: the reading's AGE was carried correctly and the MODEL was wrong. Detection JUDGMENT-ONLY with its search named (scan_unmeasured_quantity passed, because the figure did name its source); durability via S6-B2972's prompt rewrite and the test_b2123 fragment.**

**Instance L839 (B2973): A FIGURE INSIDE A SCHEDULED INSTRUMENT IS OUTSIDE EVERY REPO SWEEP AND RE-ASSERTS ITSELF ON A TIMER - GIVE THE BASELINE AN ARTIFACT TO BE QUOTED FROM, STAMPED WITH ITS WINDOW AND UNIVERSE. COMPLIANCE FAILURE AGAINST ITEM #182 in the form L836 named - a count carries no unit, so an unstated window and universe cannot betray themselves. MEASURED 2026-09-22: the hourly monitor's own prompt quotes R5 baselines of 1,596 and 1,674, which are 4-year figures over 481 and 474 tickers, against Step-1 cubes running 1 year over 200 tickers; matched on both axes R5 holds 137 and 157, so the honest ratios are 4.09x and 3.38x where the prompt's numbers give 0.35x and 0.32x. THE SWEEP IS THE FINDING, AND ITS FIRST VERSION WAS WRONG (B2974): I published that a tree-wide grep returns ONE hit, reading only the HEAD of the output. Re-measured with git grep -n -E "1,596|1,674" over tracked .md .py .json: 43 FILES. Hand-read, 42 are sound (37 spec _fires_basis fields that state both scopes explicitly, 4 producer_variant_table quantile derivations where full-window IS the right population, one within-R5 manifest risk line) and ONE is a real defect - L832 said 1,596 is 4 years x 544 tickers where the measured universe is 481. So 1 defect in 43, and it was inside the tree. Mechanism: scripts/candle_r5_baselines.py stamps both grains with their window and universe so neither can be quoted bare; detection is JUDGMENT-ONLY (no scan reads a scheduled prompt), durability pinned by test_b2973_baseline_artifact_carries_both_grains_with_their_scope.**

**Instance L838 (B2971): A NAMED TRIGGER THAT NOTHING EVALUATES IS A REMINDER, NOT A GATE - REGISTER AN EVALUATOR OR DISCLOSE THE TRIGGER AS UNEVALUATED. COMPLIANCE FAILURE AGAINST ITEM #224 - a gate nobody calls is not enforcement - extended from a GATE to a DEFERRAL'S OWN TRIGGER, which is the same claim with no code at all. MEASURED 2026-09-22: S6-B2620b's trigger (ledger-flip exempt commits exceeds 5) stood at 8 and had been over the line for three commits; S6-B2933 measured 5 eight days earlier and wrote that it sat one landing away. It surfaced only because the turn gate forced a grep of an unrelated identifier. Population hand-read: 17 DEFERRED tickets, at least 11 carrying a re-open condition, 0 with an evaluator before this batch. Mechanism: scripts/deferral_trigger_audit.py, which evaluates the registered triggers and DISCLOSES BY ID every one it cannot, rather than printing a clean zero; pinned by test_b2971_deferral_audit_discloses_what_it_cannot_evaluate.**

**Instance L837 (B2963): A BUCKET A FILTER CANNOT POPULATE MUST BE REFUSED, NOT REPORTED AS ZERO - CHECK THE COLUMN'S ATTRIBUTION BEFORE FILTERING ON IT. COMPLIANCE FAILURE AGAINST ITEM #166 - prove a pattern CAN match before reporting absence - extended from a grep pattern to a DATAFRAME FILTER, which is the same claim wearing different syntax. MEASURED 2026-09-21, council-caught by two advisors independently: I reported run_only_that_R5_BLOCKED = 0 as the decisive result of a discrepancy investigation, when all 391,782 of 391,782 occupancy rows in output_r5_merged_1_7 carry the literal (same-strategy) in the strategy column, so a strategy-name filter returns nothing for any strategy, always. S6-B2904 recorded that as UNQUANTIFIABLE and I had READ THAT ROW EARLIER IN THE SAME TURN. A FABRICATED ZERO SITTING IN A ROW OF HONEST NUMBERS INHERITS THEIR CREDIBILITY - the other four buckets in the same table were real counts. One nunique() call would have shown 1. In an instrument the fix is a REFUSAL, not care. Mechanism: the attribution guard in scripts/b2960_discrepancy_extract.py, pinned by test_b2962_extract_refuses_an_unpopulatable_bucket.**

**Instance L836 (B2955): TWO RATES ARE NOT COMPARABLE BECAUSE BOTH ARE PERCENTAGES - NAME THE DENOMINATOR OF EACH BEFORE PUTTING THEM IN ONE SENTENCE. COMPLIANCE FAILURE AGAINST ITEM #182, EXTENDED - the item disciplines a VERDICT's denominator and is silent on a RATE, which is where the denominator does the most damage. MEASURED 2026-09-21, council-caught: S6-B2949 retracts a grain mismatch in my own words, and two rows later I set this run's 36.0 pct conversion (landed over entry-stage signals, POST-screening) against R5's 8.8 pct (signal bars to trades, PRE-screening) and drew a conclusion from the gap. The counts were obviously incommensurable and were caught; TWO PERCENTAGES INVITE NO QUESTION AT ALL because they are already normalised, already unitless, already the same shape. The landed comparisons 560 against 137 and 531 against 157 survive; the rate comparison and the phrase 'the engine converts 3-4x better' are retracted. Two council advisors caught it independently from different lenses; no gate in a 44-gate suite could. Mechanism: scan_rate_comparison_without_denominators, pinned both directions by test_b2955_rate_comparison_needs_both_denominators.**

**Instance L835 (B2945): ONE GATE'S SILENCE IS NOT LAUNCH READINESS - A SPEC-TIME VALIDATOR THAT DOES NOT ENFORCE WHAT THE LAUNCH-TIME GATE REQUIRES JUST MOVES THE REFUSAL DOWNSTREAM. MEASURED 2026-09-21: 36 campaign specs returned 0 refusals from launch_refusals and I read that as launch-ready; the chain HALTED on spec 1 because prelaunch_gate - a DIFFERENT gate, reading the MANIFEST run_wave derives from the spec - fails closed on an absent fires_at_production, a field knowable in seconds at spec-build time. launch_refusals is thorough (nine checks per strategy) and thoroughness in one gate reads as coverage of the whole question - but it is a SPEC gate, not a LAUNCH gate. BEFORE LAUNCHING ON A VALIDATOR'S SILENCE, NAME THE OTHER GATES BETWEEN YOU AND THE THING YOU ARE STARTING, AND RUN THEM; here the chain was launch_refusals -> run_wave -> prelaunch_gate -> engine. Cheap probe that settles it: take the manifest the halted run already wrote, add the field, run the gate directly - this confirmed no SECOND hidden refusal, which mattered because B2937's gate refused THREE times, one relaunch each. Mechanism: launch_refusals refuses a wave spec without fires_at_production using the gate's own reason string, pinned by test_b2945.**

**Instance L834 (B2942): A PATCHER THAT BUILDS CODE FROM STRING LITERALS CAN COMMENT OUT WHAT IT MEANT TO ADD - AN ANCHOR COUNT CANNOT SEE THAT ITS MATCH SITS INSIDE A COMMENT. MEASURED 2026-09-21: my own patchers wrote a literal backslash-n inside non-raw Python literals, so multi-line inserts landed as ONE physical line; where it began with a hash, everything after became comment text. SIX tripwire rows (L827-L832) and the assert that guards their count sat inside a single 1,976-character comment for 10 commits (B2917-B2937). THE TWO FAILURES HID EACH OTHER: the count stopped rising because rows never landed, and the check stopped running because it was commented out - the test stayed GREEN throughout. Every patcher's s.count(anchor)==1 guard was TRUE as a substring; A SUBSTRING COUNT CANNOT SEE ITS MATCH IS INSIDE A COMMENT. Never build multi-line inserts from escape sequences - take the newline from the target file - and after a patcher runs, RE-READ THE REGION AS THE PARSER SEES IT. Mechanism: test_b2942_no_comment_line_swallowed_code, counterfactual EXECUTED (quiet at B2913, fires at B2917).**

**Instance L833 (B2941): A LADDER RUNG WITH NO ENFORCEMENT IS A CHECKLIST ITEM, NOT A GATE - AND THE UNENFORCED RUNG IS WHERE THE NEXT FAMILY WALKS THROUGH. MEASURED 2026-09-21: R1 was mechanized at B2883 and refuses an incomplete adapter; R2 was never mechanized, so the candle pair passed every launch-readiness probe with NO Table D axis family and a candle cfg fell through to smc's catch-all - six columns of dashes, all four candle knobs invisible, after 18 engine runs. No check could have caught it: 8 battery families, 3 axis entries, nothing declaring which strategies an entry serves. The rung's own probe was also loose (keyed on FAMILY, so it fails for smc_breaker_block_long too) - check a WORKING family before concluding yours is uniquely broken. WHEN ONE RUNG GETS MECHANIZED, ASK WHICH SIBLINGS DID NOT. Mechanism: serves per axis entry, pinned by test_b2941_every_battery_family_has_a_table_d_axis_entry.**

**Instance L832 (B2937): A FIGURE THAT DRIVES A DECISION NEEDS A PERSISTED ARTIFACT, NOT JUST A COMPUTATION THAT ONCE RAN. COMPLIANCE FAILURE AGAINST ITEM #201 - no new item is warranted, the item is right and my reading of it was too weak: it asks a figure to NAME its source, and I named a computation that left no trace, which is not a source anyone can return to. MEASURED 2026-09-21, owner-caught: '27 of 54 cells below the holdout floor' was quoted across many turns - in ticket rows, commit messages and a recommendation about 227 engine hours - with NOTHING on disk carrying it. Re-deriving reproduced 27 exactly, which is not the point. The sweep found 3 of 4 load-bearing figures had no artifact, including one (28 of 54 for the crows leg) that had never been computed at all, only assumed symmetric. WRITE A FIGURE TO AN ARTIFACT THE FIRST TIME IT WILL BE QUOTED TWICE; the test is not did-I-measure-it but can-the-owner-re-derive-it-without-me. Mechanism: JUDGMENT-ONLY for detection (no scan separates a figure needing an artifact from one that does not); durability pinned by test_b2123.**

**Instance L831 (B2935): A NEW DETECTOR'S FIRST OUTPUT IS NOT ITS VALIDATION - HAND-READ WHAT IT FLAGGED BEFORE QUOTING IT AS PROOF THE TOOL WORKS. COMPLIANCE FAILURE AGAINST ITEM #201 - no new item is warranted, since #201 already requires a figure to name its source and L644 already states the rule; what was missing was applying either to a number my own new tool produced. MEASURED 2026-09-21: a blocker audit's first run reported 2 stale predecessors and I published that as the tool's validation in three places; hand-reading showed 2 of 2 were CITATIONS and the true count was ZERO. The error ran in the direction that made the new tool look necessary. The discriminator that would have caught it was written by me one batch earlier into a sibling gate and not carried across (L608) - so when you ship a second detector over the same corpus, DIFF IT AGAINST THE FIRST ONE'S EXCLUSIONS before trusting its output. Mechanism: test_b2934_blocker_join_requires_dependency_phrasing, both directions.**

**Instance L830 (B2932): DECLARING A BACKLOG OWNER-BLOCKED IS A CLAIM ABOUT EVERY TICKET IN IT, AND A BLOCKER IS THE LEAST-AUDITED CLAIM A TICKET CARRIES. MEASURED 2026-09-20: three times in one session I reported the remaining backlog as owner-blocked and three times a stated blocker did not hold - S6-B2861 waited on a ticket already EXECUTED, S6-B2533 blamed a byte that measures zero, S6-B1248 waited TWO MONTHS for a Batch B trade log while 25 cubes on disk answer its question. A blocker is prose in a reason cell, never re-derived, while the row's status stays honestly non-terminal - so nothing looks wrong. RE-DERIVE A BLOCKER BEFORE REPEATING IT, and never summarise a backlog as blocked without doing so for every member: one sentence asserting N unexamined facts is the sentence the owner acts on. Mechanism: audit_ticket_staleness.py --blockers decides the blocked-on-a-now-terminal-ticket shape and enumerates the rest; it found 2 more on its first run. Pinned by test_b2932_blocker_audit_resolves_a_stale_predecessor.**

**Instance L829 (B2926): A TICKET THAT SAYS NEEDS AN OWNER DECISION IS A QUESTION ABOUT AUTHORITY, AND MERIT ARGUMENTS DO NOT ANSWER IT. MEASURED 2026-09-20: S6-B2918 was filed BLOCKED saying 'needs an owner decision between (a) and (b)'; I shipped (a) and justified it with cost, a unanimous self-convened council, and non-foreclosure - three true statements, none of them an approval. The cost of asking was ZERO. The damage is not the change, which was correct and reversible, but that the owner can no longer tell which other decision-pending tickets were resolved by my reasoning rather than by him. When a ticket's own text says a decision is owed to someone else, ASK - or say plainly you are proceeding without the answer and why; silently substituting your judgement is the failure, while disagreeing out loud never is (L633). Mirror of L619, which is wrongly labelling work owner-gated; both come from the same missing definition, now asked as S6-B2925. Mechanism: MECHANIZED at B2929 - scan_owner_decision_taken fires when a ticket whose EARLIER rows state a pending owner decision is marked terminal with no acknowledgement in the response. The original JUDGMENT-ONLY was WRONG and is a compliance failure against #299: no scan can read PROSE for load-bearing owner-decision language, but the LEDGER is a state diff - prior rows are a fact and a terminal transition is a fact. Keys on decision-pending forms, never citations; any acknowledgement clears it, including proceeding openly (L633). Proven fires on S6-B2918, silent on five compliant shapes. Durability also pinned by test_b2123.**

**Instance L828 (B2920): A TICKET BLOCKED ON AN OWNER DECISION CAN HAVE THAT DECISION EXECUTED BY ACCIDENT, AND NOTHING TELLS THE TICKET. A ticket's STATUS and its CAUSE rot at different rates, and only STATUS is watched - so a BLOCKED row keeps asking for a decision after the decision has been executed by something else. MEASURED: S6-B2533 asked the owner to approve an EOL renormalisation it priced at 19,659 lines; commit 0334d98fc had already landed a 19,926-line whole-file renormalisation of LEARNINGS.md ten days earlier, undisclosed, inside a commit about a roster split - and the lone CR byte its RCA blamed measures ZERO today. BEFORE RE-RAISING ANY BLOCKED TICKET, EXECUTE ITS OWN CITED EVIDENCE: a row naming a file, an offset, a classification or a count has already written its verification command. Distinct from L639, where a NUMBER decays - here the ACTION decayed, so the row requested permission for the past. Mechanism: test_b2921_canonical_doc_eol_state_is_pinned.**

**Instance L827 (B2917): VALIDATE A TOOL THROUGH THE ARGV ITS CALLER BUILDS, NOT THE ARGV YOU TYPE. You supply the arguments you are thinking about; the caller passes the ones it was configured with, so any argument carrying a DEFAULT is invisible in your run and load-bearing in its run. MEASURED: a three-leg spot check for a long/short PAIR was reported validated at 120 of 120 agreement, every run passing --strategy by hand; the battery passes no such flag and the checker defaulted to the LONG leg, so grading the SHORT leg ran the long-leg check and recorded step 4 DONE against the wrong strategy. Rebuild the CALLER'S argv in the pin and assert on that - here, that the two legs receive different names. Two-sided remedy: the caller passes a declared strategy_flag AND the tool stops defaulting, refusing when told nothing (L642). Compliance failure against #276b; distinct from L654, which asks whether the call happens rather than what it carries. Mechanism: test_b2917_battery_tells_the_spot_check_which_leg_it_is_grading.**

**Instance L826 (B2913): A GATE THAT DEFINES ITS OWN POPULATION ALWAYS REPORTS FULL COVERAGE. The coverage figure and the gate's scope come from the same predicate, so a case the gate is blind to is simply not its business and never appears as a gap. MEASURED: the R1 adapter refusal reported 8 of 8 covered / 0 blind, computed with engine_requiring_params, which skips any parameter whose env field is absent; sourcing the denominator from the BAND REGISTRY instead shows 13 entries carrying 30 banded engine_implemented parameters invisible to it, 9 of them not battery families. This is L817 recurring in the SAME session that recorded it - L817 was applied to resim_band and not to env beside it. A GATE MAY COMPUTE ITS VERDICT, NEVER ITS POPULATION: take the denominator from a registry, plan table or artifact inventory the gate does not own. A sweep of 7 readiness gates found 1 self-scoping, a LOWER bound since the detector knows one spelling (L821). Mechanism: no detection mechanism for denominator provenance; durability pinned by the test_b2123 fragment.**

**Instance L825 (B2912): MIGRATING HALF A RECORD FLIPS ITS READER TO THE NEW ENCODING AND MAKES THE UNMIGRATED HALF INVISIBLE. A reader picking its encoding with any(new_field in member) switches the WHOLE record as soon as one member carries it, and unmigrated members contribute nothing. MEASURED: migrating 15 env knobs to the free/resim split flipped leverage() to per-level for an entry whose 4 OFFLINE params were still on the coarse boolean, collapsing free_combos from 32 to 1 - a 32x leverage claim destroyed by a change that looked additive, in the direction L796 names as dangerous. Caught ONLY because a pin had frozen the number. Before migrating part of a record, FIND THE READER and ask what it does with a half-migrated one; B2767's docstring predicted this and I read it while diagnosing rather than before editing. Mechanism: no detection mechanism (no scan knows which records a reader treats as one unit); durability pinned by the test_b2123 fragment.**

**Instance L824 (B2911): A PROBE THAT REPORTS ABSENCE MUST PROVE IT CAN SEE PRESENCE. MEASURED: a probe reported 0 persisted signal keys across smc trades, which would have meant the family's knobs are un-gradable offline and every level needs an engine run - written into a band inventory that governs engine spend. It used ast.literal_eval with no json fallback against a JSON-valued column, so it failed on EVERY row; re-measured across all rows with the fallback, 948 of 948 carry a non-empty dict of 833 keys. A zero from a parser that cannot read the format is byte-identical to a zero meaning absence, and the absence reading is the dangerous one because it ENDS the investigation. #166 says prove the PATTERN can match; this is its sibling one layer down - prove the READER can PARSE, by running the extractor against a row known to carry the field and printing that row's value. Mechanism: no DETECTION mechanism (no scan knows what a column's values should parse as); durability pinned by the test_b2123 fragment.**

**Instance L823 (B2908): A DETECTOR TESTS THE PROPERTY YOU ENCODED, NOT THE ONE THE RULE REQUIRES. MEASURED: a backlog sized at 109 of 213 JUDGMENT-ONLY declarations naming no search; hand-reading 10 of the flagged set found 10 of 10 FALSE POSITIVES - five state a REASON (what L548 actually requires), one says 'NOT judgment-only', two quote the rule while describing a violation, one names a scan the pattern could not see. The count measured my encoding, not the corpus, and it carried every marker of soundness: a denominator, a named instrument, a stated limit, a complete population. RIGOUR ABOUT THE MEASUREMENT CANNOT CORRECT AN ERROR IN WHAT IS MEASURED. Before quoting a backlog figure, write down what a COMPLIANT instance looks like and confirm the matcher recognises one - construct the PASS, not just a sample of the flags (L701 samples the misses; this constructs the pass). The figure is RETIRED, not re-measured. Mechanism: no DETECTION mechanism is possible (no scan compares a detector's predicate to a rule's requirement); durability pinned by the test_b2123 fragment.**

**Instance L822 (B2906): A SIBLING ENCODES WHAT WAS TRUE WHEN IT WAS WRITTEN - READ THE CONSUMER'S CONTRACT BEFORE INHERITING ITS ASSUMPTIONS. A sibling is one IMPLEMENTATION of a contract, not the contract; its assumptions were correct on the day it was written, and the CONSUMER that invokes both is where the current contract lives - it moves without either file recording that it moved. MEASURED: a candle grader modelled on the institutional one inherited the rule that a cube carries exactly one strategy, superseded by B2721 (a cube carries a graded strategy PLUS declared riders, and graded_and_riders reads it); a long/short pair in one engine pass puts both legs in one cube, so the draft would have refused the campaign's own shape on config 1 of 54. Caught by a council advisor, not by my process. Distinct from L613 (copied DEFECTS - this sibling had none) and L799 (reusing an existing partition). Mechanism: the rider-bearing resolve arm of test_b2900_candle_grade_leg_is_wired_and_its_refusals_bite.**

**Instance L821 (B2901): A DETECTOR'S PATTERN IS A CLAIM ABOUT EVERY SPELLING OF WHAT IT DETECTS, AND IT IS BLINDEST ON THE IDIOM ITS OWN NEIGHBOURS USE. B1719b enumerated three RNG spellings; MEASURED, 2 of the 3 real sampling scripts used the module-level random.seed/random.sample form it never matched, so its hit rate was 1 of 3 while it reported clean. It fired on the third only because that file happened to use random.Random( - a style choice. New code is written by copying the nearest example, so a pattern that misses the neighbours has its gap exactly where the next instance lands. WHEN A DETECTOR FIRES ON ONE FILE, MEASURE ITS HIT RATE ON THE CLASS before shipping the one-line fix that makes that file pass - narrow-pattern hits versus wide-pattern hits over the same tree, one query. Compliance failure against the GENERALIZATION MANDATE; same family as B2578 (a literal needle too narrow across LINES).**

**Instance L820 (B2898): A REMEDIATION INSTRUCTION IS A CLAIM, AND ITS FIX HALF SHIPS UNVERIFIED. The DIAGNOSIS half of a gate message fires on real input and gets corrected when wrong; the REMEDY half fires on nobody until a reader acts on it, so it is untested by construction - and it is trusted because the diagnosis was right. MEASURED: a refusal written to STOP a false message told operators to complete a tools block in place; family_refusal reads a different registry, so following it changes nothing. One probe (thirty seconds) would have caught it. Before shipping remediation text, EXECUTE the path it recommends. Compliance failure against the MECHANISM-EXISTENCE rule; L688 is its other face - find the branch that EMITS a string, and run the path a string RECOMMENDS.**

**Instance L819 (B2891): A SWEEP THAT REPORTS A CLASS CLOSED IS THE ONE LEAST LIKELY TO BE VERIFIED, BECAUSE CLOSURE ENDS THE WORK. MEASURED by regex over the 58 ticket rows of one session: 3 assert closure or zero collateral, 2 of the 3 are one repeated wrong claim, 1 records the overclaim - so every genuine closure assertion was wrong. A figure that CONTINUES the work invites scrutiny because someone must act on it; a figure that ENDS it invites none, and it is the same arithmetic. Before writing a closure figure, state what you would have to READ to be wrong, then read it. Compliance failure against item #270 and L679; the INCENTIVE half is promoted to the skill. Detection JUDGMENT-ONLY (a measured and an assumed closure figure are both integers); durability pinned by test_b2123.**

**Instance L818 (B2889): EVIDENCE ACCUMULATES ACROSS A TURN; A CLAIM IS SUPERSEDED BY THE LATEST BLOCK. B2555's rule (do not reset the window on the gate's own block) is right for evidence and wrong for claims. MEASURED: the verdict-denominator gate blocked three consecutive closes on one sentence from an earlier close while the current close carried its denominators - and because the denominator must sit in the SAME text block as the verdict, and that block is already in the transcript, no wording could clear it. Claim-reading gates now judge the current close; evidence-reading gates still span the turn. Pinned by test_b2889, five cases, both directions.**

**Instance L817-promoted (B2888): the generalised half of L817 - blast radius and coverage are the SAME number and mean opposite things, so ask how many of the UNREFUSED were explicitly exempt versus silently unclassifiable - is promoted to the skill tripwire table and pinned by test_b2123. The measurement stays in LEARNINGS because a row carrying a count decays. Compliance failure against the L642 rule, which carries the GUARD form of this defect and not the MEASUREMENT form.**

**Instance L817 (B2887): A DISCRIMINATOR KEYED ON AN OPTIONAL FIELD IS BLIND WHERE THE FIELD IS ABSENT, AND THE BLINDNESS MEASURES AS PRECISION. I keyed an R1 refusal on env-knob-plus-resim-level, measured 2 of 18 refused, and reported that as precision. MEASURED afterwards: of the entries carrying env knobs but no engine-requiring band, 3 of 3 are blind (resim_band ABSENT) and 0 of 3 are explicitly production-only - including smc_breaker_block_long, the most-run engine family in the repo. A low refusal count is what a precise rule gives you AND what a blind one gives you; the separating question is how many of the UNREFUSED were explicitly exempt versus silently unclassifiable. Compliance failure against the L642 rule (a check conditioned on the presence of the thing it guards converts UNDECLARED into APPROVED), arriving through a discriminator rather than a guard. Mechanism: test_b2887 freezes the blind population shrink-only.**

**Instance L816 (B2883): A COMPLETENESS CLAIM ABOUT A SEARCH IS SCOPED TO THE PARAMETER SPACE THAT EXISTED WHEN THE READ WAS TAKEN. MEASURED: I cited a 130-line Step-2 holdout as proof a strategy pair was exhausted; the artifact covers ONE axis (RSI levels) and the four anatomy axes did not exist until the same session wrote them - true scope 1 of 5, and the owner had to overrule the recommendation. The artifact keeps its authoritative header, line count and FAIL verdict, and its own preamble was TRUE when written, which is what makes it feel complete. Before citing any prior search as settling a question, list the axes it covers against the axes that exist NOW. Compliance failure against item #182, whose examples are populations of ROWS rather than DIMENSIONS of a search.**

**Instance L815-promoted (B2882): the generalised half of L815 - a check proving its members from recent HISTORY makes commit ORDER part of what it checks - is promoted to the skill tripwire table and pinned by test_b2123; the measurement stays in LEARNINGS because a row carrying a count decays. Compliance failure against item #231: L628 carried the INSTRUCTION (land members together) and never the REASON, and the reason is the half that reaches past Phase 5 to any history-reading gate.**

**Instance L815 (B2881): A REMEDIATION SPLIT ACROSS COMMITS MAKES ITS OWN GATES ALTERNATE. Phase-5 members are proved by _artifact_touched, which reads ONE COMMIT DEEP, so a correctly-written entry goes invisible the moment a later commit lands. MEASURED with git log --name-only over the last 12 commits: 9 carry a Phase-5 member, 2 of 9 carry all three, 7 of 9 are partial, 4 of those predating the turn. Five consecutive closes were blocked, each on a different member, each satisfying the one the last had missed - the gates correct every time and the commit ORDER the whole defect. Generalised: when a check proves its members by inspecting recent history rather than the working tree, the order of your commits is part of what is being checked. Compliance failure against item #231 - L628 states the remedy and has never had a mechanism; the preventive gate is filed PROPOSED-NOT-BUILT at S6-B2880.**

**Instance L814-addendum-3 (B2878): A FIX THAT DOES NOT CLEAR THE SYMPTOM IS EVIDENCE OF A SECOND CONSUMER, NOT EVIDENCE THE FIX WAS WRONG. After the trunk launch-classifier fix the gate still fired; the reflex was to doubt the patch and widen it, which is how an escape drifts until it fails open (L528). The cheaper question is which OTHER code asks the same question - one grep. MEASURED: three patches to one question across two detectors in one turn, and S6-B1925a records the same leaf-vs-trunk split between the same two detectors for the heredoc strip. Compliance failure against item #262 (carry the rule to the trunk, do not re-learn it per consumer), which covers the REMEDY; this instance records the DIAGNOSTIC that identifies the situation, promoted to the skill tripwire table and pinned by test_b2123.**

**Instance L814 (B2875): FIXING A MEASUREMENT BREAKS EVERY INFERENCE KEYED ON THE DEFECT'S ARTIFACT. Correcting a band counter that scored prose bands as one level immediately made a second number wrong: the factorial footer read n == 1 as "no env actuator", a proxy that held only because the bug produced that value, so unactuated axes began counting as engine runs. The invalidated inference sat SEVEN LINES BELOW the edit, so #196's grep-for-shipped-conclusions would not have found it. After correcting any measurement, read every consumer and ask whether it read a real property or a side-effect of the bug; the tell is a consumer testing for a SPECIFIC VALUE rather than using a magnitude. Compliance failure against #196.**
...
### #311 - CLASSIFY ABSENCE AGAINST LIFECYCLE BEFORE A CHECK MAY INDICT IT (L847, S6-B2947/S6-B3006, 2026-09-22)
...
### #313 - A REGISTRY FIELD READ AS BOTH LABEL AND KEY NEEDS THE MEMBER WHERE THEY DIFFER (L849, S6-B3015, 2026-09-23)

When a registry field is consumed both as display text and as a lookup key, the convention holds only while every member's label happens to equal its key - and it fails SILENTLY on the first member where they differ, rendering a not-recorded token about data that is recorded. MEASURED: SPECS `param` served as both, so `cfg.get(id + "_" + param)` reconstructed smc's keys exactly and missed all four candle axes, printing `?` across fourteen landed configs of the live campaign while a sibling renderer read the same values correctly. Split the field (here: an optional `cfg_key` defaulting to the old expression, so no existing family changes), and pin the member whose two readings differ - if none exists, the next family added is where the defect will appear. Enforced by test_b3018_table_c_reads_the_family_own_config_keys, which asserts the declared keys, asserts smc still uses the default, and MUTATES the cfg_key away to prove the owner-caught `?` returns.

### #314 - A WRITER DECLARES THE LINE ENDING ITS TARGET IS STORED WITH, AND INFERENCE USES THE MAJORITY (L850, S6-B3024, 2026-09-23)

**INSTANCE 2 (L858, B3065/B3067, 2026-09-23).** This item's rule - never infer the ending from presence - was violated twice in one turn and neither instance was visible in a diff. First, a patch concatenated an already-converted anchor and then converted again, producing a DOUBLED carriage return; the text attribute collapsed the trailing pair on commit and stored a line ending in a bare carriage return, so CHECKLIST.md went to the mixed state that test_b2921_canonical_doc_eol_state_is_pinned refuses - after the defect had already reached a commit. Second, recording that lesson meant writing prose ABOUT carriage returns, and sending it through a bash heredoc turned the escapes into real ones, taking LEARNINGS.md from 0 two-byte endings to 5. **So: hold the payload in plain newlines, convert ONCE at the final write, and keep escape sequences out of heredocs - the topic of this rule is precisely the prose that re-triggers it.**

Any code appending to a tracked text file writes the ending git STORES that file with - read it with `git show HEAD:<path>`, never from the working tree, whose checkout form is the platform's. A writer cannot detect its own mismatch (it does not read what it appends to), so this is decided once at the constant. Where a helper must infer, it takes the DOMINANT ending, never any-occurrence: presence-based inference lets one foreign line convert an entire file. MEASURED: the B2520 landing hook wrote CRLF into an LF-stored EXECUTION_QUEUE.md on every landing - invisible on its own commits, since a new line reads as added either way - and presence-based helpers turned that single row into a 120-line flip that preflight C13 refused. Classify before fixing: of three CRLF sites in scripts/, one was the defect, one (a Windows .cmd writer) is REQUIRED to use CRLF, and one was correct-but-latent. Enforced by test_b3024_writers_match_the_stored_line_ending_of_their_target, which pins the constant, re-derives the stored form from git for all five canonical docs, asserts a lone stray CRLF does not flip the inference, and asserts the legitimate .cmd site is left alone.

### #314 AMENDED (B3024b) - MATCH THE TARGET'S DOMINANT ENDING; THE STORED FORM IS NOT THE WRITER'S QUESTION

The item above says a writer should emit the ending git STORES the file with. **That is wrong wherever `core.autocrlf` rewrites on checkout**, which is the default on Windows and is set true in this repo: git then stores LF and hands the working tree CRLF, so a writer matching the STORED form emits a line foreign to every neighbour it is appended beside. MEASURED: `EXECUTION_QUEUE.md` has no `.gitattributes` entry, and setting the landing row to LF on the stored-form premise turned `test_b2522` red on the next pyramid. The correct rule is one question lower: **read the target file and match its DOMINANT ending**, falling back to LF when it is absent or empty; hardcode neither value, and never infer from presence, since one foreign line then converts every later append. Enforced by test_b3024_landing_row_matches_the_target_files_dominant_ending, which drives a CRLF tree, an LF tree, a tree with one stray CRLF and an absent file, asserts the appended row terminates like its neighbours in each, and asserts the seeded bytes are left untouched.

### #315 - A CHANGE'S TEST SELECTOR NAMES THE PINS IT BREAKS, NOT ONLY THE PINS IT ADDS (L851, S6-B3027, 2026-09-23)

Any patch, held patch, or behaviour change that ships a `-k` selector or a targeted test list answers one question first: WHICH EXISTING PINS ASSERT THE BEHAVIOUR I AM CHANGING? Those go in the selector beside the new ones. A selector written from intent names what was built, never what was invalidated - and the pins encoding the old behaviour are precisely the ones a deliberate change should expect to break. MEASURED: a parked patch adding an N/A branch to a fail-closed path named that path's block three times in its own docstring and said its new pin mirrors that harness, while its checklist ran only the new pin - so a standing pin contradicting an owner ruling would have surfaced at the pyramid late in a large apply batch instead of at a two-second targeted run. Detection is JUDGMENT-ONLY: no scan can know which pins a behaviour change invalidates without running them. Durability is the worktree control itself - apply the patch in an isolated worktree, run the FULL suite, and diff against a control run of the same suite on clean HEAD in the same worktree (#276b), which is what surfaced this and is the step that belongs in every held patch's checklist.

### #315 AMENDED (B3034) - THE SELECTOR RUNS AFTER WHAT IT READS, AND VALIDATION RUNS THE WHOLE CHECKLIST

Two additions, both from L852. (a) **ORDER.** A targeted test step runs AFTER every regeneration, snapshot or sync step whose output its pins read. Ask of each verification step which artifacts it reads and whether anything above it produces them; a step order is a dependency graph, not the order the work was thought of. MEASURED: three pins reading generated artifacts sat above the two commands that regenerate them, so they could not pass at the point the selector ran, whatever the patch did. (b) **UNIT OF VALIDATION.** A held patch is its CHECKLIST, not its script. Validating by running the script alone leaves a tree in no valid state - neither pre-apply nor post-apply - and every pin spanning source and artifact fails in it, indistinguishable from real breakage: 3 of 6 new failures in the B3028 diff were this, and one was investigated for a session as an uncovered gap that the patch had already covered. Durability for the vacuous-selector half is now MECHANICAL - `test_b3033_held_patch_selector_terms_name_tests_that_exist` refuses any held-patch `-k` term naming no test in the suite and none the patch appends; it catches all three historical defects (`f_002`, `15457`, `b2919_terminal`). The order and whole-checklist halves remain JUDGMENT-ONLY.

### #316 - EVERY ARM OF ONE BOOLEAN COMPARES THE SAME WAY (L853, S6-B3036, 2026-09-23)

When a single acceptance or rejection expression ORs several arms together, every arm normalises its input identically - same case folding, same whitespace, same stripping - or each divergence is justified IN THE CODE beside the arm. A mixed boolean fails as a FALSE NEGATIVE on the compliant author who writes naturally, which is the direction nobody reports: the arm that rejects wrongly is the one whose victim assumes they got the wording wrong. MEASURED: `#300`'s pin ORed three case-sensitive arms with one `.lower()` arm, so a durability clause opening a sentence with a capital D was invisible, and both #315 sections failed (written WITHOUT the header marker on purpose: the #300 pin splits sections on that token, so quoting it here would split THIS item in two and hide its JUDGMENT-ONLY from its own durability clause - the defect was live in the first draft of this line) while carrying the exact clause the rule demands. THE FIX IS NOT UNIFORM NORMALISATION: where an arm is an ESCAPE, it takes the stricter matcher (L528) - `"Mechanism"` stays capitalised because it marks L548's structured form and lowercasing it would match the prose "no mechanism is possible", the bare label the rule refuses. WHICH CANDIDATE WAS COMPARED (L720): L771 covers a PROBE diverging from the ASSERTION it validates - two expressions drifting apart over time - and does not reach one expression whose own arms disagree at authoring time; #226 proves a pin CAN fail and is silent about which compliant inputs it wrongly rejects. Mechanism: the arm's verdict is now pinned in BOTH directions by the five-case table inside `test_b2705_judgment_only_declarations_carry_their_search` - two must-refuse (bare label, "no mechanism is possible") and three must-accept (capitalised, lowercase, L548 form).

### #317 - A NEW TICKET ID IS CHECKED FREE BEFORE IT IS WRITTEN (L854, S6-B3036, 2026-09-23)

Before appending a row that OPENS a ticket, call `queue_state.is_used(tid)` - or take one from `queue_state.next_free(prefix)`. Never derive a ticket id from the current batch number: batch numbers advance every turn and ticket ids are allocated ad hoc, so the two sequences agree often enough to feel safe and diverge exactly when a ticket was opened recently. MEASURED: `S6-B3035` was opened as an OPEN owner decision and reused the next turn, from the batch number, for unrelated work marked EXECUTED - last-row-wins then made the owner's question terminal, the OPEN count fell 3 to 2, and that table was published. NO LEDGER INVARIANT CAN SEE THIS: `terminal-not-last` stayed 0, because a terminal row following a non-terminal one is exactly the shape of a legitimate close, so the defect satisfied every check the queue performs. Distinct from L640, which guards the MIRROR - a non-terminal row appended to a TERMINAL id - and fires loudly because it breaks last-row-wins; here last-row-wins works perfectly and wins for the wrong subject. Mechanism: `queue_state.is_used` / `next_free`, pinned by `test_b3036_a_ticket_id_in_use_is_reported_as_taken`, which asserts both directions and keeps the recorded collision readable. The COLLISION itself (same id, unrelated subjects) is JUDGMENT-ONLY - deciding it needs the subject, which no scan reads - and its durability is that pin plus this item.
