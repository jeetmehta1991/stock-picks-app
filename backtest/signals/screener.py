...
# subsets at the producer: get_classification_change_signals emits every
# destination/origin key in the SAME return dict as
# classification_changed_recent=True (universe.py, one emission site),
# and new_sector/prior_sector ride the signals dict, so every deleted
# variant's population is reconstructable OFFLINE from the survivor's
# cube fires (the B620 justification). Deleted: to_tech_long,
# to_defensive_short, volume_long, momentum_long, from_tech_short,
# breakout_long, with_institutional_long, with_insider_long,
# oversold_long. The producer keys stay (data, narrow scope). The
# survivor is unpromoted, so no mirror obligation triggers.
