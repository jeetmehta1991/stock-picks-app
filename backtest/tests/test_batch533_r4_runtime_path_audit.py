...
def test_batch533_drift_audit_carries_disambiguated_aws_keys():
    ...
    B907 (2026-06-18) RESOLVED via B900-DEFER-G: drift_audit_pre_phase_1a_beta.py
    extended to emit phase_1a_beta_aws_instance_type + parallel_instances +
...
def test_batch533_aws_instance_matches_drift_audit():
    ...
    is stale.
    B907 (2026-06-18) RESOLVED via B900-DEFER-G: drift_audit script extended.
    """
    data = json.loads(
        (REPO / "output_audit" / "drift_audit_live_values.json").read_text(
            encoding="utf-8",
