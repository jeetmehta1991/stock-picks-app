...
def test_b1801_synthetic_provenance():
    """S6-B1705e (#201 provenance half): a quoted number must name its input.

    `#201` asks whether a quantity was COMPUTED. It never asks what FROM - which
    is why **2.422, generated by `rng.normal(1, 3, 30)` inside my own boundary
    probe, satisfied it completely.** "Measured" was true of the arithmetic and
    false of the meaning. The probe's one real finding was the BOUNDARY
    (n=29 -> None, n=30 -> a value); the number measured nothing.

    Six arms, including four that must stay QUIET - a gate proven only on its
    incident is a gate whose false-positive half is unknown (B1767).
    """
    import importlib.util
    import pathlib as _p
    ...
    root = _p.Path(__file__).resolve().parents[2]
    spec = importlib.util.spec_from_file_location(
        "vtc_b1801", root / "scripts" / "verify_turn_compliance.py")
    tg = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(tg)

    incident = ("The boundary probe measured n=29 -> None and n=30 -> a Sharpe "
                "of 2.422, so the floor is real.")
    gen = "pnl = rng.normal(1, 3, 30)"

    assert tg.scan_synthetic_provenance([], text=incident, tool_text=gen),         "must fire: a decimal in measurement language beside a generator"
    assert not tg.scan_synthetic_provenance(
        [], text=incident + " This figure is SYNTHETIC.", tool_text=gen),         "one word of provenance must clear it - the label IS the remedy"
    assert not tg.scan_synthetic_provenance(
        [], text=incident, tool_text="pd.read_csv('trade_exit_detail.csv')"),         "must be quiet when the number came from real data"
    assert not tg.scan_synthetic_provenance(
        [], text="The probe measured the boundary and it holds.", tool_text=gen),         "must be quiet when no number is quoted - the boundary WAS a real finding"
    assert not tg.scan_synthetic_provenance(
